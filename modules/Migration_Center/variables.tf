...
variable "enable_rad_gcpproject" {
  description = "Set to false to hide the \"GCP Project on RAD\" option for this module -- it may only be deployed into a customer's own GCP project. This module enables migrationcenter, which is denied in the RAD-managed production tier and absent from the sandbox and development allowlists. {{UIMeta group=0 order=110 }}"
  type        = bool
  default     = false
}
