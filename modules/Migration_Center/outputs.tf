...
output "windows_vm_external_ip" {
  description = "External IP address of the Windows VM — use this to connect via RDP. Username: migrationcenter. See 'windows_vm_password' output for the password."
...
output "windows_vm_password" {
  description = "Randomly generated RDP password for the Windows VM 'migrationcenter' user."
  value       = var.create_windows_vm ? random_password.windows_password[0].result : null
  sensitive   = true
}
