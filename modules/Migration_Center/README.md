...
| <a name="input_aws_region"></a> [aws\_region](#input\_aws\_region) | AWS region to discover EC2 instances from (e.g. 'us-east-1'). {{UIMeta group=8 order=805 }} | `string` | `"us-east-1"` | no |
...
| <a name="input_enable_rad_gcpproject"></a> [enable\_rad\_gcpproject](#input\_enable\_rad\_gcpproject) | Set to false to hide the "GCP Project on RAD" option for this module -- it may only be deployed into a customer's own GCP project. This module enables migrationcenter, which is denied in the RAD-managed production tier and absent from the sandbox and development allowlists. {{UIMeta group=0 order=110 }} | `bool` | `false` | no |
