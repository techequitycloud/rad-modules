...
variable "bank_of_anthos_version" {
  description = "The version of the Bank of Anthos application to deploy."
  type        = string
  default     = "v0.6.7"
}
...
variable "existing_project_id" {
  description = "The ID of an existing Google Cloud project to deploy the resources into."
  type        = string

  validation {
    condition     = var.existing_project_id != ""
    error_message = "The existing_project_id must not be empty."
  }
}
...
variable "region" {
  ...
  validation {
    ...
    error_message = "The region must be a valid GCP region, e.g. 'us-west1'."
  }
}
