...
resource "null_resource" "download_bank_of_anthos" {
  ...
  triggers = {
    ...
    always_run    = "true" # ⚡ Bolt Optimization: Use static trigger to avoid replacements, handle idempotency in bash
  }
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      if [ -d "${local.extracted_path}" ] && [ -f "${local.extracted_path}/extras/jwt/jwt-secret.yaml" ]; then
        echo "✓ Bank of Anthos ${local.bank_of_anthos_version} already downloaded and extracted. Skipping."
        exit 0
      fi
