...
resource "null_resource" "download_bank_of_anthos" {
  ...
  triggers = {
    ...
    # PERFORMANCE: Removed always_run = timestamp() to prevent unnecessary re-downloads and re-deployments on every apply
  }
...
resource "null_resource" "deploy_bank_of_anthos" {
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      # It is normally created by kubernetes_namespace.bank_of_anthos.
