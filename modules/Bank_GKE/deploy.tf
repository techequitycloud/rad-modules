...
resource "null_resource" "download_bank_of_anthos" {
  ...
  triggers = {
    ...
    # PERFORMANCE OPTIMIZATION: Removed always_run = timestamp() to prevent
    # this provisioner from running on every apply, which breaks the downstream
    # replacement chain and speeds up applies without losing idempotency.
  }
