...
variable "module_services" {
  ...
  type        = list(string)
  default     = ["GCP", "GKE", "Cloud Service Mesh", "Cloud IAM", "Cloud Networking", "VPC Network", "GKE Fleet", "Workload Identity", "Cloud Monitoring"]
}
...
variable "enable_rad_gcpproject" {
  description = "Set to false to hide the \"GCP Project on RAD\" option for this module -- it may only be deployed into a customer's own GCP project. This module enables 9 APIs the RAD-managed tier policies deny (anthos, anthosconfigmanagement, anthospolicycontroller, gkehub, gkeconnect, mesh, meshconfig, containersecurity, websecurityscanner). {{UIMeta group=0 order=110 }}"
  type        = bool
  default     = false
}
