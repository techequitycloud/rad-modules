...
For a detailed technical walkthrough of the full implementation, see [Bank\_GKE.md](../../docs/modules/Bank_GKE.md).
...
| <a name="provider_kubernetes.primary"></a> [kubernetes.primary](#provider\_kubernetes.primary) | 3.2.1 |
...
| <a name="input_enable_rad_gcpproject"></a> [enable\_rad\_gcpproject](#input\_enable\_rad\_gcpproject) | Set to false to hide the "GCP Project on RAD" option for this module -- it may only be deployed into a customer's own GCP project. This module enables 9 APIs the RAD-managed tier policies deny (anthos, anthosconfigmanagement, anthospolicycontroller, gkehub, gkeconnect, mesh, meshconfig, containersecurity, websecurityscanner). {{UIMeta group=0 order=110 }} | `bool` | `false` | no |
...
| <a name="input_region"></a> [region](#input\_region) | GCP region where the GKE cluster, VPC, and all supporting resources will be deployed (e.g. 'us-central1', 'europe-west1'). Defaults to 'us-central1'. Deployment may fail if sufficient resource quota is not available in the selected region. {{UIMeta group=1 order=103 }} | `string` | `"us-central1"` | no |
