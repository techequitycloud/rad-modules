...
variable "module_description" {
  ...
  type        = string
  default     = "Strapi: Deploy open-source Headless CMS on Google Cloud Run for performant, customizable APIs with user-friendly content editor interface. Omnichannel content delivery to websites, mobile apps, and IoT from single source with data sovereignty. Includes Strapi v4/v5 on Node.js 20 Alpine, PostgreSQL 15 via Cloud SQL with Unix socket connection, GCS integration for media assets using @strapi-community/strapi-provider-upload-google-cloud-storage, and automated database initialization. Features Cloud Run Gen2, multi-stage Docker build with vips-dev for sharp image processing, automated db-init job for idempotent setup, Secret Manager for JWT secrets and credentials, startup/liveness probes, autoscaling (0-3 instances), optional SMTP email support, tini signal handling, non-root execution, dedicated strapi-uploads bucket, private VPC connectivity, and secure configuration injection. Ideal for developer freedom with customizable Node.js APIs, omnichannel delivery, enterprise scalability for high traffic, self-hosted data sovereignty, and modern headless CMS architecture."
}
...
variable "module_services" {
  ...
  type        = list(string)
  default     = ["Cloud Run Gen2", "Cloud Run Jobs", "Cloud Build", "Artifact Registry", "Cloud Storage", "GCS Media Storage", "Cloud SQL", "Cloud SQL Auth Proxy", "PostgreSQL 15", "Unix Socket", "VPC Network", "Serverless VPC Access", "Private IP", "Secret Manager", "Cloud IAM", "Service Accounts", "Cloud Logging", "Cloud Monitoring", "Health Checks", "SMTP Integration", "Node.js 20"]
}
...
variable "application_version" {
  description = "Application version tag (e.g., 5.5.0, latest). {{UIMeta group=0 order=3002 updatesafe }}"
  type        = string
  default     = "5.5.0"
}
