...
variable "module_services" {
  ...
  type        = list(string)
  default     = ["GCP", "GKE", "Migrate to Containers", "Cloud Compute", "Cloud Networking", "Cloud IAM", "VPC Network", "Compute Engine"]
}
...
variable "enable_rad_gcpproject" {
  description = "Set to false to hide the \"GCP Project on RAD\" option for this module -- it may only be deployed into a customer's own GCP project. This module enables containerregistry, which is absent from the RAD-managed sandbox and development allowlists. {{UIMeta group=0 order=110 }}"
  type        = bool
  default     = false
}
