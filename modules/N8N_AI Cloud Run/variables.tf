...
variable "module_description" {
  ...
  type        = string
  default     = "N8N AI: Deploy n8n workflow automation with integrated AI capabilities on Google Cloud Run. Includes n8n automation engine, Qdrant vector database for RAG, and Ollama LLM server for running local AI models (Llama 3, Mistral, Gemma). Build intelligent agents, chatbots, and document analysis workflows with no-code drag-and-drop interface. Features privacy-first AI (models run on your infrastructure), vector database for document embeddings and AI memory, automatic service integration, internal VPC networking, swappable open-source models, optional GPU acceleration, and predictable costs. Includes PostgreSQL 15, GCS Fuse persistence, and feature toggles for flexible AI stack configuration. Ideal for enterprise AI automation, RAG document search, intelligent chatbots, AI agents, and secure GenAI workflows without ML engineering expertise."
}

variable "module_documentation" {
  description = "The URL to the module documentation. {{UIMeta group=0 order=102 }}"
  type        = string
  default     = "https://docs.techequity.cloud/docs/applications/n8n-ai"
}
...
variable "module_services" {
  ...
  type        = list(string)
  default     = ["Cloud Run", "Cloud Run Jobs", "Cloud Run GPU", "Cloud Build", "Artifact Registry", "Cloud Storage", "GCS Fuse", "Cloud SQL (PostgreSQL 15)", "VPC Network", "Serverless VPC Access", "Private Service Connect", "Internal DNS", "Secret Manager", "Cloud IAM", "Cloud Logging", "Cloud Monitoring", "Health Checks", "Webhooks", "Session Affinity", "Qdrant Vector Database", "Ollama LLM Server"]
}
...
variable "application_version" {
  description = "Application version tag (e.g., 1.0.0, latest). {{UIMeta group=0 order=4002 updatesafe }}"
  type        = string
  default     = "2.4.7"
}
...
variable "application_name" {
  description = "Application name used in resource naming. Must start with a letter and contain only lowercase letters, numbers, and hyphens (1-20 characters). {{UIMeta group=0 order=4004 updatesafe }}"
  type        = string
  default     = "n8nai"
...
variable "application_display_name" {
  description = "Human-readable application name for display purposes. {{UIMeta group=0 order=4005 updatesafe }}"
  type        = string
  default     = "N8N AI Starter Kit"
}

variable "enable_ai_components" {
  description = "Set to true to enable AI components (Qdrant, Ollama). {{UIMeta group=0 order=4006 }}"
  type        = bool
  default     = true
}

variable "enable_qdrant" {
  description = "Set to true to enable Qdrant vector database. Requires enable_ai_components=true. {{UIMeta group=0 order=4007 }}"
  type        = bool
  default     = true
}

variable "qdrant_version" {
  description = "Qdrant Docker image version/tag. {{UIMeta group=0 order=4008 }}"
  type        = string
  default     = "latest"
}

variable "enable_ollama" {
  description = "Set to true to enable Ollama LLM provider. Requires enable_ai_components=true. {{UIMeta group=0 order=4009 }}"
  type        = bool
  default     = true
}

variable "ollama_version" {
  description = "Ollama Docker image version/tag. {{UIMeta group=0 order=4010 }}"
  type        = string
  default     = "latest"
}

variable "ollama_model" {
  description = "The Ollama model to pull on startup (if script implemented) or for documentation. Default is llama3.2. {{UIMeta group=0 order=4011 }}"
  type        = string
  default     = "llama3.2"
}
