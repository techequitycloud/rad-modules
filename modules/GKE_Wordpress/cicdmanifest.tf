...
# Resource for creating the 'docker-entrypoint.sh' file in the GitHub repository on the 'dev' branch
resource "github_repository_file" "dev_docker_entrypoint" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "dev"
  commit_message      = "Add docker-entrypoint.sh to repo"
  overwrite_on_create = true
  file                = "docker-entrypoint.sh"
  content             = data.local_file.dev_docker_entrypoint[count.index].content
...
data "local_file" "dev_docker_entrypoint" {
  count    = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
  filename = "${path.module}/scripts/ci/docker-entrypoint.sh"
}

# Resource definition for a GitHub repository file for the hazelcast configuration
resource "github_repository_file" "dev_wp_config_docker" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "qa"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "wp-config-docker.php"
  content             = data.local_file.dev_wp_config_docker[count.index].content
...
data "local_file" "dev_wp_config_docker" {
  count    = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  filename = "${path.module}/scripts/ci/wp-config-docker.php"
}
...
# Resource for creating the 'docker-entrypoint.sh' file in the GitHub repository on the 'qa' branch
resource "github_repository_file" "qa_docker_entrypoint" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "qa"
  commit_message      = "Add docker-entrypoint.sh to repo"
  overwrite_on_create = true
  file                = "docker-entrypoint.sh"
  content             = data.local_file.qa_docker_entrypoint[count.index].content
...
# Resource definition for a GitHub repository file for the hazelcast configuration
resource "github_repository_file" "qa_wp_config_docker" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "qa"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "wp-config-docker.php"
  content             = data.local_file.qa_wp_config_docker[count.index].content
...
# Resource for creating the 'docker-entrypoint.sh' file in the GitHub repository on the 'prod' branch
resource "github_repository_file" "prod_docker_entrypoint" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_production_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "prod"
  commit_message      = "Add docker-entrypoint.sh to repo"
  overwrite_on_create = true
  file                = "docker-entrypoint.sh"
  content             = data.local_file.prod_docker_entrypoint[count.index].content
...
# Resource definition for a GitHub repository file for the hazelcast configuration
resource "github_repository_file" "prod_wp_config_docker" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_production_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "prod"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "wp-config-docker.php"
  content             = data.local_file.prod_wp_config_docker[count.index].content
