...
# Conditionally create service account only if it doesn't exist
module "create_service_accounts" {
  ...
  generate_keys = true
  project_roles = [
    "${var.existing_project_id}=>roles/gkehub.connect",
    "${var.existing_project_id}=>roles/gkehub.admin",
    "${var.existing_project_id}=>roles/logging.logWriter",
    "${var.existing_project_id}=>roles/monitoring.metricWriter",
    "${var.existing_project_id}=>roles/monitoring.dashboardEditor",
    "${var.existing_project_id}=>roles/stackdriver.resourceMetadata.writer",
    "${var.existing_project_id}=>roles/opsconfigmonitoring.resourceMetadata.writer",
  ]
}

# Reference existing service account (no key generation)
data "google_service_account" "anthos_sa" {
  account_id = var.anthos_service_account_name
...
resource "google_project_iam_member" "anthos_sa_roles" {
  for_each = toset([
  ...
  project = var.existing_project_id
  role    = each.value
  member  = "serviceAccount:${data.google_service_account.anthos_sa.email}"
...
resource "local_file" "cluster_yaml_bundledlb" {
  ...
  content = templatefile(local.cluster_yaml_template_file, {
    ...
    projectId       = var.existing_project_id,
...
resource "local_file" "init_args_file" {
  ...
  # Use nonsensitive() to unwrap the sensitive value
  for_each = toset(nonsensitive(local.vm_hostnames))
...
module "gke_hub_membership" {
  ...
  destroy_cmd_body      = "container hub memberships delete --quiet --project ${var.existing_project_id} ${var.abm_cluster_id} --verbosity=none || true"
}
