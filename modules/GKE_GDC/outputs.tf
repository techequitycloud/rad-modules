...
output "admin_vm_ssh" {
  ...
  value = var.mode != "setup" ? null : join("\n", [
    ...
    "################################################################################",
    "",
    "> gcloud compute ssh ${var.username}@${nonsensitive(local.admin_vm_hostnames[0])} --project=${var.existing_project_id} --zone=${var.zone}",
    "",
    "# ------------------------------------------------------------------------------",
    "# You must be SSH'ed into the admin host ${nonsensitive(local.admin_vm_hostnames[0])} as ${var.username} user now",
    "# ------------------------------------------------------------------------------",
    "> sudo -E ./run_initialization_checks.sh && \\",
    "  sudo -E bmctl create config -c ${var.abm_cluster_id} && \\",
    "  sudo -E cp ~/${var.abm_cluster_id}.yaml bmctl-workspace/${var.abm_cluster_id} && \\",
    "  sudo -E bmctl create cluster -c ${var.abm_cluster_id}",
