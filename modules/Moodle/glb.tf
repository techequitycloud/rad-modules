# Copyright 2024 (c) Tech Equity Ltd
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     https://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

#########################################################################
# Configure Dev resources
#########################################################################

# Reserve a global IP address for the load balancer
resource "google_compute_global_address" "dev" {
  count   = length(local.regions) >= 2 && var.configure_development_environment ? 1 : 0
  project = local.project.project_id
  name = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}dev"
}

resource "google_compute_backend_service" "dev_default" {
  count                 = length(local.regions) >= 2 && var.configure_development_environment ? 1 : 0
  project               = local.project.project_id
  provider              = google-beta
  name                  = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}dev"
  load_balancing_scheme = "EXTERNAL_MANAGED"

  dynamic "backend" {
    for_each = local.regions  # Iterate over each region in local.regions

    content {
      group = google_compute_region_network_endpoint_group.dev_default[backend.key].id
    }
  }
}

resource "google_compute_url_map" "dev_default" {
  count           = length(local.regions) >= 2 && var.configure_development_environment ? 1 : 0
  project         = local.project.project_id
  provider        = google-beta
  name            = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}dev-lb-urlmap"
  default_service = google_compute_backend_service.dev_default[count.index].id

  path_matcher {
    name            = "allpaths"
    default_service = google_compute_backend_service.dev_default[count.index].id
    route_rules {
      priority = 1
      url_redirect {
        https_redirect         = true
        redirect_response_code = "MOVED_PERMANENTLY_DEFAULT"
      }
    }
  }
}

resource "google_compute_managed_ssl_certificate" "dev_default" {
  count    = length(local.regions) >= 2 && var.configure_development_environment ? 1 : 0
  project  = local.project.project_id
  provider = google-beta
  name     = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}dev-ssl-cert"

  managed {
    domains = ["app${var.application_name}${var.tenant_deployment_id}${local.random_id}dev.${google_compute_global_address.dev[count.index].address}.nip.io"]
  }
}

resource "google_compute_target_https_proxy" "dev_default" {
  count    = length(local.regions) >= 2 && var.configure_development_environment ? 1 : 0
  project  = local.project.project_id
  provider = google-beta
  name     = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}dev-https-proxy"
  url_map  = google_compute_url_map.dev_default[count.index].id
  ssl_certificates = [
    google_compute_managed_ssl_certificate.dev_default[count.index].name
  ]

  depends_on = [
    google_compute_managed_ssl_certificate.dev_default
  ]
}

resource "google_compute_global_forwarding_rule" "dev_default" {
  count                 = length(local.regions) >= 2 && var.configure_development_environment ? 1 : 0
  project               = local.project.project_id
  provider              = google-beta
  name                  = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}dev-lb-fr"
  load_balancing_scheme = "EXTERNAL_MANAGED"
  target                = google_compute_target_https_proxy.dev_default[count.index].id
  ip_address            = google_compute_global_address.dev[count.index].id
  port_range            = "443"

  depends_on            = [
    google_compute_target_https_proxy.dev_default
  ]
}

resource "google_compute_region_network_endpoint_group" "dev_default" {
  count                 = length(local.regions) >= 2 && var.configure_development_environment ? length(local.regions) : 0
  project               = local.project.project_id
  provider              = google-beta
  name                  = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}dev-neg-${count.index}"
  network_endpoint_type = "SERVERLESS"
  region                = local.regions[count.index]
  cloud_run {
    # Construct the key for the for_each map in service.tf: "dev-<region>"
    service = google_cloud_run_v2_service.app_service["dev-${local.regions[count.index]}"].name
  }

  depends_on = [
    google_cloud_run_v2_service.app_service
  ]
}

resource "google_compute_url_map" "dev_https" {
  count    = length(local.regions) >= 2 && var.configure_development_environment ? 1 : 0
  project  = local.project.project_id
  provider = google-beta
  name     = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}dev-https-urlmap"

  default_url_redirect {
    redirect_response_code = "MOVED_PERMANENTLY_DEFAULT"
    https_redirect         = true
    strip_query            = false
  }
}

resource "google_compute_target_http_proxy" "dev_https" {
  count    = length(local.regions) >= 2 && var.configure_development_environment ? 1 : 0
  project  = local.project.project_id
  provider = google-beta
  name     = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}dev-http-proxy"
  url_map  = google_compute_url_map.dev_https[count.index].id

  depends_on = [
    google_compute_url_map.dev_https
  ]
}

resource "google_compute_global_forwarding_rule" "dev_https" {
  count      = length(local.regions) >= 2 && var.configure_development_environment ? 1 : 0
  project    = local.project.project_id
  provider   = google-beta
  name       = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}dev-https-fr"
  target     = google_compute_target_http_proxy.dev_https[count.index].id
  ip_address = google_compute_global_address.dev[count.index].id
  port_range = "80"

  depends_on = [
    google_compute_target_http_proxy.dev_https
  ]
}

#########################################################################
# Configure QA resources
#########################################################################

# Reserve a global IP address for the load balancer
resource "google_compute_global_address" "qa" {
  count   = length(local.regions) >= 2 && var.configure_nonproduction_environment ? 1 : 0
  project = local.project.project_id
  name = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}qa"
}

resource "google_compute_backend_service" "qa_default" {
  count                 = length(local.regions) >= 2 && var.configure_nonproduction_environment ? 1 : 0
  provider              = google-beta
  project               = local.project.project_id
  name                  = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}qa"
  load_balancing_scheme = "EXTERNAL_MANAGED"

  dynamic "backend" {
    for_each = local.regions  # Iterate over each region in local.regions

    content {
      group = google_compute_region_network_endpoint_group.qa_default[backend.key].id
    }
  }
}

resource "google_compute_url_map" "qa_default" {
  count           = length(local.regions) >= 2 && var.configure_nonproduction_environment ? 1 : 0
  project         = local.project.project_id
  provider        = google-beta
  name            = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}qa-lb-urlmap"
  default_service = google_compute_backend_service.qa_default[count.index].id

  path_matcher {
    name            = "allpaths"
    default_service = google_compute_backend_service.qa_default[count.index].id
    route_rules {
      priority = 1
      url_redirect {
        https_redirect         = true
        redirect_response_code = "MOVED_PERMANENTLY_DEFAULT"
      }
    }
  }
}

resource "google_compute_managed_ssl_certificate" "qa_default" {
  count    = length(local.regions) >= 2 && var.configure_nonproduction_environment ? 1 : 0
  project  = local.project.project_id
  provider = google-beta
  name     = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}qa-ssl-cert"

  managed {
    domains = ["app${var.application_name}${var.tenant_deployment_id}${local.random_id}qa.${google_compute_global_address.qa[count.index].address}.nip.io"]
  }
}

resource "google_compute_target_https_proxy" "qa_default" {
  count    = length(local.regions) >= 2 && var.configure_nonproduction_environment ? 1 : 0
  project  = local.project.project_id
  provider = google-beta
  name     = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}qa-https-proxy"
  url_map  = google_compute_url_map.qa_default[count.index].id
  ssl_certificates = [
    google_compute_managed_ssl_certificate.qa_default[count.index].name
  ]

  depends_on = [
    google_compute_managed_ssl_certificate.qa_default
  ]
}

resource "google_compute_global_forwarding_rule" "qa_default" {
  count                 = length(local.regions) >= 2 && var.configure_nonproduction_environment ? 1 : 0
  project               = local.project.project_id
  provider              = google-beta
  name                  = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}qa-lb-fr"
  load_balancing_scheme = "EXTERNAL_MANAGED"
  target                = google_compute_target_https_proxy.qa_default[count.index].id
  ip_address            = google_compute_global_address.qa[count.index].id
  port_range            = "443"
  
  depends_on            = [
    google_compute_target_https_proxy.qa_default
  ]
}

resource "google_compute_region_network_endpoint_group" "qa_default" {
  count                 = length(local.regions) >= 2 && var.configure_nonproduction_environment ? length(local.regions) : 0
  project               = local.project.project_id
  provider              = google-beta
  name                  = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}qa-neg-${count.index}"
  network_endpoint_type = "SERVERLESS"
  region                = local.regions[count.index]
  cloud_run {
    service = google_cloud_run_v2_service.app_service["qa-${local.regions[count.index]}"].name
  }

  depends_on = [
    google_cloud_run_v2_service.app_service
  ]
}

resource "google_compute_url_map" "qa_https" {
  count    = length(local.regions) >= 2 && var.configure_nonproduction_environment ? 1 : 0
  project  = local.project.project_id
  provider = google-beta
  name     = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}qa-https-urlmap"

  default_url_redirect {
    redirect_response_code = "MOVED_PERMANENTLY_DEFAULT"
    https_redirect         = true
    strip_query            = false
  }
}

resource "google_compute_target_http_proxy" "qa_https" {
  count    = length(local.regions) >= 2 && var.configure_nonproduction_environment ? 1 : 0
  project  = local.project.project_id
  provider = google-beta
  name     = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}qa-http-proxy"
  url_map  = google_compute_url_map.qa_https[count.index].id

  depends_on = [
    google_compute_url_map.qa_https
  ]
}

resource "google_compute_global_forwarding_rule" "qa_https" {
  count      = length(local.regions) >= 2 && var.configure_nonproduction_environment ? 1 : 0
  project    = local.project.project_id
  provider   = google-beta
  name       = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}qa-https-fr"
  target     = google_compute_target_http_proxy.qa_https[count.index].id
  ip_address = google_compute_global_address.qa[count.index].id
  port_range = "80"
  depends_on = [
    google_compute_target_http_proxy.qa_https
  ]
}

#########################################################################
# Configure Prod resources
#########################################################################

# Reserve a global IP address for the load balancer
resource "google_compute_global_address" "prod" {
  count   = length(local.regions) >= 2 && var.configure_production_environment ? 1 : 0
  project = local.project.project_id
  name = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}prod"
}

resource "google_compute_backend_service" "prod_default" {
  count                 = length(local.regions) >= 2 && var.configure_production_environment ? 1 : 0
  project               = local.project.project_id
  provider              = google-beta
  name                  = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}prod"
  load_balancing_scheme = "EXTERNAL_MANAGED"

  dynamic "backend" {
    for_each = local.regions  # Iterate over each region in local.regions

    content {
      group = google_compute_region_network_endpoint_group.prod_default[backend.key].id
    }
  }
}

resource "google_compute_url_map" "prod_default" {
  count           = length(local.regions) >= 2 && var.configure_production_environment ? 1 : 0
  project         = local.project.project_id
  provider        = google-beta
  name            = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}prod-lb-urlmap"
  default_service = google_compute_backend_service.prod_default[count.index].id

  path_matcher {
    name            = "allpaths"
    default_service = google_compute_backend_service.prod_default[count.index].id
    route_rules {
      priority = 1
      url_redirect {
        https_redirect         = true
        redirect_response_code = "MOVED_PERMANENTLY_DEFAULT"
      }
    }
  }
}

resource "google_compute_managed_ssl_certificate" "prod_default" {
  count    = length(local.regions) >= 2 && var.configure_production_environment ? 1 : 0
  project  = local.project.project_id
  provider = google-beta
  name     = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}prod-ssl-cert"

  managed {
    domains = ["app${var.application_name}${var.tenant_deployment_id}${local.random_id}prod.${google_compute_global_address.prod[count.index].address}.nip.io"]
  }
}

resource "google_compute_target_https_proxy" "prod_default" {
  count    = length(local.regions) >= 2 && var.configure_production_environment ? 1 : 0
  project  = local.project.project_id
  provider = google-beta
  name     = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}prod-https-proxy"
  url_map  = google_compute_url_map.prod_default[count.index].id
  ssl_certificates = [
    google_compute_managed_ssl_certificate.prod_default[count.index].name
  ]

  depends_on = [
    google_compute_managed_ssl_certificate.prod_default
  ]
}

resource "google_compute_global_forwarding_rule" "prod_default" {
  count                 = length(local.regions) >= 2 && var.configure_production_environment ? 1 : 0
  project               = local.project.project_id
  provider              = google-beta
  name                  = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}prod-lb-fr"
  load_balancing_scheme = "EXTERNAL_MANAGED"
  target                = google_compute_target_https_proxy.prod_default[count.index].id
  ip_address            = google_compute_global_address.prod[count.index].id
  port_range            = "443"

  depends_on            = [
    google_compute_target_https_proxy.prod_default
  ]
}

resource "google_compute_region_network_endpoint_group" "prod_default" {
  count                 = length(local.regions) >= 2 && var.configure_production_environment ? length(local.regions) : 0
  project               = local.project.project_id
  provider              = google-beta
  name                  = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}prod-neg-${count.index}"
  network_endpoint_type = "SERVERLESS"
  region                = local.regions[count.index]
  cloud_run {
    service = google_cloud_run_v2_service.app_service["prod-${local.regions[count.index]}"].name
  }

  depends_on = [
    google_cloud_run_v2_service.app_service
  ]
}

resource "google_compute_url_map" "prod_https" {
  count    = length(local.regions) >= 2 && var.configure_production_environment ? 1 : 0
  project  = local.project.project_id
  provider = google-beta
  name     = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}prod-https-urlmap"

  default_url_redirect {
    redirect_response_code = "MOVED_PERMANENTLY_DEFAULT"
    https_redirect         = true
    strip_query            = false
  }
}

resource "google_compute_target_http_proxy" "prod_https" {
  count    = length(local.regions) >= 2 && var.configure_production_environment ? 1 : 0
  project  = local.project.project_id
  provider = google-beta
  name     = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}prod-http-proxy"
  url_map  = google_compute_url_map.prod_https[count.index].id

  depends_on = [
    google_compute_url_map.prod_https
  ]
}

resource "google_compute_global_forwarding_rule" "prod_https" {
  count      = length(local.regions) >= 2 && var.configure_production_environment ? 1 : 0
  project    = local.project.project_id
  provider   = google-beta
  name       = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}prod-https-fr"
  target     = google_compute_target_http_proxy.prod_https[count.index].id
  ip_address = google_compute_global_address.prod[count.index].id
  port_range = "80"

  depends_on = [
    google_compute_target_http_proxy.prod_https
  ]
}
