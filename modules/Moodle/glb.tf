...
resource "google_compute_backend_service" "lb_default" {
  ...
  dynamic "backend" {
    ...
    content {
      group = google_compute_region_network_endpoint_group.lb_default[backend.key].id
    }
  }
...
resource "google_compute_url_map" "lb_default" {
  ...
  default_service = google_compute_backend_service.lb_default[count.index].id
...
resource "google_compute_managed_ssl_certificate" "lb_default" {
  ...
  managed {
    domains = ["app${var.application_name}${var.tenant_deployment_id}${local.random_id}.${google_compute_global_address.lb[count.index].address}.nip.io"]
  }
...
resource "google_compute_target_https_proxy" "lb_default" {
  ...
  url_map  = google_compute_url_map.lb_default[count.index].id
  ssl_certificates = [
    google_compute_managed_ssl_certificate.lb_default[count.index].name
  ]
...
resource "google_compute_global_forwarding_rule" "lb_default" {
  ...
  ip_address            = google_compute_global_address.lb[count.index].id
  port_range            = "443"
...
resource "google_compute_target_http_proxy" "lb_https" {
  count    = length(local.regions) >= 2 && var.configure_environment ? 1 : 0
  project  = local.project.project_id
  provider = google-beta
  name     = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}-http-proxy"
  url_map  = google_compute_url_map.lb_https[count.index].id

  depends_on = [
    google_compute_url_map.lb_https
  ]
}
...
resource "google_compute_global_forwarding_rule" "lb_https" {
  ...
  target     = google_compute_target_http_proxy.lb_https[count.index].id
  ip_address = google_compute_global_address.lb[count.index].id
  port_range = "80"

  depends_on = [
    google_compute_target_http_proxy.lb_https
  ]
}
