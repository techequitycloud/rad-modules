...
locals {
  ...
  nfs_enabled_envs = toset([for env, enabled in local.nfs_env_config : env if enabled])
}

# Create nfs import script for each environment
resource "local_file" "import_nfs_script_output" {
  for_each = local.nfs_server_exists ? local.nfs_enabled_envs : toset([])
  ...
  content = templatefile("${path.module}/scripts/app/import_nfs.tpl", {
    ...
    DB_NAME             = "app${var.application_database_name}${var.tenant_deployment_id}${local.random_id}${each.key}"
    DB_USER             = "app${var.application_database_name}${var.tenant_deployment_id}${local.random_id}${each.key}"
    APP_NAME            = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}${each.key}"
    ...
    APP_URL             = "https://app${var.application_name}${var.tenant_deployment_id}${local.random_id}${each.key}-${local.project_number}.${local.region}.run.app"
  })
}
    ...
    command = <<-EOF
      ...
      for i in {1..5}; do
        if [ -z "${local.project_sa_email}" ] || [ -z "${var.resource_creator_identity}" ]; then
          ...
          if gcloud compute ssh --project ${local.project.project_id} --quiet $NFS_VM --zone ${data.google_compute_zones.available_zones.names[0]} --command="sudo bash -s" < ${path.module}/scripts/app/${each.key}/import-nfs.sh --impersonate-service-account=${local.project_sa_email}; then
            echo "SSH command succeeded"
            break
          else
            echo "SSH attempt $i failed, retrying in 30 seconds..."
