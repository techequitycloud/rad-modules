...
output "cloud_sql_info" {
  value = {
    ...
    database_name = "app${local.application_database_name}${var.tenant_deployment_id}${local.random_id}"
    database_user = "app${local.application_database_user}${var.tenant_deployment_id}${local.random_id}"
  }
}
...
output "application_info" {
  value = {
    application_url  = var.configure_environment ? "https://app${local.application_name}${var.tenant_deployment_id}${local.random_id}-${local.project_number}.${local.region}.run.app" : ""
...
output "service_info" {
  value = {
    service_url  = var.configure_environment ? "https://console.cloud.google.com/run/detail/${local.region}/app${local.application_name}${var.tenant_deployment_id}${local.random_id}/metrics?orgonly=true&project=${local.project.project_id}&supportedpurview=organizationId" : ""
    cloud_secret_manager = var.configure_environment ? "https://console.cloud.google.com/security/secret-manager?inv=1&invt=AbioWw&orgonly=true&project=${local.project.project_id}&supportedpurview=organizationId" : ""
  }
}
