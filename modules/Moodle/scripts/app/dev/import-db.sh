...
check_database_exists() {
    local result=$(PGPASSWORD="lHh0E%k7@JzSi8Jo" psql -U postgres -h 10.150.0.3 -d postgres -t -c "SELECT 1 FROM pg_database WHERE datname = 'appmoodledemo16a6dev';" 2>/dev/null | tr -d ' \n')
    [[ "$result" == "1" ]]
}
...
while [ $attempt_num -le $max_retries ]; do
    ...
    echo "Terminating connections to database appmoodledemo16a6dev..."
    export PGPASSWORD='lHh0E%k7@JzSi8Jo'
    psql -U 'postgres' -h '10.150.0.3' -d postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'appmoodledemo16a6dev' AND pid <> pg_backend_pid();" 2>/dev/null

    echo "Creating/updating user role appmoodledemo16a6dev..."
    psql -U 'postgres' -h '10.150.0.3' -d postgres <<EOF
    DO \$\$
    BEGIN
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'appmoodledemo16a6dev') THEN
        CREATE ROLE appmoodledemo16a6dev WITH LOGIN PASSWORD 'FXfbd_3bf52bhICm';
        RAISE NOTICE 'Role appmoodledemo16a6dev created';
    ELSE
        ALTER ROLE appmoodledemo16a6dev WITH PASSWORD 'FXfbd_3bf52bhICm';
        RAISE NOTICE 'Role appmoodledemo16a6dev password updated';
    END IF;
    END
    \$\$;
    GRANT ALL PRIVILEGES ON DATABASE postgres TO appmoodledemo16a6dev;
    ALTER ROLE appmoodledemo16a6dev CREATEDB;
    ALTER ROLE appmoodledemo16a6dev INHERIT;
EOF

    # Check if the database exists using the function
    if check_database_exists; then
        echo "Database appmoodledemo16a6dev exists, attempting to drop it..."
        
        # Try to drop using database owner credentials first
        echo "Dropping database using owner credentials..."
        export PGPASSWORD='FXfbd_3bf52bhICm'
        drop_result=$(psql -U 'appmoodledemo16a6dev' -h '10.150.0.3' -d postgres -c "DROP DATABASE IF EXISTS appmoodledemo16a6dev;" 2>&1)
        drop_exit_code=$?
        
        # Check if drop was successful
        if [ $drop_exit_code -eq 0 ] && ! check_database_exists; then
            echo "Database appmoodledemo16a6dev dropped successfully."
            break
        else
            ...
            export PGPASSWORD='lHh0E%k7@JzSi8Jo'
            psql -U 'postgres' -h '10.150.0.3' -d postgres -c "ALTER DATABASE appmoodledemo16a6dev OWNER TO postgres;" 2>/dev/null
            ...
            drop_result2=$(psql -U 'postgres' -h '10.150.0.3' -d postgres -c "DROP DATABASE IF EXISTS appmoodledemo16a6dev;" 2>&1)
        ...
        echo "Database appmoodledemo16a6dev does not exist."
        break
    fi
...
if [ $attempt_num -gt $max_retries ]; then
    echo "Reached maximum number of retries. Failed to drop database appmoodledemo16a6dev."
    echo "Database still exists - manual intervention required."
    exit 1
fi

# Create the database - only if it doesn't exist
echo "Creating database appmoodledemo16a6dev..."
export PGPASSWORD=FXfbd_3bf52bhICm
if ! check_database_exists; then
    create_result=$(psql -U appmoodledemo16a6dev -h 10.150.0.3 -d postgres -c "CREATE DATABASE appmoodledemo16a6dev OWNER appmoodledemo16a6dev;" 2>&1)
    if [ $? -eq 0 ]; then
        ...
        echo "Creating/updating user role appmoodledemo16a6dev..."
        psql -U 'postgres' -h '10.150.0.3' -d postgres <<EOF
        GRANT ALL PRIVILEGES ON DATABASE appmoodledemo16a6dev TO appmoodledemo16a6dev;
        ALTER ROLE appmoodledemo16a6dev CREATEDB;
        ALTER ROLE appmoodledemo16a6dev INHERIT;
EOF
    else
...
if [ -f "appmoodledemo16a6dev.zip" ]; then
    ...
    export PGPASSWORD=FXfbd_3bf52bhICm
    restore_result=$(psql "host=10.150.0.3 port=5432 sslmode=disable dbname=appmoodledemo16a6dev user=appmoodledemo16a6dev" < appmoodledemo16a6dev/dump.sql 2>&1)
    ...
    sudo rm -rf appmoodledemo16a6dev/dump.sql && rm -rf appmoodledemo16a6dev.zip
fi
