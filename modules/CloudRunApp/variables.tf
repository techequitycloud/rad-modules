...
variable "module_description" {
  ...
  type        = string
  default     = "CloudRunApp: A production-ready module for deploying containerized applications on Google Cloud Run (v2) with integrated support for Cloud SQL, NFS/GCS storage, Secret Manager, VPC networking, initialization jobs, and CI/CD pipelines. Implements security best practices with least-privilege IAM, serverless auto-scaling, and comprehensive observability."
}

variable "module_documentation" {
  description = "The URL to the module documentation. {{UIMeta group=0 order=102 }}"
  type        = string
  default     = "https://docs.techequity.cloud/docs/applications/cloud-run-app"
}
...
variable "module_services" {
  ...
  type        = list(string)
  default     = ["Cloud Run", "Cloud Run Jobs", "Cloud Build", "Artifact Registry", "Cloud Storage", "GCS Fuse", "Cloud SQL", "Cloud SQL Auth Proxy", "Filestore (NFS)", "VPC Network", "Direct VPC Egress", "Secret Manager", "Cloud IAM", "Cloud Logging", "Cloud Monitoring", "Uptime Checks"]
}
...
variable "existing_project_id" {
  description = "Select an existing project on the RAD platform or enter the project ID of an external GCP project. You must grant Owner role to the RAD GCP Project agent service account when deploying into an external project. {{UIMeta group=1 order=201 }}"
  type        = string
...
variable "resource_creator_identity" {
  description = "The terraform Service Account used to create resources in the destination project. This Service Account must be assigned roles/owner IAM role in the destination project. {{UIMeta group=0 order=301 updatesafe }}"
  type        = string
...
variable "tenant_deployment_id" {
  description = "Specify a unique tenant or deployment identifier. This uniquely identifies your application deployment and is used in resource naming (1-20 lowercase alphanumeric characters and hyphens). {{UIMeta group=1 order=403 updatesafe }}"
  type        = string
  default     = "demo"
...
variable "application_version" {
  description = "Application version tag (e.g., 1.0.0, latest). {{UIMeta group=0 order=404 updatesafe }}"
  type        = string
  default     = "latest"
}

variable "resource_labels" {
  description = "Labels to apply to all resources. {{UIMeta group=0 order=405 updatesafe }}"
  type        = map(string)
  default     = {}
}
...
variable "application_display_name" {
  description = "Human-readable application name for display purposes. {{UIMeta group=0 order=803 updatesafe }}"
  type        = string
  default     = "Cloudrun Application"
}

variable "application_description" {
  description = "Brief description of your application. {{UIMeta group=0 order=804 updatesafe }}"
  type        = string
  default     = "Sample application to showcase Cloudrun features"
}
...
variable "container_image_source" {
  description = "Container image source: 'prebuilt' (use existing image from registry) or 'custom' (build from Dockerfile). {{UIMeta group=0 order=901 updatesafe }}"
  type        = string
  default     = "prebuilt"
...
variable "container_port" {
  description = "Container port to expose (1-65535). {{UIMeta group=0 order=903 updatesafe }}"
  type        = number
  default     = 8080
...
variable "container_protocol" {
  description = "Container protocol: 'http1' or 'h2c' (HTTP/2 Cleartext). {{UIMeta group=0 order=904 updatesafe }}"
  type        = string
  default     = "http1"
...
variable "enable_image_mirroring" {
  description = "Enable container image mirroring to Artifact Registry. When enabled, the image is pulled from the source and pushed to the project's Artifact Registry. Required for some applications to ensure stability and availability. {{UIMeta group=0 order=905 updatesafe }}"
  type        = bool
  default     = true
}

variable "container_resources" {
  description = "Container resource limits. Specify CPU (e.g., '1000m' for 1 CPU) and memory (e.g., '512Mi', '2Gi'). {{UIMeta group=0 order=906 updatesafe }}"
  type = object({
    cpu_limit    = string
    memory_limit = string
...
variable "container_build_config" {
  description = "Custom container build configuration. Required when container_image_source='custom'. Provide Dockerfile path or content, build context, and optional build arguments. {{UIMeta group=0 order=909 updatesafe }}"
  type = object({
    enabled            = bool
...
variable "service_annotations" {
  description = "Additional annotations for Cloud Run service (advanced). {{UIMeta group=0 order=1001 updatesafe }}"
  type        = map(string)
  default     = {}
}

variable "service_labels" {
  description = "Additional labels for Cloud Run service (advanced). {{UIMeta group=0 order=1002 updatesafe }}"
  type        = map(string)
  default     = {}
}

variable "execution_environment" {
  description = "Execution environment: gen1 or gen2. {{UIMeta group=0 order=1003 updatesafe }}"
  type        = string
  default     = "gen2"
...
variable "timeout_seconds" {
  description = "Request timeout in seconds (0-3600). Maximum time a request can take. {{UIMeta group=0 order=1004 updatesafe }}"
  type        = number
  default     = 300
...
variable "vpc_egress_setting" {
  description = "VPC egress setting: 'ALL_TRAFFIC' (route all through VPC) or 'PRIVATE_RANGES_ONLY' (only private IPs through VPC, public direct). {{UIMeta group=0 order=1006 updatesafe }}"
  type        = string
...
variable "network_tags" {
  description = "Network tags for Cloud Run service (e.g., ['nfsserver', 'web']). {{UIMeta group=0 order=1007 updatesafe }}"
  type        = list(string)
  default     = ["nfsserver"]
}

variable "secret_propagation_delay" {
  description = "Delay in seconds after creating secrets before using them (0-300). {{UIMeta group=0 order=1008 updatesafe }}"
  type        = number
  default     = 30
...
variable "gcs_volumes" {
  description = "GCS FUSE volume mounts. Specify volume name, bucket name (null for auto-created), mount path, readonly mode, and mount options. {{UIMeta group=0 order=1011 updatesafe }}"
  type = list(object({
    name        = string
...
variable "enable_cloudsql_volume" {
  description = "Enable Cloud SQL instance volume for Unix socket connections. When enabled, the Cloud SQL instance will be mounted as a volume, allowing connections via Unix socket instead of TCP/IP. {{UIMeta group=0 order=1012 updatesafe }}"
  type        = bool
  default     = true
}

variable "cloudsql_volume_mount_path" {
  description = "Mount path for Cloud SQL Unix socket (e.g., '/cloudsql'). Only used when enable_cloudsql_volume is true. {{UIMeta group=0 order=1013 updatesafe }}"
  type        = string
  default     = "/cloudsql"
}

variable "initialization_jobs" {
  description = "Cloud Run jobs for initialization tasks (e.g., database migrations, data seeding). Jobs can mount NFS/GCS volumes and access secrets. {{UIMeta group=0 order=1014 updatesafe }}"
  type = list(object({
    name              = string
...
variable "database_type" {
  description = "Database type: MYSQL, POSTGRES, SQLSERVER (or specific versions like MYSQL_8_0, POSTGRES_15). {{UIMeta group=0 order=1101 updatesafe }}"
  type        = string
  default     = "POSTGRES"
...
variable "database_password_length" {
  description = "Length of auto-generated database password (8-64 characters). {{UIMeta group=0 order=1102 updatesafe }}"
  type        = number
  default     = 16
...
variable "enable_postgres_extensions" {
  description = "Enable automatic installation of PostgreSQL extensions. Only applicable when using PostgreSQL databases. {{UIMeta group=0 order=1103 updatesafe }}"
  type        = bool
  default     = false
}

variable "postgres_extensions" {
  description = "List of PostgreSQL extensions to install (e.g., ['postgis', 'uuid-ossp', 'pg_trgm']). Common extensions: postgis, cube, earthdistance, unaccent, pg_stat_statements, uuid-ossp, pg_trgm. {{UIMeta group=0 order=1104 updatesafe }}"
  type        = list(string)
  default     = []
...
variable "enable_mysql_plugins" {
  description = "Enable automatic installation of MySQL plugins and components. Only applicable when using MySQL databases. {{UIMeta group=0 order=1105 updatesafe }}"
  type        = bool
  default     = false
}

variable "mysql_plugins" {
  description = "List of MySQL plugins to install (e.g., ['audit_log', 'validate_password']). Common plugins: validate_password, audit_log, clone, group_replication, authentication_ldap_simple. {{UIMeta group=0 order=1106 updatesafe }}"
  type        = list(string)
  default     = []
...
# ===========================
# SECTION 12: Database Backup & Import
# ===========================

variable "backup_schedule" {
  description = "Cron schedule for automated database and NFS backups (e.g., '0 2 * * *' for daily at 2am). Leave empty to disable scheduled backups. {{UIMeta group=0 order=1201 updatesafe }}"
  type        = string
  default     = "0 2 * * *"
}

variable "backup_retention_days" {
  description = "Number of days to retain automated backups. {{UIMeta group=0 order=1202 updatesafe }}"
  type        = number
  default     = 7
}

# ===========================
# SECTION 13: Backup Import
# ===========================

variable "enable_backup_import" {
  description = "Enable automatic import of database backup during deployment. Use backup_source to specify Google Drive or Google Cloud Storage. {{UIMeta group=0 order=1301 updatesafe }}"
  type        = bool
  default     = false
}

variable "backup_source" {
  description = "Backup source: 'gdrive' (Google Drive) or 'gcs' (Google Cloud Storage). GCS is recommended for production due to better security and performance. {{UIMeta group=0 order=1302 updatesafe }}"
  type        = string
  default     = "gcs"
...
variable "backup_uri" {
  description = "Backup URI. For GCS: full URI like 'gs://bucket/path/backup.sql'. For Google Drive: file ID from URL 'https://drive.google.com/file/d/FILE_ID/view'. {{UIMeta group=0 order=1303 updatesafe }}"
  type        = string
  default     = ""
}

variable "backup_format" {
  description = "Backup file format: 'sql', 'tar', 'gz', 'tgz', 'tar.gz', 'zip', or 'auto'. The 'auto' format (GCS only) auto-detects from file extension. All other formats supported by both GCS and Google Drive. {{UIMeta group=0 order=1304 updatesafe }}"
  type        = string
  default     = "sql"

  validation {
    condition     = contains(["sql", "tar", "gz", "tgz", "tar.gz", "zip", "auto"], var.backup_format)
    error_message = "Backup format must be 'sql', 'tar', 'gz', 'tgz', 'tar.gz', 'zip', or 'auto'."
  }
}

# ===========================
# SECTION 14: Cloud Build & Github
# ===========================

variable "enable_cicd_trigger" {
  description = "Enable automated Cloud Build trigger for CI/CD. When enabled, pushes to the main branch will automatically build and deploy your application. {{UIMeta group=0 order=1401 updatesafe }}"
  type        = bool
  default     = false
}

variable "cicd_trigger_config" {
  description = "Cloud Build trigger configuration for automated CI/CD pipeline. Configure branch patterns, included/ignored files, and build settings. {{UIMeta group=0 order=1402 updatesafe }}"
  type = object({
...
variable "github_repository_url" {
  description = "GitHub repository URL for automated CI/CD (e.g., 'https://github.com/username/repo'). Required when using Cloud Build triggers for automated deployments. {{UIMeta group=0 order=1403 updatesafe }}"
  type        = string
  default     = ""
}

variable "github_token" {
  description = "GitHub Personal Access Token (PAT). Required when enable_cicd_trigger is true. To generate: https://github.com/settings/tokens -> Generate new token (classic). Scopes: repo, admin:repo_hook, workflow, read:user, user:email, read:org. {{UIMeta group=0 order=1404 updatesafe }}"
  type        = string
  default     = ""
  sensitive   = true
}
...
variable "github_app_installation_id" {
  description = "GitHub App installation ID (RECOMMENDED to avoid manual approval). When set with github_token_secret_name, uses your pre-installed Google Cloud Build app and completes connection immediately without 'Configuration incomplete' state. To find: Visit https://github.com/settings/installations -> Configure Google Cloud Build -> Copy ID from URL (e.g., .../installations/12345678). {{UIMeta group=0 order=1405 updatesafe }}"
  type        = string
  default     = ""
}

# ===========================
# SECTION 15: SQL Configuration
# ===========================

variable "enable_custom_sql_scripts" {
  description = "Enable execution of custom SQL scripts from GCS during initialization. Useful for seeding data, creating additional schemas, or running custom DDL. {{UIMeta group=0 order=1501 updatesafe }}"
  type        = bool
  default     = false
}

variable "custom_sql_scripts_bucket" {
  description = "GCS bucket name containing custom SQL scripts (without gs:// prefix, e.g., 'my-bucket'). Only used when enable_custom_sql_scripts is true. {{UIMeta group=0 order=1502 updatesafe }}"
  type        = string
  default     = ""
}

variable "custom_sql_scripts_path" {
  description = "Path prefix in GCS bucket for SQL scripts (e.g., 'scripts/init/'). Scripts will be executed in alphabetical order. Name scripts with numeric prefixes for ordering (e.g., 001_schema.sql, 002_data.sql). {{UIMeta group=0 order=1503 updatesafe }}"
  type        = string
  default     = ""
}

variable "custom_sql_scripts_use_root" {
  description = "Execute custom SQL scripts as database root user instead of application user. Enable this for scripts that require elevated privileges (e.g., creating users, installing extensions). {{UIMeta group=0 order=1504 updatesafe }}"
  type        = bool
  default     = false
}

# ===========================
# SECTION 16: Observability & Alerting
# ===========================

variable "health_check_config" {
  description = "Liveness probe configuration. Checks if the application is running and restarts if unhealthy. {{UIMeta group=0 order=1601 updatesafe }}"
  type = object({
    enabled               = bool
...
variable "startup_probe_config" {
  description = "Startup probe configuration. Checks if the application has started successfully before accepting traffic. {{UIMeta group=0 order=1602 updatesafe }}"
  type = object({
    enabled               = bool
...
variable "uptime_check_config" {
  description = "Uptime check configuration. Monitors service availability and sends alerts on failures. {{UIMeta group=0 order=1603 updatesafe }}"
  type = object({
    enabled        = bool
    path           = optional(string, "/")
...
variable "alert_policies" {
  description = "Custom alert policies. Define metric-based alerts for your application (e.g., error rates, latency). {{UIMeta group=0 order=1604 updatesafe }}"
  type = list(object({
    name               = string
    metric_type        = string
    comparison         = string
