...
detect_service_url() {
    ...
    # Get Region from Cloud Run environment variable (preferred method)
    REGION="${CLOUD_RUN_REGION:-}"
    
    # Fallback: Try to get region from K_REVISION (format: service-name-xxxxx)
    if [ -z "$REGION" ] && [ -n "$K_REVISION" ]; then
        # Cloud Run sets CLOUD_RUN_REGION, but if not available, try metadata
    ...
    # Fetch service details from Cloud Run API
...
echo "Mail: ${mail__transport:-Direct}"
