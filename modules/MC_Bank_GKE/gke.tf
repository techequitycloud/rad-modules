...
locals {
  # Security Fix: Removed roles/storage.objectAdmin as roles/storage.objectViewer is already present.
  # Impact: Prevents the GKE node service account from having project-wide destructive access to GCS buckets.
  # Fix: Apply least privilege by restricting to read-only access (roles/storage.objectViewer).
