...
variable "module_services" {
  ...
  type        = list(string)
  default     = ["GCP", "GKE", "Cloud Service Mesh", "Cloud IAM", "Cloud Networking", "VPC Network", "GKE Fleet", "Workload Identity"]
}

variable "credit_cost" {
  description = "Number of platform credits consumed when this module is deployed. Credits are purchased separately; if require_credit_purchases is true, users must have sufficient credit balance before deploying. {{UIMeta group=0 order=104 }}"
  type        = number
  default     = 0
}
