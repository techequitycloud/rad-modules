...
resource "null_resource" "download_bank_of_anthos" {
  ...
  triggers = {
    ...
    # Performance Optimization: Use a static string instead of timestamp() to avoid redundant downloads on every terraform apply, improving idempotency and deploy time.
    always_run = "true"
  }
