...
resource "null_resource" "download_bank_of_anthos" {
  ...
  triggers = {
    ...
    # PERFORMANCE: Removed always_run = timestamp() to allow caching of downloaded manifests, preventing unnecessary re-downloads and speeding up terraform apply.
  }
...
resource "null_resource" "deploy_bank_of_anthos" {
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      # additional clusters. This null_resource is replaced when triggers change.
