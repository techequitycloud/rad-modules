...
resource "null_resource" "download_bank_of_anthos" {
  ...
  triggers = {
    ...
    # OPTIMIZATION: Removed always_run = timestamp() to prevent unnecessary resource replacements on every apply, allowing bash idempotency to work and speeding up subsequent runs.
  }
...
resource "null_resource" "deploy_bank_of_anthos" {
  ...
  triggers = {
    ...
    # OPTIMIZATION: Removed dynamic download_id trigger to break the replacement chain and prevent unnecessary re-deployments. Relies on depends_on for sequencing.
