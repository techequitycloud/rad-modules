...
resource "null_resource" "download_bank_of_anthos" {
  ...
  triggers = {
    ...
    # PERFORMANCE: Removed always_run = timestamp() to allow caching and prevent unnecessary downloads and redeployments on every apply.
  }
...
resource "null_resource" "deploy_bank_of_anthos" {
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      # (its download_id trigger changes when download_bank_of_anthos is
      # replaced). Recreate the namespace idempotently here
