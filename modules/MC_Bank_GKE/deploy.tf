...
resource "null_resource" "download_bank_of_anthos" {
  ...
  triggers = {
    ...
    # PERFORMANCE: Removed always_run = timestamp() to prevent unnecessary re-downloads and re-deployments on every apply, speeding up execution.
  }
...
resource "null_resource" "deploy_bank_of_anthos" {
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      # additional clusters. This null_resource is also replaced when the
      # download_id trigger changes (when download_bank_of_anthos triggers).
