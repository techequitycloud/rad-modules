...
variable "module_description" {
  ...
  type        = string
  default     = <<-EOT
**Purpose:** This module deploys the Odoo Enterprise Resource Planning (ERP) system on Google Cloud Run. This provides a serverless environment for the ERP system.
...
variable "trusted_users" {
  description = "List of trusted users with limited Google Cloud project admin privileges. (e.g. `username@abc.com`). {{UIMeta group=0 order=104 updatesafe }}"
  type        = set(string)
  default     = []
}
...
variable "application_release" {
  description = "Enter application release. This value can be updated from the docker files at https://hub.docker.com/_/odoo {{UIMeta group=0 order=505 updatesafe}}"
  type        = string
  default     = "20251008"
}
