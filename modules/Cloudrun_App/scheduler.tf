...
resource "google_cloud_scheduler_job" "dev_backup" {
  count            = var.configure_development_environment && var.configure_backups ? 1 : 0
  paused           = false 
...
resource "google_cloud_scheduler_job" "qa_backup" {
  count            = var.configure_nonproduction_environment && var.configure_backups ? 1 : 0
  paused           = false 
...
resource "google_cloud_scheduler_job" "prod_backup" {
  count            = var.configure_production_environment && var.configure_backups ? 1 : 0
  paused           = false 
