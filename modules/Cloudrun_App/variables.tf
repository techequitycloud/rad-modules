...
variable "configure_backups" {
  description = "Select to schedule application database backups. Configures a Cloud Scheduler trigger to execute a Cloud Run backup job. {{UIMeta group=5 order=808 updatesafe }}"
  type        = bool
  default     = false
}

variable "application_backup_schedule" {
  description = "Enter the application backup schedule in cron format. This is used to configure the Cloud Scheduler cron job. {{UIMeta group=5 order=809 updatesafe }}"
  type        = string
  default     = "0 0 * * *"
}

variable "application_backup_fileid" {
  description = "Enter application backup file ID. When enabled, terraform attempts to download the file from Google Drive, and if found, imports the backup files during deployment. {{UIMeta group=5 order=810 updatesafe}}"
  type        = string
  default     = ""
}
