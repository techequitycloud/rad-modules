...
variable "module_description" {
  ...
  type        = string
  default     = "This module can be used to deploy Cyclos"
}
...
variable "application_module" {
  description = "Select a pre-configured application module for automatic configuration. Leave empty or null for manual configuration. When using a module, container image, port, database type, resource limits, and other settings are automatically configured. You can still override any module value by explicitly setting the corresponding variable. {{UIMeta group=0 order=299 OPTIONS=cyclos,wordpress,moodle,cyclos,django,openemr,n8n,payload,ghost,strapi,wikijs,directus updatesafe }}"
  type        = string
  default     = "cyclos"

  validation {
    condition = var.application_module == null || var.application_module == "" || contains([
      "cyclos", "wordpress", "moodle", "cyclos", "django", "openemr", "n8n", "ghost", "wikijs", "directus", "strapi", "medusa", "sample"
    ], var.application_module)
    error_message = "Application module must be one of: cyclos, wordpress, moodle, cyclos, django, openemr, n8n, ghost, wikijs, directus, strapi, medusa, sample, or leave empty for manual configuration."
  }
}
...
variable "application_display_name" {
  ...
  type        = string
  default     = "Cyclos Community Edition"
}
...
variable "initialization_jobs" {
  ...
  default = [
      ...
      {
        name            = "nfs-init"
        description     = "Initialize NFS directories for Cyclos"
        image           = "alpine:3.19"
        command         = ["/bin/sh", "-c"]
        args            = [
          <<-EOT
      ...
      {
        name            = "cyclos-config"
        description     = "Generate Cyclos configuration file"
        image           = "alpine:3.19"
        command         = ["/bin/sh", "-c"]
        args            = [
          <<-EOT
            set -e
            echo "=========================================="
            echo "Generating Cyclos Configuration File"
            echo "=========================================="

            CONFIG_FILE="/mnt/cyclos.conf"
...
proxy_mode = True
logfile = /var/log/cyclos/cyclos.log
log_level = info
log_handler = :INFO
log_db = False
...
EOF
            ...
            # Set proper permissions (Cyclos runs as UID 101)
      ...
      # Job 4: Cyclos Initialization
      {
        name            = "cyclos-init"
        description     = "Initialize Cyclos database"
        image           = null
        command         = ["/bin/bash", "-c"]
        args            = [
          <<-EOT
            set -e
            echo "=========================================="
            echo "Cyclos Database Initialization"
            echo "=========================================="
            ...
            if [ ! -f /mnt/cyclos.conf ]; then
              echo "ERROR: /mnt/cyclos.conf not found"
              ls -la /mnt/
              exit 1
            fi
            echo ""
            ...
            if ! cat /mnt/cyclos.conf > /dev/null 2>&1; then
              echo "ERROR: Cannot read /mnt/cyclos.conf"
              ls -la /mnt/cyclos.conf
              exit 1
            fi
            ...
            echo "=========================================="
            echo "Starting Cyclos initialization..."
            echo "=========================================="
            cyclos -c /mnt/cyclos.conf -i base --stop-after-init --log-level=info
