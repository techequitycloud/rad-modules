...
variable "module_description" {
  ...
  type        = string
  default     = "Cyclos: Deploy Cyclos Banking System on Google Cloud Run with managed PostgreSQL, secure networking, automated initialization, and serverless scaling. Ideal for microfinance institutions, local banks, and complementary currency systems. Features zero-maintenance infrastructure, automated PostgreSQL extensions (PostGIS, pg_trgm), secure Secret Manager integration, and customizable branding."
}

variable "module_documentation" {
  description = "The URL to the module documentation. {{UIMeta group=0 order=102 }}"
  type        = string
  default     = "https://docs.techequity.cloud/docs/applications/cyclos"
}
...
variable "module_services" {
  ...
  type        = list(string)
  default     = ["Cloud Run", "Cloud Run Jobs", "Cloud Build", "Artifact Registry", "Cloud Storage", "Cloud SQL (PostgreSQL 15)", "Filestore (Optional NFS)", "VPC Network", "Serverless VPC Access", "Secret Manager", "Cloud IAM", "Cloud Logging", "Cloud Monitoring"]
}
...
variable "application_version" {
  description = "Application version tag (e.g., 4.16.17, latest). {{UIMeta group=0 order=4002 updatesafe }}"
  type        = string
  default     = "4.16.17"
}
