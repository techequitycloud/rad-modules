...
variable "agent_service_account" {
  description = "Grant your GCP_Project agent service account IAM Owner role on the target deployment project, if using an external Google Cloud project. Enter the GCP_Project agent service account e.g. rad-agent@GCP_PROJECT_ID.iam.gserviceaccount.com {{UIMeta group=1 order=200 updatesafe }}"
  type        = string
  default     = null
}
