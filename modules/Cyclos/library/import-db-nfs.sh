...
for i in $(gcloud --project $PROJECT_ID compute os-login ssh-keys list $SA_ARG 2>/dev/null | grep -v FINGERPRINT); do
    echo $i
    gcloud --project $PROJECT_ID compute os-login ssh-keys remove --key $i $SA_ARG 2>/dev/null
done # Avoid error "Login profile size exceeds 32 KiB"
