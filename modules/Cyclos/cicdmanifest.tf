...
resource "local_file" "secondary_dev_base_deploy" {
  ...
  content  = templatefile("${path.module}/scripts/ci/base/deploy.yaml.tpl", {
    ...
    APP_NAME          = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}"
    DATABASE_INSTANCE = local.db_instance_name
    NETWORK_NAME      = var.network_name  # No need for interpolation here
...
resource "local_file" "dev_cyclos_properties" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
  filename = "${path.module}/scripts/ci/cyclos.properties"
...
# Resource for creating a local configuration file directly from an existing template file.
resource "local_file" "dev_hazelcast_xml" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
  filename = "${path.module}/scripts/ci/hazelcast.xml"
...
# Resource for creating a local configuration file directly from an existing template file.
resource "local_file" "qa_hazelcast_xml" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  filename = "${path.module}/scripts/ci/hazelcast.xml"
  content  = file("${path.module}/scripts/ci/hazelcast.xml.tpl")
...
# Resource for creating a local configuration file directly from an existing template file.
resource "local_file" "prod_hazelcast_xml" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_production_environment ? 1 : 0
  filename = "${path.module}/scripts/ci/hazelcast.xml"
  content  = file("${path.module}/scripts/ci/hazelcast.xml.tpl")
...
# Resource definition for a GitHub repository file for the configuration
resource "github_repository_file" "dev_hazelcast_xml" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "dev"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "hazelcast.xml"
  content             = local_file.dev_hazelcast_xml[count.index].content

  depends_on      = [
    local_file.dev_hazelcast_xml,
    null_resource.init_git_repo
  ]
}
...
# Resource definition for a GitHub repository file for the configuration
resource "github_repository_file" "qa_hazelcast_xml" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "qa"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "hazelcast.xml"
  content             = local_file.qa_hazelcast_xml[count.index].content

  depends_on      = [
    local_file.qa_hazelcast_xml,
    null_resource.init_git_repo
  ]
}
...
# Resource definition for a GitHub repository file for the configuration
resource "github_repository_file" "prod_hazelcast_xml" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_production_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "prod"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "hazelcast.xml"
  content             = local_file.prod_hazelcast_xml[count.index].content

  depends_on      = [
    local_file.prod_hazelcast_xml,
    null_resource.init_git_repo
  ]
}
