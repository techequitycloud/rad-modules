...
resource "google_monitoring_service" "cloud_run" {
  count =  (var.configure_monitoring && var.configure_environment) ? 1 : 0
  service_id   = "app${var.application_name}-monitoring-service-${var.tenant_deployment_id}-${local.random_id}-${local.region}"
