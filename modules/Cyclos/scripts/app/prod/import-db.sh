...
check_database_exists() {
    local result=$(PGPASSWORD="g%L9hkVajm3p@ApK" psql -U postgres -h 172.21.0.5 -d postgres -t -c "SELECT 1 FROM pg_database WHERE datname = 'appcyclosdemo5a96prod';" 2>/dev/null | tr -d ' \n')
    [[ "$result" == "1" ]]
}
...
while [ $attempt_num -le $max_retries ]; do
    ...
    echo "Terminating connections to database appcyclosdemo5a96prod..."
    export PGPASSWORD='g%L9hkVajm3p@ApK'
    psql -U 'postgres' -h '172.21.0.5' -d postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'appcyclosdemo5a96prod' AND pid <> pg_backend_pid();" 2>/dev/null

    echo "Creating/updating user role appcyclosdemo5a96prod..."
    psql -U 'postgres' -h '172.21.0.5' -d postgres <<EOF
    DO \$\$
    BEGIN
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'appcyclosdemo5a96prod') THEN
        CREATE ROLE appcyclosdemo5a96prod WITH LOGIN PASSWORD 'NoM7qXY5dKjbybJj';
        RAISE NOTICE 'Role appcyclosdemo5a96prod created';
    ELSE
        ALTER ROLE appcyclosdemo5a96prod WITH PASSWORD 'NoM7qXY5dKjbybJj';
        RAISE NOTICE 'Role appcyclosdemo5a96prod password updated';
    END IF;
    END
    \$\$;
    GRANT ALL PRIVILEGES ON DATABASE postgres TO appcyclosdemo5a96prod;
    ALTER ROLE appcyclosdemo5a96prod CREATEDB;
    ALTER ROLE appcyclosdemo5a96prod INHERIT;
EOF

    echo "Creating extensions in appcyclosdemo5a96prod..."
    psql -U 'postgres' -h '172.21.0.5' -d "appcyclosdemo5a96prod" <<EOF
...
EOF
    ...
    if check_database_exists; then
        echo "Database appcyclosdemo5a96prod exists, attempting to drop it..."
        ...
        drop_result=$(psql -U 'appcyclosdemo5a96prod' -h '172.21.0.5' -d postgres -c "DROP DATABASE IF EXISTS appcyclosdemo5a96prod;" 2>&1)
        drop_exit_code=$?
        
        # Check if drop was successful
        if [ $drop_exit_code -eq 0 ] && ! check_database_exists; then
            echo "Database appcyclosdemo5a96prod dropped successfully."
            break
        else
            ...
            export PGPASSWORD='g%L9hkVajm3p@ApK'
            psql -U 'postgres' -h '172.21.0.5' -d postgres -c "ALTER DATABASE appcyclosdemo5a96prod OWNER TO postgres;" 2>/dev/null
            ...
            drop_result2=$(psql -U 'postgres' -h '172.21.0.5' -d postgres -c "DROP DATABASE IF EXISTS appcyclosdemo5a96prod;" 2>&1)
        ...
        echo "Database appcyclosdemo5a96prod does not exist."
        break
    fi
...
if [ $attempt_num -gt $max_retries ]; then
    echo "Reached maximum number of retries. Failed to drop database appcyclosdemo5a96prod."
    echo "Database still exists - manual intervention required."
    exit 1
fi

# Create the database - only if it doesn't exist
echo "Creating database appcyclosdemo5a96prod..."
export PGPASSWORD=NoM7qXY5dKjbybJj
if ! check_database_exists; then
    create_result=$(psql -U appcyclosdemo5a96prod -h 172.21.0.5 -d postgres -c "CREATE DATABASE appcyclosdemo5a96prod OWNER appcyclosdemo5a96prod;" 2>&1)
    if [ $? -eq 0 ]; then
        ...
        echo "Creating/updating user role appcyclosdemo5a96prod..."
        psql -U 'postgres' -h '172.21.0.5' -d postgres <<EOF
        GRANT ALL PRIVILEGES ON DATABASE appcyclosdemo5a96prod TO appcyclosdemo5a96prod;
        ALTER ROLE appcyclosdemo5a96prod CREATEDB;
        ALTER ROLE appcyclosdemo5a96prod INHERIT;
EOF
...
if [ -f "appcyclosdemo5a96prod.zip" ]; then
    ...
    export PGPASSWORD=NoM7qXY5dKjbybJj
    restore_result=$(psql "host=172.21.0.5 port=5432 sslmode=disable dbname=appcyclosdemo5a96prod user=appcyclosdemo5a96prod" < appcyclosdemo5a96prod/dump.sql 2>&1)
    ...
    sudo rm -rf appcyclosdemo5a96prod/dump.sql && rm -rf appcyclosdemo5a96prod.zip
fi
