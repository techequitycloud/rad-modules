...
resource "google_vmwareengine_network_peering" "vpc_peering" {
  ...
  name                  = local.network_peering_name
  vmware_engine_network = google_vmwareengine_network.vmware_engine_network.id
  peer_network          = "projects/${local.project.project_id}/global/networks/${local.peer_vpc_name}"
  ...
  description = "Peering between ${local.ven_name} and VPC ${local.peer_vpc_name}"
