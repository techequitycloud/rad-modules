...
resource "google_cloud_run_v2_service" "qdrant_service" {
  count               = var.configure_environment && var.enable_ai_components && var.enable_qdrant ? 1 : 0
...
resource "google_cloud_run_service_iam_binding" "qdrant_service" {
  count = var.configure_environment && var.enable_ai_components && var.enable_qdrant ? 1 : 0

  project  = local.project.project_id
  location = local.region
  service  = google_cloud_run_v2_service.qdrant_service[0].name
  role     = "roles/run.invoker"
  members = [
    "allUsers" # Or restrict to internal if possible, but n8n might need public URL if not strictly internal
  ]
}
...
resource "google_cloud_run_v2_service" "ollama_service" {
  count               = var.configure_environment && var.enable_ai_components && var.enable_ollama ? 1 : 0
  project             = local.project.project_id
...
resource "google_cloud_run_service_iam_binding" "ollama_service" {
  count = var.configure_environment && var.enable_ai_components && var.enable_ollama ? 1 : 0

  project  = local.project.project_id
  location = local.region
  service  = google_cloud_run_v2_service.ollama_service[0].name
  role     = "roles/run.invoker"
  members = [
    "allUsers"
  ]
