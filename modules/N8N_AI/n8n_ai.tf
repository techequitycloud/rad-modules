locals {
    ...
    initialization_jobs = [
      {
        ...
        image           = "alpine:3.19"
        script_path       = "${path.module}/scripts/n8n_ai/db-init.sh"
        mount_nfs         = false
        mount_gcs_volumes = []
  ...
  n8n_env_vars = {
    ...
    QDRANT_URL                   = var.enable_ai_components && var.enable_qdrant ? google_cloud_run_v2_service.qdrant_service[0].uri : ""
    OLLAMA_HOST                  = var.enable_ai_components && var.enable_ollama ? google_cloud_run_v2_service.ollama_service[0].uri : ""
  }
