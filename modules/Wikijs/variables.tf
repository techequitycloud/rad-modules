...
variable "module_description" {
  ...
  type        = string
  default     = "Wiki.js: Deploy powerful open-source wiki software on Google Cloud Run for beautiful, intuitive knowledge management. Modern responsive UI with Git-backed content sync for version control. Includes Wiki.js 2 (requarks/wiki) on Cloud Run Gen2, PostgreSQL 15 with pg_trgm extension for native full-text search, GCS FUSE-mounted bucket (wikijs-storage) at /wiki-storage for persistent asset storage, optional NFS mount, and auto-detected Redis caching. Features automated self-healing db-init job, Unix socket/Internal IP connection, auto-scaling (0-3 instances), liveness/startup probes, configurable ingress, Secret Manager integration, wrapper pattern with CloudRunApp, CI/CD support, and least privilege IAM. Default 1 vCPU / 2Gi Memory on port 3000. Supports modular architecture with various authentication providers, editors (Markdown, WYSIWYG), and storage backends. Ideal for team documentation, Git-backed version control, powerful instant search, extensible authentication, and scalable persistent storage."
}
...
variable "module_services" {
  ...
  type        = list(string)
  default     = ["Cloud Run Gen2", "Cloud Run Jobs", "Cloud Build", "Artifact Registry", "Cloud Storage", "GCS FUSE", "Cloud SQL", "Cloud SQL Auth Proxy", "PostgreSQL 15", "pg_trgm Extension", "Unix Socket", "NFS", "Redis", "VPC Network", "Internal IP", "Serverless VPC Access", "Secret Manager", "Cloud IAM", "Service Accounts", "Cloud Logging", "Cloud Monitoring", "Health Checks", "Load Balancer"]
}
...
variable "application_version" {
  description = "Application version tag (e.g., 2.5.309, latest). {{UIMeta group=0 order=4002 updatesafe }}"
  type        = string
  default     = "2.5.311"
}
...
variable "environment_variables" {
  description = "Static environment variables for the application as key-value pairs (e.g., {APP_ENV='production', LOG_LEVEL='info'}). {{UIMeta group=0 order=7002 updatesafe }}"
  type        = map(string)
  default     = {
    DB_TYPE         = "postgres"
    DB_PORT         = "5432"
