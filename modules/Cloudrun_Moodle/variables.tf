...
variable "module_description" {
  ...
  type        = string
  default     = <<-EOT
**Purpose:** This module deploys Moodle, a popular open-source Learning Management System (LMS), on Google Cloud Run. This provides a serverless environment for the LMS.

**What it does:**
- Deploys the Moodle application on Cloud Run.
...
variable "trusted_users" {
  description = "List of trusted users with limited Google Cloud project admin privileges. (e.g. `username@abc.com`). {{UIMeta group=1 order=104 updatesafe }}"
  type        = set(string)
  default     = []
}
