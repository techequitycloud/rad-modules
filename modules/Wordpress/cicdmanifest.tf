...
# Resource for creating the file in the GitHub repository on the 'main' branch
resource "github_repository_file" "dockerfile_entrypoint" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "main"
...
resource "github_repository_file" "wp_config_docker" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "main"
  ...
  content             = data.local_file.wp_config_docker[count.index].content

  depends_on      = [
    data.local_file.wp_config_docker,
    null_resource.init_git_repo
  ]
}
...
resource "github_repository_file" "cicd_cloudbuild" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "main"
  commit_message      = "Add file to repo"
  ...
  content             = local_file.cicd_cloudbuild[count.index].content

  depends_on      = [
    local_file.cicd_cloudbuild,
    null_resource.init_git_repo
  ]
}

# Resource definition for a GitHub repository file for the Dockerfile
resource "github_repository_file" "cicd_dockerfile" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "main"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "Dockerfile"
  content             = local_file.cicd_dockerfile[count.index].content

  depends_on      = [
    local_file.cicd_dockerfile,
    null_resource.init_git_repo
  ]
}

# Resource definition for a GitHub repository file for the Skaffold configuration
resource "github_repository_file" "cicd_skaffold" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "main"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "skaffold.yaml"
  content             = local_file.cicd_skaffold[count.index].content

  depends_on      = [
    local_file.cicd_skaffold,
    null_resource.init_git_repo
  ]
}
