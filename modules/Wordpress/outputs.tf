...
output "cloud_sql_info" {
  value = {
    ...
    development_database_name =  "app${var.application_database_name}${var.tenant_deployment_id}${local.random_id}"
    development_database_user = "app${var.application_database_name}${var.tenant_deployment_id}${local.random_id}"
  }
}
...
output "application_lb_url_info" {
  value = {
    application_lb_url  = length(local.regions) >= 2 && (length(google_compute_global_address.default) > 0) ? "https://app${var.application_name}${var.tenant_deployment_id}${local.random_id}.${google_compute_global_address.default[0].address}.nip.io" : ""
  }
}
