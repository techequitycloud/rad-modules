...
while [ $attempt -lt $max_attempts ]; do
  ...
  if gcloud run services describe "appwpdemof367dev" --project="qwiklabs-gcp-02-9f7942837ab3" --region="$APP_REGION_1" 2>/dev/null; then
    echo "Cloud Run service still exists in region $APP_REGION_1. Attempting to delete..."
    
    if gcloud run services delete "appwpdemof367dev" --project="qwiklabs-gcp-02-9f7942837ab3" --region="$APP_REGION_1" --quiet; then
  ...
  if gcloud run services describe "appwpdemof367dev" --project="qwiklabs-gcp-02-9f7942837ab3" --region="$APP_REGION_2" 2>/dev/null; then
    echo "Cloud Run service still exists in region $APP_REGION_2. Attempting to delete..."
    
    if gcloud run services delete "appwpdemof367dev" --project="qwiklabs-gcp-02-9f7942837ab3" --region="$APP_REGION_2" --quiet; then
...
check_database_exists() {
    local result=$(mysql --defaults-file=~/.my.cnf -u root -h "10.230.0.3" -e "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = 'appwpdemof367dev';" 2>/dev/null | grep -c "appwpdemof367dev")
    [[ "$result" == "1" ]]
}

# Create/update user with NO global privileges initially
echo "Creating/updating user appwpdemof367dev..."
mysql --defaults-file=~/.my.cnf -u root -h "10.230.0.3" <<EOF
CREATE USER IF NOT EXISTS 'appwpdemof367dev'@'%' IDENTIFIED BY 'os0%z1Q@FDVAliL0';
ALTER USER 'appwpdemof367dev'@'%' IDENTIFIED BY 'os0%z1Q@FDVAliL0';
# -- ✅ Remove any existing global privileges
# REVOKE ALL PRIVILEGES ON *.* FROM 'appwpdemof367dev'@'%';
FLUSH PRIVILEGES;
EOF
...
while [ $attempt_num -le $max_retries ]; do
    ...
    echo "Terminating connections to database appwpdemof367dev..."
    # Fixed connection termination logic
    kill_statements=$(mysql --defaults-file=~/.my.cnf -u root -h "10.230.0.3" -e "SELECT CONCAT('KILL ', id, ';') FROM INFORMATION_SCHEMA.PROCESSLIST WHERE db = 'appwpdemof367dev' AND id != CONNECTION_ID();" -N -s 2>/dev/null)
    if [ -n "$kill_statements" ]; then
        echo "$kill_statements" | mysql --defaults-file=~/.my.cnf -u root -h "10.230.0.3" 2>/dev/null
    fi

    if check_database_exists; then
        echo "Database appwpdemof367dev exists, attempting to drop it..."
        
        # Try to drop as root user (DB_USER may not have drop privileges yet)
        echo "Dropping database as root user..."
        drop_result=$(mysql --defaults-file=~/.my.cnf -u root -h "10.230.0.3" -e "DROP DATABASE IF EXISTS \`appwpdemof367dev\`;" 2>&1)
        
        if [ $? -eq 0 ] && ! check_database_exists; then
            echo "Database appwpdemof367dev dropped successfully."
            break
        else
            echo "Failed to drop database. Error: $drop_result"
        fi
    else
        echo "Database appwpdemof367dev does not exist."
        break
    fi
...
if [ $attempt_num -gt $max_retries ]; then
    echo "Reached maximum number of retries. Failed to drop database appwpdemof367dev."
    echo "Database still exists - manual intervention required."
    exit 1
fi

# Create the database as root
echo "Creating database appwpdemof367dev..."
if ! check_database_exists; then
    create_result=$(mysql --defaults-file=~/.my.cnf -u root -h "10.230.0.3" -e "CREATE DATABASE \`appwpdemof367dev\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" 2>&1)
    if [ $? -eq 0 ]; then
...
echo "Granting privileges to appwpdemof367dev ONLY on database appwpdemof367dev..."
mysql --defaults-file=~/.my.cnf -u root -h "10.230.0.3" <<EOF
-- Grant all privileges on the specific database only
GRANT ALL PRIVILEGES ON \`appwpdemof367dev\`.* TO 'appwpdemof367dev'@'%';
-- Allow user to grant privileges on this database to others (if needed)
GRANT GRANT OPTION ON \`appwpdemof367dev\`.* TO 'appwpdemof367dev'@'%';
FLUSH PRIVILEGES;
EOF

if [ $? -eq 0 ]; then
    echo "Privileges granted successfully on database appwpdemof367dev only."
else
    echo "Failed to grant privileges on database appwpdemof367dev."
    exit 1
fi

# Verify user has access only to the intended database
echo "Verifying user privileges..."
mysql --defaults-file=~/.my.cnf -u root -h "10.230.0.3" -e "SHOW GRANTS FOR 'appwpdemof367dev'@'%';"
...
if [ -n "" ] ; then
    ...
    if sudo /root/.local/bin/gdown "" -O "appwpdemof367dev.zip"; then
        echo "Backup file downloaded successfully"
        if [ -f "appwpdemof367dev.zip" ]; then
            echo "Backup file exists and is $(du -h "appwpdemof367dev.zip" | cut -f1) in size"
        fi
    else
...
if [ -f "appwpdemof367dev.zip" ]; then
    ...
    sudo mkdir -p "appwpdemof367dev" && sudo rm -rf "appwpdemof367dev"/* && sudo unzip "appwpdemof367dev.zip" -d "appwpdemof367dev"
    ...
    cat > ~/.my.cnf << 'EOF'
    [client]
    user=appwpdemof367dev
    password=os0%z1Q@FDVAliL0
    host=10.230.0.3
EOF
    chmod 600 ~/.my.cnf

    restore_result=$(mysql --defaults-file=~/.my.cnf -u "appwpdemof367dev" -h "10.230.0.3" "appwpdemof367dev" < "appwpdemof367dev/dump.sql" 2>&1)
    ...
    sudo rm -rf "appwpdemof367dev" && rm -rf "appwpdemof367dev.zip"
fi
