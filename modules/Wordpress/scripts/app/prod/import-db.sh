...
while [ $attempt -lt $max_attempts ]; do
  ...
  if gcloud run services describe "appwpdemof367prod" --project="qwiklabs-gcp-02-9f7942837ab3" --region="$APP_REGION_1" 2>/dev/null; then
    echo "Cloud Run service still exists in region $APP_REGION_1. Attempting to delete..."
    
    if gcloud run services delete "appwpdemof367prod" --project="qwiklabs-gcp-02-9f7942837ab3" --region="$APP_REGION_1" --quiet; then
  ...
  if gcloud run services describe "appwpdemof367prod" --project="qwiklabs-gcp-02-9f7942837ab3" --region="$APP_REGION_2" 2>/dev/null; then
    echo "Cloud Run service still exists in region $APP_REGION_2. Attempting to delete..."
    
    if gcloud run services delete "appwpdemof367prod" --project="qwiklabs-gcp-02-9f7942837ab3" --region="$APP_REGION_2" --quiet; then
...
check_database_exists() {
    local result=$(mysql --defaults-file=~/.my.cnf -u root -h "10.230.0.3" -e "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = 'appwpdemof367prod';" 2>/dev/null | grep -c "appwpdemof367prod")
    [[ "$result" == "1" ]]
}

# Create/update user with NO global privileges initially
echo "Creating/updating user appwpdemof367prod..."
mysql --defaults-file=~/.my.cnf -u root -h "10.230.0.3" <<EOF
CREATE USER IF NOT EXISTS 'appwpdemof367prod'@'%' IDENTIFIED BY 'os0%z1Q@FDVAliL0';
ALTER USER 'appwpdemof367prod'@'%' IDENTIFIED BY 'os0%z1Q@FDVAliL0';
# -- ✅ Remove any existing global privileges
# REVOKE ALL PRIVILEGES ON *.* FROM 'appwpdemof367prod'@'%';
FLUSH PRIVILEGES;
EOF
...
while [ $attempt_num -le $max_retries ]; do
    ...
    echo "Terminating connections to database appwpdemof367prod..."
    # Fixed connection termination logic
    kill_statements=$(mysql --defaults-file=~/.my.cnf -u root -h "10.230.0.3" -e "SELECT CONCAT('KILL ', id, ';') FROM INFORMATION_SCHEMA.PROCESSLIST WHERE db = 'appwpdemof367prod' AND id != CONNECTION_ID();" -N -s 2>/dev/null)
    if [ -n "$kill_statements" ]; then
        echo "$kill_statements" | mysql --defaults-file=~/.my.cnf -u root -h "10.230.0.3" 2>/dev/null
    fi

    if check_database_exists; then
        echo "Database appwpdemof367prod exists, attempting to drop it..."
        
        # Try to drop as root user (DB_USER may not have drop privileges yet)
        echo "Dropping database as root user..."
        drop_result=$(mysql --defaults-file=~/.my.cnf -u root -h "10.230.0.3" -e "DROP DATABASE IF EXISTS \`appwpdemof367prod\`;" 2>&1)
        
        if [ $? -eq 0 ] && ! check_database_exists; then
            echo "Database appwpdemof367prod dropped successfully."
            break
        else
            echo "Failed to drop database. Error: $drop_result"
        fi
    else
        echo "Database appwpdemof367prod does not exist."
        break
    fi
...
if [ $attempt_num -gt $max_retries ]; then
    echo "Reached maximum number of retries. Failed to drop database appwpdemof367prod."
    echo "Database still exists - manual intervention required."
    exit 1
fi

# Create the database as root
echo "Creating database appwpdemof367prod..."
if ! check_database_exists; then
    create_result=$(mysql --defaults-file=~/.my.cnf -u root -h "10.230.0.3" -e "CREATE DATABASE \`appwpdemof367prod\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" 2>&1)
    if [ $? -eq 0 ]; then
...
echo "Granting privileges to appwpdemof367prod ONLY on database appwpdemof367prod..."
mysql --defaults-file=~/.my.cnf -u root -h "10.230.0.3" <<EOF
-- Grant all privileges on the specific database only
GRANT ALL PRIVILEGES ON \`appwpdemof367prod\`.* TO 'appwpdemof367prod'@'%';
-- Allow user to grant privileges on this database to others (if needed)
GRANT GRANT OPTION ON \`appwpdemof367prod\`.* TO 'appwpdemof367prod'@'%';
FLUSH PRIVILEGES;
EOF

if [ $? -eq 0 ]; then
    echo "Privileges granted successfully on database appwpdemof367prod only."
else
    echo "Failed to grant privileges on database appwpdemof367prod."
    exit 1
fi

# Verify user has access only to the intended database
echo "Verifying user privileges..."
mysql --defaults-file=~/.my.cnf -u root -h "10.230.0.3" -e "SHOW GRANTS FOR 'appwpdemof367prod'@'%';"
...
if [ -n "" ] ; then
    ...
    if sudo /root/.local/bin/gdown "" -O "appwpdemof367prod.zip"; then
        echo "Backup file downloaded successfully"
        if [ -f "appwpdemof367prod.zip" ]; then
            echo "Backup file exists and is $(du -h "appwpdemof367prod.zip" | cut -f1) in size"
        fi
    else
...
if [ -f "appwpdemof367prod.zip" ]; then
    ...
    sudo mkdir -p "appwpdemof367prod" && sudo rm -rf "appwpdemof367prod"/* && sudo unzip "appwpdemof367prod.zip" -d "appwpdemof367prod"
    ...
    cat > ~/.my.cnf << 'EOF'
    [client]
    user=appwpdemof367prod
    password=os0%z1Q@FDVAliL0
    host=10.230.0.3
EOF
    chmod 600 ~/.my.cnf

    restore_result=$(mysql --defaults-file=~/.my.cnf -u "appwpdemof367prod" -h "10.230.0.3" "appwpdemof367prod" < "appwpdemof367prod/dump.sql" 2>&1)
    ...
    sudo rm -rf "appwpdemof367prod" && rm -rf "appwpdemof367prod.zip"
fi
