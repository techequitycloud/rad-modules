...
variable "module_description" {
  ...
  type        = string
  default     = "WordPress: Deploy world's most popular CMS on Google Cloud Run serverless platform for fast, secure, auto-scalable corporate websites, blogs, and marketing pages. Automatically scales for viral traffic and scales down to save costs with optimized performance for SEO. Includes WordPress on Cloud Run Gen2 with PHP 8.4 Apache, MySQL 8.0 via Cloud SQL (Unix socket), GCS FUSE-mounted bucket (wp-uploads) at /var/www/html/wp-content for persistent shared media uploads, and stateless plugin mechanism. Features automated db-init job for schema creation, custom Docker build with extensions (gd, mysqli, imagick, bcmath, intl, zip), Apache mod_remoteip, docker-entrypoint.sh with Cloud SQL wait and wp-config.php generation with unique salts, Direct VPC Egress, Secret Manager integration, configurable scaling (1-3 instances), configurable ingress, 2Gi/128M memory defaults, 300s timeout, Cloud Logging, least-privilege IAM, and Cloud SQL Auth Proxy. Removes server attack surface with no OS patching. Ideal for corporate sites, blogs, marketing pages, viral traffic, SEO performance, and unlimited media storage."
}

variable "module_documentation" {
  description = "The URL to the module documentation. {{UIMeta group=0 order=102 }}"
  type        = string
  default     = "https://docs.techequity.cloud/docs/applications/wordpress"
}
...
variable "module_services" {
  ...
  type        = list(string)
  default     = ["Cloud Run Gen2", "Cloud Run Jobs", "Cloud Build", "Artifact Registry", "Cloud Storage", "GCS FUSE", "Cloud SQL", "Cloud SQL Auth Proxy", "MySQL 8.0", "Unix Socket", "VPC Network", "Direct VPC Egress", "Private IP", "Serverless VPC Access", "Secret Manager", "Cloud IAM", "Service Accounts", "Cloud Logging", "Cloud Monitoring", "Load Balancer", "Apache 2.4", "PHP 8.4"]
}

variable "credit_cost" {
  description = "Specify the module cost. {{UIMeta group=0 order=5 }}"
  type        = number
  default     = 100
}

variable "require_credit_purchases" {
  description = "Set to true to require credit purchases to deploy this module. {{UIMeta group=0 order=6 }}"
  type        = bool
  default     = false
}

variable "enable_purge" {
  description = "Set to true to enable the ability to purge this module. {{UIMeta group=0 order=7 }}"
  type        = bool
  default     = true
}
...
# ===========================
# SECTION 1: Service Accounts
# ===========================

variable "resource_creator_identity" {
  description = "The Service Account used by terraform to create resources in the destination project. Assign time limited conditional Basic Owner IAM role in the destination project. {{UIMeta group=0 order=101 }}"
  type        = string
  default     = "rad-module-creator@tec-rad-ui-2b65.iam.gserviceaccount.com"
}

variable "agent_service_account" {
  description = "If deploying into an existing GCP project outside of the RAD platform, enter a RAD GCP project agent service account (e.g., rad-agent@gcp-project.iam.gserviceaccount.com) and grant this service account IAM Owner role in the target Google Cloud project. Leave this field blank if deploying into a target project on the RAD platform. {{UIMeta group=0 order=102 updatesafe }}"
  type        = string
  default     = null
}

variable "cloudrun_service_account" {
  description = "Service account for Cloud Run (auto-detected if not provided). {{UIMeta group=0 order=103 updatesafe }}"
  type        = string
  default     = null
}

variable "cloudbuild_service_account" {
  description = "Service account for Cloud Build (auto-detected if not provided). {{UIMeta group=0 order=104 updatesafe }}"
  type        = string
  default     = null
}

variable "cloudsql_service_account" {
  description = "Service account for Cloud SQL (auto-detected if not provided). {{UIMeta group=0 order=105 updatesafe }}"
  type        = string
  default     = null
}
...
variable "trusted_users" {
  description = "Email addresses for monitoring alerts and notifications (e.g., ['admin@example.com', 'ops@example.com']). {{UIMeta group=0 order=2002 updatesafe }}"
  type        = list(string)
  default     = []
}
...
variable "deployment_id" {
  description = "Unique ID suffix for resources. Leave blank to generate random ID. {{UIMeta group=0 order=301 }}"
  type        = string
  default     = null
}

variable "configure_environment" {
  description = "Set to true to deploy Cloud Run service. {{UIMeta group=0 order=302 updatesafe }}"
  type        = bool
  default     = true
}

variable "tenant_deployment_id" {
  description = "Specify a unique tenant or deployment identifier. This uniquely identifies your application deployment and is used in resource naming (1-20 lowercase alphanumeric characters and hyphens). {{UIMeta group=1 order=3001 updatesafe }}"
  type        = string
  default     = "demo"
...
variable "application_version" {
  description = "Application version tag (e.g., 6.7.1, latest). {{UIMeta group=0 order=4002 updatesafe }}"
  type        = string
  default     = "6.7.1"
}
...
variable "php_memory_limit" {
  description = "PHP memory limit (e.g., '512M'). {{UIMeta group=0 order=4001 updatesafe }}"
  type        = string
  default     = "512M"
}

variable "post_max_size" {
  description = "Maximum POST data size (e.g., '64M'). {{UIMeta group=0 order=4002 updatesafe }}"
  type        = string
  default     = "64M"
}

variable "upload_max_filesize" {
  description = "Maximum upload file size (e.g., '64M'). {{UIMeta group=0 order=4003 updatesafe }}"
  type        = string
  default     = "64M"
}
...
variable "enable_redis" {
  description = "Enable Redis for object caching. {{UIMeta group=0 order=710 updatesafe }}"
  type        = bool
  default     = true
}

variable "redis_host" {
  description = "Redis host (optional). If provided, overrides the default NFS server Redis. {{UIMeta group=0 order=711 updatesafe }}"
  type        = string
  default     = ""
}

variable "redis_port" {
  description = "Redis port. {{UIMeta group=0 order=712 updatesafe }}"
  type        = string
  default     = "6379"
}
...
variable "vpc_egress_setting" {
  description = "VPC egress setting: 'ALL_TRAFFIC' (route all through VPC) or 'PRIVATE_RANGES_ONLY' (only private IPs through VPC, public direct). {{UIMeta group=0 order=807 updatesafe }}"
  type        = string
...
variable "network_tags" {
  description = "Network tags for Cloud Run service (e.g., ['nfsserver', 'web']). {{UIMeta group=0 order=808 updatesafe }}"
  type        = list(string)
  default     = ["nfsserver"]
}

variable "secret_propagation_delay" {
  description = "Delay in seconds after creating secrets before using them (0-300). {{UIMeta group=0 order=809 updatesafe }}"
  type        = number
  default     = 30
...
variable "database_password_length" {
  description = "Length of auto-generated database password (8-64 characters). {{UIMeta group=0 order=812 updatesafe }}"
  type        = number
  default     = 16
...
variable "gcs_volumes" {
  description = "GCS FUSE volume mounts. Specify volume name, bucket name (null for auto-created), mount path, readonly mode, and mount options. {{UIMeta group=0 order=813 updatesafe }}"
  type = list(object({
    name        = string
...
variable "initialization_jobs" {
  description = "Cloud Run jobs for initialization tasks (e.g., database migrations, data seeding). Jobs can mount NFS/GCS volumes and access secrets. {{UIMeta group=0 order=814 updatesafe }}"
  type = list(object({
    name              = string
...
# ===========================
# SECTION 9: Database Backup & Import
# ===========================
...
variable "backup_source" {
  description = "Backup source: 'gdrive' (Google Drive) or 'gcs' (Google Cloud Storage). GCS is recommended for production due to better security and performance. {{UIMeta group=0 order=10005 updatesafe }}"
  type        = string
  default     = "gcs"
...
variable "backup_uri" {
  description = "Backup URI. For GCS: full URI like 'gs://bucket/path/backup.sql'. For Google Drive: file ID from URL 'https://drive.google.com/file/d/FILE_ID/view'. {{UIMeta group=0 order=10005 updatesafe }}"
  type        = string
  default     = ""
}

variable "backup_format" {
  description = "Backup file format. For GCS: 'sql', 'tar', 'gz', 'tgz', 'tar.gz', 'zip'. For Google Drive: 'sql', 'tar', 'zip'. {{UIMeta group=0 order=10006 updatesafe }}"
  type        = string
  default     = "gz"
...
variable "uptime_check_config" {
  description = "Uptime check configuration. Monitors service availability and sends alerts on failures. {{UIMeta group=0 order=13001 updatesafe }}"
  type = object({
    enabled        = bool
...
variable "alert_policies" {
  description = "Custom alert policies. Define metric-based alerts for your application (e.g., error rates, latency). {{UIMeta group=0 order=13002 updatesafe }}"
  type = list(object({
    name               = string
    metric_type        = string
    comparison         = string
