...
variable "configure_database" {
  description = "Select to configure database on service. {{UIMeta group=0 order=501 updatesafe }}"
  type        = bool
...
variable "application_git_usernames" {
  description = "Specify the usernames to add as collaborators to the git repo. {{UIMeta group=0 order=603 updatesafe}}"
  type        = set(string)
  default     = []
}
...
variable "configure_cloud_storage_volume" {
  description = "Select this checkbox to configure GCS volume. {{UIMeta group=0 order=804 updatesafe }}"
  type        = bool
  default     = true
}
...
variable "configure_backups" {
  description = "Select to schedule application database backups. Configures a Cloud Scheduler trigger to execute a Cloud Run backup job. {{UIMeta group=0 order=808 updatesafe }}"
  type        = bool
  default     = false
}

variable "application_backup_schedule" {
  description = "Enter the application backup schedule in cron format. This is used to configure the Cloud Scheduler cron job. {{UIMeta group=0 order=809 updatesafe }}"
  type        = string
  default     = "0 0 * * *"
}

variable "application_backup_fileid" {
  description = "Enter application backup file ID. When enabled, terraform attempts to download the file from Google Drive, and if found, imports the backup files during deployment. {{UIMeta group=0 order=810 updatesafe}}"
  type        = string
  default     = ""
}

variable "configure_application_security" {
  description = "Select this checkbox to configure web application security.  Configures a global load balancer with Cloud Armor web application security. {{UIMeta group=0 order=806 updatesafe }}"
  type        = bool
  default     = false
}
