...
variable "module_description" {
  ...
  type        = string
  default     = "This module can be used to deploy Cyclos, Django, Moodle, N8N, Odoo, OpenEMR, Wordpress, Ghost, Invoice Ninja, Strapi, Wikijs, Directus or Sanity"
}
...
variable "application_module" {
  description = "Select a pre-configured application module for automatic configuration. Leave empty or null for manual configuration. When using a module, container image, port, database type, resource limits, and other settings are automatically configured. You can still override any module value by explicitly setting the corresponding variable. {{UIMeta group=3 order=299 OPTIONS=odoo,wordpress,moodle,cyclos,django,openemr,n8n,ghost,invoiceninja,strapi,wikijs,directus,sanity updatesafe }}"
  type        = string
  default     = null

  validation {
    condition = var.application_module == null || var.application_module == "" || contains([
      "odoo", "wordpress", "moodle", "cyclos", "django", "openemr", "n8n", "ghost", "wikijs", "sanity", "invoiceninja", "directus", "strapi", "medusa"
    ], var.application_module)
    error_message = "Application module must be one of: odoo, wordpress, moodle, cyclos, django, openemr, n8n, ghost, wikijs, sanity, invoiceninja, directus, strapi, medusa, or leave empty for manual configuration."
  }
}
