...
locals {
  ...
  available_regions = length(local.discovered_regions) > 0 ? local.discovered_regions : [local.region]
  ...
  # Create a map of region to subnet name for easy lookup
  region_to_subnet = {
    for subnet in local.subnet_details :
