...
data "external" "sql_instance_info" {
  program = ["bash", "${path.module}/scripts/get-sqlserver-info.sh", local.project.project_id, local.database_client_type, local.agent_service_account != null ? local.agent_service_account : ""]
}
