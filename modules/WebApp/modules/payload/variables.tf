...
locals {
  payload_module = {
    ...
    # Storage volumes
    # Payload supports GCS adapter.
    # We provision a bucket which can be used by the adapter.
    gcs_volumes = [
      {
        name          = "payload-media"
    ...
    initialization_jobs = [
      {
        name            = "db-init"
        description     = "Create Payload Database and User"
        image           = "alpine:3.19"
        command         = ["/bin/sh", "-c"]
        args            = [
          <<-EOT
    ...
    startup_probe = {
      enabled               = true
      type                  = "HTTP"
      path                  = "/health" # Payload default health check
