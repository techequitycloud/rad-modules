...
odoo_module = {
  ...
  container_args = [
    "echo 'Starting Odoo with DB: $DB_HOST' && echo 'Checking addons directory...' && ls -la /extra-addons && exec odoo --database=\"$DB_NAME\" --db_host=\"$DB_HOST\" --db_port=5432 --db_user=\"$DB_USER\" --db_password=\"$DB_PASSWORD\" --data-dir=/mnt/filestore --addons-path=/usr/lib/python3/dist-packages/odoo/addons,/extra-addons --http-port=8069 --logfile=False --log-level=info --proxy-mode"
  ]
  ...
  initialization_jobs = [
    ...
    },
    {
      ...
      command         = ["/bin/sh", "-c"]
      args            = [
        "mkdir -p /extra-addons && chmod 777 /extra-addons && echo 'GCS addons directory initialized' && ls -la /extra-addons"
      ]
      mount_nfs         = false
    ...
    },
    {
      ...
      command         = ["/bin/bash", "-c"]
      args            = [
        "echo 'Verifying mount points...' && echo 'NFS (/mnt):' && ls -la /mnt/ && echo 'GCS (/extra-addons):' && ls -la /extra-addons && echo 'Starting Odoo initialization...' && odoo -d $DB_NAME --db_host=$DB_HOST --db_port=5432 --db_user=$DB_USER --db_password=$DB_PASSWORD --data-dir=/mnt/filestore --addons-path=/usr/lib/python3/dist-packages/odoo/addons,/extra-addons -i base --stop-after-init --log-level=info"
      ]
      mount_nfs         = true
      mount_gcs_volumes = ["odoo-addons-volume"]
      execute_on_apply  = true
