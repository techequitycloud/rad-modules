...
variable "enable_rad_gcpproject" {
  description = "Set to false to hide the \"GCP Project on RAD\" option for this module -- it may only be deployed into a customer's own GCP project. This module enables 7 APIs the RAD-managed tier policies deny (anthos, gkehub, gkeconnect, connectgateway, gkemulticloud, kubernetesmetadata, opsconfigmonitoring); it also attaches a cluster in another cloud, which a RAD-managed project is not the place for. {{UIMeta group=0 order=110 }}"
  type        = bool
  default     = false
}
