...
| <a name="input_cluster_name_prefix"></a> [cluster\_name\_prefix](#input\_cluster\_name\_prefix) | Prefix used verbatim as the AKS cluster name and the Google Cloud attached-cluster name, and to derive the Resource Group ('<prefix>-rg') and AKS DNS prefix ('<prefix>-dns'). The deployment ID is not appended, so the prefix must be unique per Azure subscription and GCP project. Use lowercase letters, digits, and hyphens only. Defaults to 'azure-aks-cluster'. | `string` | `"azure-aks-cluster"` | no |
...
| <a name="input_platform_version"></a> [platform\_version](#input\_platform\_version) | GKE Hub Attached Clusters platform version for the managed components installed onto the AKS cluster (format: major.minor.patch-gke.N, e.g. '1.35.0-gke.1'). Its minor version must equal k8s\_version or be exactly one below it. Defaults to '1.35.0-gke.1'. Run 'gcloud container attached get-server-config --location=<gcp\_location>' for the versions currently offered. | `string` | `"1.35.0-gke.1"` | no |
| <a name="input_public_access"></a> [public\_access](#input\_public\_access) | Set to true to make this module visible and deployable by all platform users. Set to false to restrict the module to platform administrators only. Defaults to true (visible and deployable by all platform users). | `bool` | `true` | no |
...
| <a name="input_module_documentation"></a> [module\_documentation](#input\_module\_documentation) | URL linking to the external documentation for this module. Displayed in the platform UI as a help reference. Metadata only. | `string` | `"https://docs.radmodules.dev/docs/modules/AKS_GKE"` | no |
| <a name="input_shared_users"></a> [shared\_users](#input\_shared\_users) | List of users who can view and deploy this module regardless of the public\_access setting. Metadata only — consumed by the deployment platform, not by the Terraform module. | `list(string)` | <pre>[<br>  "shiyghan.navti@techequity.cloud"<br>]</pre> | no |
