...
variable "module_description" {
  ...
  type        = string
  default     = "This module deploys the Odoo Enterprise Resource Planning (ERP) application on Google Kubernetes Engine (GKE), providing a comprehensive suite of business management tools including CRM, e-commerce, billing, and accounting in a complete and secure environment."
}
...
variable "application_release" {
  description = "Enter application release. Container images are tagged with this version number. {{UIMeta group=0 order=505 updatesafe}}"
  type        = string
  default     = "20251008"
}

variable "application_sha" {
  description = "Enter application sha. Container images are tagged with this version number. {{UIMeta group=0 order=506 updatesafe}}"
  type        = string
...
variable "configure_continuous_integration" {
  description = "Select the checkbox to configure GitHub continuous integration and continous delivery pipeline that supports single and multi-region deployment. {{UIMeta group=4 order=602 updatesafe}}"
  type        = bool
  default     = false
}
