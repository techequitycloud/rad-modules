...
# Resource for creating moodlecron local script directly from an existing file.
resource "local_file" "dev_moodlecron" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
  filename = "${path.module}/scripts/ci/moodlecron"
  content  = file("${path.module}/scripts/ci/moodlecron.tpl")
...
resource "local_file" "dev_moodle_config" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
  filename = "${path.module}/scripts/ci/moodle-config.php"
  content  = file("${path.module}/scripts/ci/moodle-config.php.tpl")
...
resource "local_file" "dev_foreground" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
...
# Resource for creating moodlecron local script directly from an existing file.
resource "local_file" "qa_moodlecron" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  filename = "${path.module}/scripts/ci/moodlecron"
  content  = file("${path.module}/scripts/ci/moodlecron.tpl")
...
# Resource for creating a local configuration file directly from an existing template file.
resource "local_file" "qa_moodle_config" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  filename = "${path.module}/scripts/ci/moodle-config.php"
  content  = file("${path.module}/scripts/ci/moodle-config.php.tpl")
...
# Resource for creating a local script, directly from an existing template file.
resource "local_file" "qa_foreground" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  filename = "${path.module}/scripts/ci/foreground.sh"
  content  = file("${path.module}/scripts/ci/foreground.sh.tpl")
...
# Resource for creating moodlecron local script directly from an existing file.
resource "local_file" "prod_moodlecron" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_production_environment ? 1 : 0
  filename = "${path.module}/scripts/ci/moodlecron"
  content  = file("${path.module}/scripts/ci/moodlecron.tpl")
...
# Resource for creating a local configuration file directly from an existing template file.
resource "local_file" "prod_moodle_config" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_production_environment ? 1 : 0
  filename = "${path.module}/scripts/ci/moodle-config.php"
  content  = file("${path.module}/scripts/ci/moodle-config.php.tpl")
...
# Resource definition for a GitHub repository file for the moodlecron script
resource "github_repository_file" "dev_moodlecron" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "dev"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "moodlecron"
  content             = local_file.dev_moodlecron[count.index].content

  depends_on      = [
    local_file.dev_moodlecron,
    null_resource.init_git_repo
  ]
}

# Resource definition for a GitHub repository file for the configuration
resource "github_repository_file" "dev_moodle_config" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "dev"
  commit_message      = "Add file to repo"
  ...
  content             = local_file.dev_moodle_config[count.index].content

  depends_on      = [
    local_file.dev_moodle_config,
    null_resource.init_git_repo
  ]
}

# Resource definition for a GitHub repository file for the script
resource "github_repository_file" "dev_foreground" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "dev"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "foreground.sh"
  content             = local_file.dev_foreground[count.index].content

  depends_on      = [
    local_file.dev_foreground,
    null_resource.init_git_repo
  ]
}
...
# Resource definition for a GitHub repository file for the moodlecron script
resource "github_repository_file" "qa_moodlecron" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "qa"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "moodlecron"
  content             = local_file.qa_moodlecron[count.index].content

  depends_on      = [
    local_file.qa_moodlecron,
    null_resource.init_git_repo
  ]
}

# Resource definition for a GitHub repository file for the configuration
resource "github_repository_file" "qa_moodle_config" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "qa"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "moodle-config.php"
  content             = local_file.qa_moodle_config[count.index].content

  depends_on      = [
    local_file.qa_moodle_config,
    null_resource.init_git_repo
  ]
}

# Resource definition for a GitHub repository file for the script
resource "github_repository_file" "qa_foreground" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "qa"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "foreground.sh"
  content             = local_file.qa_foreground[count.index].content

  depends_on      = [
    local_file.qa_foreground,
    null_resource.init_git_repo
  ]
}
...
resource "github_repository_file" "prod_moodle_config" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_production_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "prod"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "moodle-config.php"
  content             = local_file.prod_moodle_config[count.index].content

  depends_on      = [
    local_file.prod_moodle_config,
    null_resource.init_git_repo
  ]
}
...
resource "github_repository_file" "prod_moodlecron" {
  ...
  content             = local_file.prod_moodlecron[count.index].content

  depends_on      = [
    local_file.prod_moodlecron,
    null_resource.init_git_repo
  ]
}
