...
variable "module_description" {
  ...
  type        = string
  default     = "Moodle: Deploy a scalable Learning Management System (LMS) on Google Cloud Run with PostgreSQL 15 and GCS Fuse storage for course materials. Built for educational institutions to support thousands of concurrent students with auto-scaling infrastructure. Features PHP 8.3/Apache, automated installation, pg_trgm search optimization, Redis session handling, automated cron jobs, PDF annotation support, and health check optimization. Includes automated backups for course data, global deployment options, and high-performance content delivery. Ideal for schools, universities, corporate training, and online learning platforms."
}

variable "module_documentation" {
  description = "The URL to the module documentation. {{UIMeta group=0 order=102 }}"
  type        = string
  default     = "https://docs.techequity.cloud/docs/applications/moodle"
}
...
variable "module_services" {
  ...
  type        = list(string)
  default     = ["Cloud Run", "Cloud Run Jobs", "Cloud Build", "Artifact Registry", "Cloud Storage", "GCS Fuse", "Cloud SQL (PostgreSQL 15)", "PostgreSQL Extensions", "Filestore (Optional NFS)", "VPC Network", "Serverless VPC Access", "Secret Manager", "Cloud IAM", "Cloud Logging", "Cloud Monitoring", "Health Checks", "Cloud Scheduler", "Cloud CDN", "Cloud Armor"]
}
...
variable "redis_auth" {
  description = "Redis password/auth string. Leave empty if no auth. {{UIMeta group=0 order=706 updatesafe }}"
  type        = string
  default     = ""
  sensitive   = true
}
