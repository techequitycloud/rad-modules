...
resource "null_resource" "get_gke_credentials" {
  ...
  depends_on = [
    local_file.dev_statefulset_yaml_output,
    local_file.qa_statefulset_yaml_output,
    local_file.prod_statefulset_yaml_output,
...
timeout 300 bash -c '
gcloud container clusters get-credentials ${local.gke_cluster_name} --region ${local.gke_cluster_region} --project ${local.project.project_id}
kubectl apply -f ${local_file.dev_service_dns_yaml_output[0].filename} || true
' || true
EOF
  kubectl_destroy_command     = <<EOF
timeout 300 bash -c '
kubectl delete -f ${local_file.dev_service_dns_yaml_output[0].filename} --force || true
' || true
EOF
...
timeout 300 bash -c '
gcloud container clusters get-credentials ${local.gke_cluster_name} --region ${local.gke_cluster_region} --project ${local.project.project_id}
kubectl apply -f ${local_file.dev_statefulset_yaml_output[0].filename} || true
' || true
EOF
  kubectl_destroy_command     = <<EOF
timeout 300 bash -c '
kubectl delete -f ${local_file.dev_statefulset_yaml_output[0].filename} --force || true
' || true
EOF
...
timeout 300 bash -c '
gcloud container clusters get-credentials ${local.gke_cluster_name} --region ${local.gke_cluster_region} --project ${local.project.project_id}
kubectl apply -f ${local_file.qa_service_dns_yaml_output[0].filename} || true
' || true
EOF
  kubectl_destroy_command     = <<EOF
timeout 300 bash -c '
kubectl delete -f ${local_file.qa_service_dns_yaml_output[0].filename} --force || true
' || true
EOF
...
timeout 300 bash -c '
gcloud container clusters get-credentials ${local.gke_cluster_name} --region ${local.gke_cluster_region} --project ${local.project.project_id}
kubectl apply -f ${local_file.qa_statefulset_yaml_output[0].filename} || true
' || true
EOF
  kubectl_destroy_command     = <<EOF
timeout 300 bash -c '
kubectl delete -f ${local_file.qa_statefulset_yaml_output[0].filename} --force || true
' || true
EOF
...
module "deploy_prod_service_dns" {
  count                       = local.gke_cluster_exists && var.configure_production_environment ? 1 : 0
  source                      = "terraform-google-modules/gcloud/google//modules/kubectl-wrapper"
  project_id                  = local.project.project_id
  cluster_name                = local.gke_cluster_name
...
timeout 300 bash -c '
gcloud container clusters get-credentials ${local.gke_cluster_name} --region ${local.gke_cluster_region} --project ${local.project.project_id}
kubectl apply -f ${local_file.prod_service_dns_yaml_output[0].filename} || true
' || true
EOF
  kubectl_destroy_command     = <<EOF
timeout 300 bash -c '
kubectl delete -f ${local_file.prod_service_dns_yaml_output[0].filename} --force || true
' || true
EOF
...
timeout 300 bash -c '
gcloud container clusters get-credentials ${local.gke_cluster_name} --region ${local.gke_cluster_region} --project ${local.project.project_id}
kubectl apply -f ${local_file.prod_statefulset_yaml_output[0].filename} || true
' || true
EOF
  kubectl_destroy_command     = <<EOF
timeout 300 bash -c '
kubectl delete -f ${local_file.prod_statefulset_yaml_output[0].filename} --force || true
' || true
EOF
