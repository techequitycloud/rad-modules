...
resource "local_file" "clouddeploy_dev_service_dns" {
    count                     = var.configure_continuous_deployment  ? 1 : 0
    filename                  = "${path.module}/scripts/cd/app/base_dev/service-dns.yaml"
    content                   = templatefile("${path.module}/scripts/cd/app/base_dev/service-dns.yaml.tpl", {
    APP_NAME                  = "app${var.application_name}${local.random_id}"
  })
...
resource "local_file" "clouddeploy_dev_statefulset_app" {
    count                     = var.configure_continuous_deployment  ? 1 : 0
    filename                  = "${path.module}/scripts/cd/app/overlay_dev/statefulset-app.yaml"
    content                   = templatefile("${path.module}/scripts/cd/app/overlay_dev/statefulset-app.yaml.tpl", {
    PROJECT_ID                = local.project.project_id
...
# Resource for creating a local service DNS configuration file from a template
resource "local_file" "clouddeploy_qa_service_dns" {
    count                     = var.configure_continuous_deployment  ? 1 : 0
    filename                  = "${path.module}/scripts/cd/app/base_qa/service-dns.yaml"
    content                   = templatefile("${path.module}/scripts/cd/app/base_qa/service-dns.yaml.tpl", {
    APP_NAME                  = "app${var.application_name}${local.random_id}"
  })
...
# Resource for creating a statefulset configuration for the stateful app from a template.
# This includes all necessary information for the statefulset of the application to Kubernetes.
resource "local_file" "clouddeploy_qa_statefulset_app" {
    count                     = var.configure_continuous_deployment  ? 1 : 0
    filename                  = "${path.module}/scripts/cd/app/overlay_qa/statefulset-app.yaml"
    content                   = templatefile("${path.module}/scripts/cd/app/overlay_qa/statefulset-app.yaml.tpl", {
    PROJECT_ID                = local.project.project_id
    GCP_SERVICE_ACCOUNT       = "gke-sa@${local.project.project_id}.iam.gserviceaccount.com"
    APP_REGION                = local.region
    APP_NAME                  = "app${var.application_name}${local.random_id}"
...
# Resource for creating a local service DNS configuration file from a template
resource "local_file" "clouddeploy_prod_service_dns" {
    count                     = var.configure_continuous_deployment  ? 1 : 0
    filename                  = "${path.module}/scripts/cd/app/base_prod/service-dns.yaml"
    content                   = templatefile("${path.module}/scripts/cd/app/base_prod/service-dns.yaml.tpl", {
    APP_NAME                  = "app${var.application_name}${local.random_id}"
  })
...
# Resource for creating a statefulset configuration for the stateful app from a template.
# This includes all necessary information for the statefulset of the application to Kubernetes.
resource "local_file" "clouddeploy_prod_statefulset_app" {
    count                     = var.configure_continuous_deployment  ? 1 : 0
    filename                  = "${path.module}/scripts/cd/app/overlay_prod/statefulset-app.yaml"
    content                   = templatefile("${path.module}/scripts/cd/app/overlay_prod/statefulset-app.yaml.tpl", {
    PROJECT_ID                = local.project.project_id
    GCP_SERVICE_ACCOUNT       = "gke-sa@${local.project.project_id}.iam.gserviceaccount.com"
    APP_REGION                = local.region
    APP_NAME                  = "app${var.application_name}${local.random_id}"
