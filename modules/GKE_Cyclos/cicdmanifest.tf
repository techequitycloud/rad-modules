...
# Resource for creating a local service DNS configuration file from a template
resource "local_file" "dev_service_dns" {
    count                     = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
    filename                  = "${path.module}/scripts/ci/base/dev/service-dns.yaml"
    content                   = templatefile("${path.module}/scripts/ci/base/service-dns.yaml.tpl", {
    APP_NAME                  = "app${var.application_name}${local.random_id}"
  })
...
# Resource for creating a statefulset configuration for the stateful app from a template.
# This includes all necessary information for the statefulset of the application to Kubernetes.
resource "local_file" "dev_statefulset_app" {
    count                     = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
    filename                  = "${path.module}/scripts/ci/overlay/dev/statefulset-app.yaml"
    content                   = templatefile("${path.module}/scripts/ci/overlay/statefulset-app.yaml.tpl", {
    PROJECT_ID                = local.project.project_id
...
# Resource for creating a local cloud run properties file directly from an existing file.
resource "local_file" "dev_cyclos_properties" {
    count                     = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
    filename                  = "${path.module}/scripts/ci/dev/cyclos.properties"
    content                   = file("${path.module}/scripts/ci/cyclos.properties.tpl"
  )
...
# Resource for creating a hazelcast configuration file directly from an existing file.
resource "local_file" "dev_hazelcast" {
    count                     = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
    filename                  = "${path.module}/scripts/ci/dev/hazelcast.xml"
    content                   = file("${path.module}/scripts/ci/hazelcast.xml.tpl"
  )
...
# Resource for creating a local service DNS configuration file from a template
resource "local_file" "qa_service_dns" {
    count                     = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
    filename                  = "${path.module}/scripts/ci/base/qa/service-dns.yaml"
    content                   = templatefile("${path.module}/scripts/ci/base/service-dns.yaml.tpl", {
    APP_NAME                  = "app${var.application_name}${local.random_id}"
  })
...
# Resource for creating a statefulset configuration for the stateful app from a template.
# This includes all necessary information for the statefulset of the application to Kubernetes.
resource "local_file" "qa_statefulset_app" {
    count                     = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
    filename                  = "${path.module}/scripts/ci/overlay/qa/statefulset-app.yaml"
    content                   = templatefile("${path.module}/scripts/ci/overlay/statefulset-app.yaml.tpl", {
    PROJECT_ID                = local.project.project_id
    GCP_SERVICE_ACCOUNT       = "gcp-project-account@gcp-sa-cloudbuild.iam.gserviceaccount.com"
    APP_REGION                = local.region
...
# Resource for creating a local cloud run properties file directly from an existing file.
resource "local_file" "qa_cyclos_properties" {
  count    = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  filename = "${path.module}/scripts/ci/qa/cyclos.properties"
  content  = file("${path.module}/scripts/ci/cyclos.properties.tpl")
...
# Resource for creating a hazelcast configuration file directly from an existing file.
resource "local_file" "qa_hazelcast" {
  count    = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  filename = "${path.module}/scripts/ci/qa/hazelcast.xml"
  content  = file("${path.module}/scripts/ci/hazelcast.xml.tpl")
...
# Resource for creating a local service DNS configuration file from a template
resource "local_file" "prod_service_dns" {
    count                     = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_production_environment ? 1 : 0
    filename                  = "${path.module}/scripts/ci/base/prod/service-dns.yaml"
    content                   = templatefile("${path.module}/scripts/ci/base/service-dns.yaml.tpl", {
    APP_NAME                  = "app${var.application_name}${local.random_id}"
  })
...
# Resource for creating a statefulset configuration for the stateful app from a template.
# This includes all necessary information for the statefulset of the application to Kubernetes.
resource "local_file" "prod_statefulset_app" {
    count                     = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_production_environment ? 1 : 0
    filename                  = "${path.module}/scripts/ci/overlay/prod/statefulset-app.yaml"
    content                   = templatefile("${path.module}/scripts/ci/overlay/statefulset-app.yaml.tpl", {
    PROJECT_ID                = local.project.project_id
    GCP_SERVICE_ACCOUNT       = "gcp-project-account@gcp-sa-cloudbuild.iam.gserviceaccount.com"
    APP_REGION                = local.region
...
# Resource for creating a local cloud run properties file directly from an existing file.
resource "local_file" "prod_cyclos_properties" {
  count    = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_production_environment ? 1 : 0
  filename = "${path.module}/scripts/ci/prod/cyclos.properties"
  content  = file("${path.module}/scripts/ci/cyclos.properties.tpl")
...
# Resource for creating a hazelcast configuration file directly from an existing file.
resource "local_file" "prod_hazelcast" {
  count    = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_production_environment ? 1 : 0
  filename = "${path.module}/scripts/ci/prod/hazelcast.xml"
  content  = file("${path.module}/scripts/ci/hazelcast.xml.tpl")
...
# Resource for service DNS configuration file in a GitHub repository
resource "github_repository_file" "dev_service_dns" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "dev"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "base/service-dns.yaml"
  content             = local_file.dev_service_dns[count.index].content

  depends_on = [
    local_file.dev_service_dns,
    null_resource.init_git_repo
  ]
}
...
# Resource definition for a GitHub repository file for the statefulset app configuration
resource "github_repository_file" "dev_statefulset_app" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "dev"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "overlay/statefulset-app.yaml"
  content             = local_file.dev_statefulset_app[count.index].content

  depends_on      = [
    local_file.dev_statefulset_app,
    null_resource.init_git_repo
  ]
}

# Resource definition for a cyclos properties file for the Cloud Run application
resource "github_repository_file" "dev_cyclos_properties" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "dev"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "cyclos.properties"
  content             = local_file.dev_cyclos_properties[count.index].content

  depends_on      = [
    local_file.dev_cyclos_properties,
    null_resource.init_git_repo
  ]
}

# Resource definition for a GitHub repository file for the hazelcast configuration
resource "github_repository_file" "dev_hazelcast" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_development_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "dev"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "hazelcast.xml"
  content             = local_file.dev_hazelcast[count.index].content

  depends_on      = [
    local_file.dev_hazelcast,
    null_resource.init_git_repo
  ]
}
...
# Resource for service DNS configuration file in a GitHub repository
resource "github_repository_file" "qa_service_dns" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "qa"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "base/service-dns.yaml"
  content             = local_file.qa_service_dns[count.index].content

  depends_on = [
    local_file.qa_service_dns,
    null_resource.init_git_repo
  ]
}
...
# Resource definition for a GitHub repository file for the statefulset app configuration
resource "github_repository_file" "qa_statefulset_app" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "qa"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "overlay/statefulset-app.yaml"
  content             = local_file.qa_statefulset_app[count.index].content

  depends_on      = [
    local_file.qa_statefulset_app,
    null_resource.init_git_repo
  ]
}

# Resource definition for a cyclos properties file for the Cloud Run application
resource "github_repository_file" "qa_cyclos_properties" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "qa"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "cyclos.properties"
  content             = local_file.qa_cyclos_properties[count.index].content

  depends_on      = [
    local_file.qa_cyclos_properties,
    null_resource.init_git_repo
  ]
}

# Resource definition for a GitHub repository file for the hazelcast configuration
resource "github_repository_file" "qa_hazelcast" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_nonproduction_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "qa"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "hazelcast.xml"
  content             = local_file.qa_hazelcast[count.index].content

  depends_on      = [
    local_file.qa_hazelcast,
    null_resource.init_git_repo
  ]
}
...
# Resource for service DNS configuration file in a GitHub repository
resource "github_repository_file" "prod_service_dns" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_production_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "prod"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "base/service-dns.yaml"
  content             = local_file.prod_service_dns[count.index].content

  depends_on = [
    local_file.prod_service_dns,
    null_resource.init_git_repo
  ]
}
...
# Resource definition for a GitHub repository file for the statefulset app configuration
resource "github_repository_file" "prod_statefulset_app" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_production_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "prod"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "overlay/statefulset-app.yaml"
  content             = local_file.prod_statefulset_app[count.index].content

  depends_on      = [
    local_file.prod_statefulset_app,
    null_resource.init_git_repo
  ]
}

# Resource definition for a cyclos properties file for the Cloud Run application
resource "github_repository_file" "prod_cyclos_properties" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_production_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "prod"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "cyclos.properties"
  content             = local_file.prod_cyclos_properties[count.index].content

  depends_on      = [
    local_file.prod_cyclos_properties,
    null_resource.init_git_repo
  ]
}

# Resource definition for a GitHub repository file for the hazelcast configuration
resource "github_repository_file" "prod_hazelcast" {
  count               = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_production_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "prod"
  commit_message      = "Add file to repo"
  overwrite_on_create = true
  file                = "hazelcast.xml"
  content             = local_file.prod_hazelcast[count.index].content

  depends_on      = [
    local_file.prod_hazelcast,
    null_resource.init_git_repo
  ]
}
