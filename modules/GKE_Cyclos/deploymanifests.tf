...
resource "local_file" "dev_service_dns_yaml_output" {
  count    = var.configure_development_environment ? 1 : 0
  filename = "${path.module}/manifests/dev/service_dns.yaml"

  content = templatefile("${path.module}/templates/service_dns.yaml.tpl", {
    APPLICATION_NAME      = "app${var.application_name}${local.random_id}dev"
    APPLICATION_NAMESPACE = "${var.application_name}${var.tenant_deployment_id}dev"
    SERVICE_ACCOUNT_NAME  = "app${var.application_name}${local.random_id}dev"
  })
...
resource "local_file" "dev_statefulset_yaml_output" {
  count    = var.configure_development_environment ? 1 : 0
  filename = "${path.module}/manifests/dev/statefulset.yaml"

  content = templatefile("${path.module}/templates/statefulset.yaml.tpl", {
    GCP_PROJECT           = local.project.project_id
    GCP_SERVICE_ACCOUNT   = "gke-sa@${local.project.project_id}.iam.gserviceaccount.com"
    APPLICATION_NAME      = "app${var.application_name}${local.random_id}dev"
    APPLICATION_REGION    = local.region
    APPLICATION_NAMESPACE = "${var.application_name}${var.tenant_deployment_id}dev"
    APPLICATION_DOMAIN    = "app${var.application_name}${local.random_id}dev.${google_compute_global_address.dev[count.index].address}.sslip.io"
    APPLICATION_IP        = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}dev"
...
resource "local_file" "qa_service_dns_yaml_output" {
  count    = var.configure_nonproduction_environment ? 1 : 0
  filename = "${path.module}/manifests/qa/service_dns.yaml"
...
resource "local_file" "qa_statefulset_yaml_output" {
  count    = var.configure_nonproduction_environment ? 1 : 0
  filename = "${path.module}/manifests/qa/statefulset.yaml"
  ...
  content = templatefile("${path.module}/templates/statefulset.yaml.tpl", {
    ...
    APPLICATION_REGION    = local.region
    APPLICATION_NAMESPACE = "${var.application_name}${var.tenant_deployment_id}qa"
    APPLICATION_DOMAIN    = "app${var.application_name}${local.random_id}qa.${google_compute_global_address.qa[count.index].address}.sslip.io"
    APPLICATION_IP        = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}qa"
...
resource "local_file" "prod_service_dns_yaml_output" {
  count    = var.configure_production_environment ? 1 : 0
  filename = "${path.module}/manifests/prod/service_dns.yaml"
...
resource "local_file" "prod_statefulset_yaml_output" {
  count    = var.configure_production_environment ? 1 : 0
  filename = "${path.module}/manifests/prod/statefulset.yaml"
  ...
  content = templatefile("${path.module}/templates/statefulset.yaml.tpl", {
    ...
    APPLICATION_REGION    = local.region
    APPLICATION_NAMESPACE = "${var.application_name}${var.tenant_deployment_id}prod"
    APPLICATION_DOMAIN    = "app${var.application_name}${local.random_id}prod.${google_compute_global_address.prod[count.index].address}.sslip.io"
    APPLICATION_IP        = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}prod"
