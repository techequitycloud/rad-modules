  ...
  template {
    template {
      ...
      containers {
        ...
        env {
          name  = "APP_NAME"
          value = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}"
        }

        env {
          name  = "APP_REGION_1"
          value = length(local.regions) > 0 ? local.regions[0] : ""
        }

        env {
          name  = "APP_REGION_2"
          value = length(local.regions) > 1 ? local.regions[1] : ""
        }
