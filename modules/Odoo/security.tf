...
resource "google_security_scanner_scan_config" "scanner_config" {
  ...
  starting_urls    = ["https://app${var.application_name}${var.tenant_deployment_id}${local.random_id}.${google_compute_global_address.app[0].address}.nip.io"]
  target_platforms = ["COMPUTE"]

  # Dependencies to ensure resources are created in the correct order
  depends_on = [
    google_compute_global_address.app,
    time_sleep.wait_for_app_ip,
    google_monitoring_uptime_check_config.app_https,
  ]
}

resource "time_sleep" "wait_for_app_ip" {
