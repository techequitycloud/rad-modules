...
variable "module_description" {
  ...
  type        = string
  default     = "Odoo: Deploy comprehensive ERP suite (CRM, Accounting, Inventory, Manufacturing, eCommerce) on Google Cloud Run serverless architecture. Includes Odoo Community/Enterprise Edition with PostgreSQL 15, Cloud Filestore NFS for shared attachments and sessions, optional Redis for caching, and automated daily backups. Features 4-step initialization (NFS, database, config, schema), scale-to-zero (0-3 instances), direct VPC egress, threaded mode for XML-RPC/Longpolling on port 8069, custom Ubuntu 24.04 build with wkhtmltopdf, Secret Manager encryption, and enterprise-grade security. Consolidates fragmented business tools into unified platform with high reliability, automated backups, private database connections, and custom add-on support. Ideal for all-in-one business management, secure enterprise deployments, and customizable business processes."
}

variable "module_documentation" {
  description = "The URL to the module documentation. {{UIMeta group=0 order=102 }}"
  type        = string
  default     = "https://docs.techequity.cloud/docs/applications/odoo"
}
...
variable "module_services" {
  ...
  type        = list(string)
  default     = ["Cloud Run Gen 2", "Cloud Run Jobs", "Cloud Build", "Artifact Registry", "Cloud Storage", "Cloud SQL", "Cloud SQL Auth Proxy", "PostgreSQL 15", "Cloud Filestore", "NFS", "Redis", "VPC Network", "Direct VPC Egress", "Serverless VPC Access", "Secret Manager", "Cloud IAM", "Cloud Logging", "Cloud Monitoring", "Health Checks", "Automated Backups"]
}
...
variable "application_version" {
  description = "Application version tag (e.g., 18.0, latest). {{UIMeta group=0 order=4002 updatesafe }}"
  type        = string
  default     = "18.0"
}
