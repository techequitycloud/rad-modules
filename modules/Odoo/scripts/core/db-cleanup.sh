...
if [ "$DB_TYPE" = "POSTGRES" ]; then
    ...
    if [ "$DB_EXISTS" = "1" ]; then
        ...
        psql -h "$DB_HOST" -p "${DB_PORT:-5432}" -U postgres -d postgres <<EOF
...
EOF
        ...
        # Step 3: CRITICAL FIX - Grant role to postgres first
        ...
        psql -h "$DB_HOST" -p "${DB_PORT:-5432}" -U postgres -d "$DB_NAME" <<EOF
...
EOF
        ...
        psql -h "$DB_HOST" -p "${DB_PORT:-5432}" -U postgres -d postgres -c \
            "DROP DATABASE IF EXISTS \"$DB_NAME\";" || {
                ...
                psql -h "$DB_HOST" -p "${DB_PORT:-5432}" -U postgres -d postgres -c \
                    "SELECT pid, usename, application_name, state FROM pg_stat_activity WHERE datname = '$DB_NAME';"
                exit 1
            }
    ...
    if [ "$USER_EXISTS" = "1" ]; then
        ...
        for db in $(psql -h "$DB_HOST" -p "${DB_PORT:-5432}" -U postgres -d postgres -tAc \
            ...
            psql -h "$DB_HOST" -p "${DB_PORT:-5432}" -U postgres -d postgres -c \
                "REVOKE ALL PRIVILEGES ON DATABASE \"$db\" FROM \"$DB_USER\";" \
                2>/dev/null || true
        done
