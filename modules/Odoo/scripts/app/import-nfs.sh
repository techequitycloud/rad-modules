...
while [ $attempt -lt $max_attempts ]; do
  ...
  if gcloud run services describe "appodoodemoaeab" --project="qwiklabs-gcp-00-9c58e150e7c1" --region="$APP_REGION_1" 2>/dev/null; then
    echo "Cloud Run service still exists in region $APP_REGION_1. Attempting to delete..."
    
    # Try to delete the service
    if gcloud run services delete "appodoodemoaeab" --project="qwiklabs-gcp-00-9c58e150e7c1" --region="$APP_REGION_1" --quiet; then
  ...
  if gcloud run services describe "appodoodemoaeab" --project="qwiklabs-gcp-00-9c58e150e7c1" --region="$APP_REGION_2" 2>/dev/null; then
    echo "Cloud Run service still exists in region $APP_REGION_2. Attempting to delete..."
    
    # Try to delete the service
    if gcloud run services delete "appodoodemoaeab" --project="qwiklabs-gcp-00-9c58e150e7c1" --region="$APP_REGION_2" --quiet; then
...
sudo mkdir -p /share/appodoodemoaeab && sudo rm -rf /share/appodoodemoaeab/* && sudo chown -R nobody:nogroup /share/appodoodemoaeab && sudo chmod 775 /share/appodoodemoaeab

# Check if the shared directory exists
if [ ! -d /share/appodoodemoaeab ]; then echo 'Error: /share/appodoodemoaeab does not exist.'; exit 1; fi
