...
while [ $attempt -lt $max_attempts ]; do
  ...
  if gcloud run services describe "appodoodemo36a1" --project="qwiklabs-gcp-00-9c58e150e7c1" --region="$APP_REGION_1" 2>/dev/null; then
    echo "Cloud Run service still exists in region $APP_REGION_1. Attempting to delete..."
    
    # Try to delete the service
    if gcloud run services delete "appodoodemo36a1" --project="qwiklabs-gcp-00-9c58e150e7c1" --region="$APP_REGION_1" --quiet; then
  ...
  if gcloud run services describe "appodoodemo36a1" --project="qwiklabs-gcp-00-9c58e150e7c1" --region="$APP_REGION_2" 2>/dev/null; then
    echo "Cloud Run service still exists in region $APP_REGION_2. Attempting to delete..."
    
    # Try to delete the service
    if gcloud run services delete "appodoodemo36a1" --project="qwiklabs-gcp-00-9c58e150e7c1" --region="$APP_REGION_2" --quiet; then
...
sudo mkdir -p /share/appodoodemo36a1 && sudo rm -rf /share/appodoodemo36a1/* && sudo chown -R nobody:nogroup /share/appodoodemo36a1 && sudo chmod 775 /share/appodoodemo36a1

# Check if the shared directory exists
if [ ! -d /share/appodoodemo36a1 ]; then echo 'Error: /share/appodoodemo36a1 does not exist.'; exit 1; fi
