...
export PG_PASS=${11}
export APP_NAME=${12}
export APP_REGION=${13}
export APP_NAMESPACE=${14}
export GCP_CLUSTER=${15}
export ASSET_BUCKET=${16}
export SERVICE_ACCOUNT=${17}
...
# Loop to check if the statefulset exists and delete it
while kubectl get statefulset ${APP_NAME} --namespace=${APP_NAMESPACE}; do
    echo "Statefulset ${APP_NAME} exists. Attempting to delete... (Attempt: $((attempt + 1)) of $max_attempts)"
    
    # If the statefulset is still there, try to delete it
    if kubectl delete statefulset ${APP_NAME} --namespace=${APP_NAMESPACE} --force; then
        echo "Statefulset ${APP_NAME} is being deleted."
        delete_attempted=true
        break
    else
        echo "Failed to delete statefulset ${APP_NAME}. Retrying..."
        attempt=$((attempt + 1))
...
if [ -n "${BACKUP_FILE}" ] && [ -f "${BACKUP_FILE}" ]; then
    ...
    # Change ownership
    gcloud compute --project ${PROJECT_ID} --quiet ssh ${NFS_VM} --zone ${NFS_ZONE} $SA_ARG --command="sudo chmod -R 775 /share/${DB_USER} && sudo chown -R nobody:nogroup /share/${DB_USER}"
