...
resource "null_resource" "import_dev_dump" {
  count      = var.create_cloud_sql && var.create_network_filesystem ? 1 : 0  
  triggers = {
    ...
    db_pass           = data.google_secret_manager_secret_version.dev_db_password[0].secret_data
    ...
    import_hash       = filesha256("${path.module}/scripts/app/import-db.sh")
    delete_hash       = filesha256("${path.module}/scripts/app/delete-db.sh")
...
resource "null_resource" "import_qa_dump" {
  ...
  triggers = {
    ...
    db_pass           = data.google_secret_manager_secret_version.qa_db_password[0].secret_data
    pg_pass           = "${google_secret_manager_secret_version.postgres_password[0].secret_data}"
...
resource "null_resource" "import_prod_dump" {
  ...
  triggers = {
    ...
    db_pass           = data.google_secret_manager_secret_version.prod_db_password[0].secret_data
    pg_pass           = "${google_secret_manager_secret_version.postgres_password[0].secret_data}"
