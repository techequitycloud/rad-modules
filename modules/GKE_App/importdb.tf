...
#########################################################################
# Configurations for Postgres backup import
#########################################################################

# Create db import script
resource "local_file" "import_dev_postgres_db_script_output" {
  count    = local.sql_server_exists ? 1 : 0  
  filename = "${path.module}/scripts/app/dev/import-db.sh"
  content = templatefile("${path.module}/scripts/app/import_postgres_db.tpl", {
...
# Create db import script
resource "local_file" "import_qa_postgres_db_script_output" {
  count    = local.sql_server_exists ? 1 : 0  
...
# Create db import script
resource "local_file" "import_prod_postgres_db_script_output" {
  count    = local.sql_server_exists ? 1 : 0  
...
#########################################################################
# Configurations for MySQL backup import
#########################################################################

# Create db import script
resource "local_file" "import_dev_mysql_db_script_output" {
  count    = !local.sql_server_exists ? 1 : 0  
  filename = "${path.module}/scripts/app/dev/import-db.sh"
  content = templatefile("${path.module}/scripts/app/import_mysql_db.tpl", {
    PROJECT_ID          = local.project.project_id
    BACKUP_FILEID       = "${var.application_backup_fileid}"
    DB_IP               = local.db_internal_ip
    DB_NAME             = "app${var.application_database_name}${var.tenant_deployment_id}${local.random_id}dev"
...
# Create db import script
resource "local_file" "import_qa_mysql_db_script_output" {
  count    = !local.sql_server_exists ? 1 : 0  
  filename = "${path.module}/scripts/app/qa/import-db.sh"
  content = templatefile("${path.module}/scripts/app/import_mysql_db.tpl", {
    PROJECT_ID          = local.project.project_id
...
# Create db import script
resource "local_file" "import_prod_mysql_db_script_output" {
  count    = !local.sql_server_exists ? 1 : 0  
  filename = "${path.module}/scripts/app/prod/import-db.sh"
  content = templatefile("${path.module}/scripts/app/import_mysql_db.tpl", {
    PROJECT_ID          = local.project.project_id
...
resource "null_resource" "import_dev_db" {
  ...
  depends_on = [
    ...
    local_file.import_dev_postgres_db_script_output,
    local_file.import_dev_mysql_db_script_output,
...
resource "null_resource" "import_qa_db" {
  ...
  depends_on = [
    ...
    local_file.import_qa_postgres_db_script_output,
    local_file.import_qa_mysql_db_script_output,
...
resource "null_resource" "import_prod_db" {
  ...
  depends_on = [
    ...
    local_file.import_prod_postgres_db_script_output,
    local_file.import_prod_mysql_db_script_output,
