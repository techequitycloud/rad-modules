...
variable "module_description" {
  ...
  type        = string
  default     = "Directus: Deploy Directus open data platform on Google Cloud Run with managed PostgreSQL, Cloud Storage integration, and automated migrations. Instantly generates REST and GraphQL APIs from your database schema with a no-code Admin App for content management. Features serverless scaling, PostGIS geospatial support, GCS Fuse asset storage, real-time WebSockets, and database autonomy without vendor lock-in."
}

variable "module_documentation" {
  description = "The URL to the module documentation. {{UIMeta group=0 order=102 }}"
  type        = string
  default     = "https://docs.techequity.cloud/docs/applications/directus"
}
...
variable "module_services" {
  ...
  type        = list(string)
  default     = ["Cloud Run", "Cloud Run Jobs", "Cloud Build", "Artifact Registry", "Cloud Storage", "GCS Fuse", "Cloud SQL (PostgreSQL 15)", "PostGIS", "VPC Network", "Serverless VPC Access", "Secret Manager", "Cloud IAM", "Cloud Logging", "Cloud Monitoring", "WebSockets"]
}
