...
resource "google_compute_security_policy" "policy" {
  for_each = { for k, v in local.environments : k => v if var.configure_application_security && length(var.application_authorized_network) > 0 && length(var.application_secure_path) > 0 }
...
# Scanner config was commented out in original file, keeping it out but refactoring structure if it were to be uncommented.
