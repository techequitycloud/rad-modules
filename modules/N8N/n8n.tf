locals {
  ...
  n8n_env_vars = {
    ...
    QUEUE_BULL_REDIS_HOST        = var.enable_redis && local.nfs_server_exists ? local.nfs_internal_ip : ""
    QUEUE_BULL_REDIS_PORT        = var.enable_redis && local.nfs_server_exists ? "6379" : ""
  }
