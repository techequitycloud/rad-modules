...
variable "module_description" {
  ...
  type        = string
  default     = "N8N: Deploy n8n fair-code workflow automation platform on Google Cloud Run with PostgreSQL 15 and GCS Fuse persistence. Self-hosted automation tool for connecting apps, APIs, and data sources without complex coding. Features serverless scaling (pay only when running), no per-execution fees, Secret Manager encryption for credentials, automated database setup, webhook endpoints for external triggers, session affinity support, and VPC connectivity for secure internal access. Includes customizable builds, SMTP integration, execution history persistence, and optional Redis queue mode for scalability. Ideal for workflow automation, API integrations, data synchronization, and business process automation with full data ownership."
}

variable "module_documentation" {
  description = "The URL to the module documentation. {{UIMeta group=0 order=102 }}"
  type        = string
  default     = "https://docs.techequity.cloud/docs/applications/n8n"
}
...
variable "module_services" {
  ...
  type        = list(string)
  default     = ["Cloud Run", "Cloud Run Jobs", "Cloud Build", "Artifact Registry", "Cloud Storage", "GCS Fuse", "Cloud SQL (PostgreSQL 15)", "VPC Network", "Serverless VPC Access", "Secret Manager", "Cloud IAM", "Cloud Logging", "Cloud Monitoring", "Health Checks", "Webhooks", "Session Affinity", "Redis", "Filestore (NFS)"]
}
...
variable "application_version" {
  description = "Application version tag (e.g., 2.4.7, latest). {{UIMeta group=0 order=4002 updatesafe }}"
  type        = string
  default     = "2.4.7"
}
