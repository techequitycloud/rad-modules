...
resource "google_monitoring_service" "cloud_run" {
  for_each     = { for k, v in local.environments : k => v if var.configure_monitoring }
  service_id   = "app${var.application_name}${each.key}-monitoring-service-${var.tenant_deployment_id}-${local.random_id}-${local.region}"
  display_name = "app${var.application_name}${each.key}-monitoring-service-${var.tenant_deployment_id}-${local.random_id}"
  project      = local.project.project_id
  ...
  basic_service {
    ...
    service_labels = {
      ...
      service_name = "app${var.application_name}${var.tenant_deployment_id}${local.random_id}${each.key}"
    }
  }
...
resource "google_monitoring_slo" "latency_slo" {
  for_each     = { for k, v in local.environments : k => v if var.configure_monitoring }
  service      = google_monitoring_service.cloud_run[each.key].service_id
  slo_id       = "app${var.application_name}${each.key}-latency-slo-${var.tenant_deployment_id}-${local.random_id}"
  display_name = "app${var.application_name}${each.key}-latency-slo-${var.tenant_deployment_id}-${local.random_id}"
  goal         = 0.95
  project      = local.project.project_id
  calendar_period = "DAY"
...
resource "google_monitoring_slo" "availability_slo" {
  for_each     = { for k, v in local.environments : k => v if var.configure_monitoring }
  service      = google_monitoring_service.cloud_run[each.key].service_id
  slo_id       = "app${var.application_name}${each.key}-availability-slo-${var.tenant_deployment_id}-${local.random_id}"
  display_name = "app${var.application_name}${each.key}-availability-slo-${var.tenant_deployment_id}-${local.random_id}"
  goal         = 0.95
  project      = local.project.project_id
  calendar_period = "DAY"
...
resource "google_monitoring_uptime_check_config" "https" {
  for_each = { for k, v in local.environments : k => v if var.configure_monitoring }
  project = local.project.project_id
  display_name = "app${var.application_name}${each.key}-uptime-check-config-${var.tenant_deployment_id}-${local.random_id}"
  timeout = "60s"
  period = "900s"
