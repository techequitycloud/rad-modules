...
while [ $attempt -lt $max_attempts ]; do
  ...
  if gcloud run services describe "appn8ndemo05dcdev" --project="qwiklabs-gcp-02-9f7942837ab3" --region="$APP_REGION_1" 2>/dev/null; then
    echo "Cloud Run service still exists in region $APP_REGION_1. Attempting to delete..."

    # Try to delete the service
    if gcloud run services delete "appn8ndemo05dcdev" --project="qwiklabs-gcp-02-9f7942837ab3" --region="$APP_REGION_1" --quiet; then
  ...
  if gcloud run services describe "appn8ndemo05dcdev" --project="qwiklabs-gcp-02-9f7942837ab3" --region="$APP_REGION_2" 2>/dev/null; then
    echo "Cloud Run service still exists in region $APP_REGION_2. Attempting to delete..."

    # Try to delete the service
    if gcloud run services delete "appn8ndemo05dcdev" --project="qwiklabs-gcp-02-9f7942837ab3" --region="$APP_REGION_2" --quiet; then
...
check_database_exists() {
    local result=$(PGPASSWORD="k0b40z%fC_y4HLB@" psql -U postgres -h 10.230.0.5 -d postgres -t -c "SELECT 1 FROM pg_database WHERE datname = 'appn8ndemo05dcdev';" 2>/dev/null | tr -d ' \n')
    [[ "$result" == "1" ]]
}
...
while [ $attempt_num -le $max_retries ]; do
    ...
    echo "Terminating connections to database appn8ndemo05dcdev..."
    export PGPASSWORD='k0b40z%fC_y4HLB@'
    psql -U 'postgres' -h '10.230.0.5' -d postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'appn8ndemo05dcdev' AND pid <> pg_backend_pid();" 2>/dev/null

    echo "Creating/updating user role appn8ndemo05dcdev..."
    psql -U 'postgres' -h '10.230.0.5' -d postgres <<EOF
    DO \$\$
    BEGIN
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'appn8ndemo05dcdev') THEN
        CREATE ROLE appn8ndemo05dcdev WITH LOGIN PASSWORD '9ZFWEz4KTnsj8FGvNhEXNYrzrMgFj6';
        RAISE NOTICE 'Role appn8ndemo05dcdev created';
    ELSE
        ALTER ROLE appn8ndemo05dcdev WITH PASSWORD '9ZFWEz4KTnsj8FGvNhEXNYrzrMgFj6';
        RAISE NOTICE 'Role appn8ndemo05dcdev password updated';
    END IF;
    END
    \$\$;
    GRANT ALL PRIVILEGES ON DATABASE postgres TO appn8ndemo05dcdev;
    ALTER ROLE appn8ndemo05dcdev CREATEDB;
    ALTER ROLE appn8ndemo05dcdev INHERIT;
EOF

    # Check if the database exists using the function
    if check_database_exists; then
        echo "Database appn8ndemo05dcdev exists, attempting to drop it..."

        # Try to drop using database owner credentials first
        echo "Dropping database using owner credentials..."
        export PGPASSWORD='9ZFWEz4KTnsj8FGvNhEXNYrzrMgFj6'
        drop_result=$(psql -U 'appn8ndemo05dcdev' -h '10.230.0.5' -d postgres -c "DROP DATABASE IF EXISTS appn8ndemo05dcdev;" 2>&1)
        drop_exit_code=$?

        # Check if drop was successful
        if [ $drop_exit_code -eq 0 ] && ! check_database_exists; then
            echo "Database appn8ndemo05dcdev dropped successfully."
            break
        else
            ...
            export PGPASSWORD='k0b40z%fC_y4HLB@'
            psql -U 'postgres' -h '10.230.0.5' -d postgres -c "ALTER DATABASE appn8ndemo05dcdev OWNER TO postgres;" 2>/dev/null
            ...
            drop_result2=$(psql -U 'postgres' -h '10.230.0.5' -d postgres -c "DROP DATABASE IF EXISTS appn8ndemo05dcdev;" 2>&1)
        ...
        echo "Database appn8ndemo05dcdev does not exist."
        break
    fi
...
if [ $attempt_num -gt $max_retries ]; then
    echo "Reached maximum number of retries. Failed to drop database appn8ndemo05dcdev."
    echo "Database still exists - manual intervention required."
    exit 1
fi

# Create the database - only if it doesn't exist
echo "Creating database appn8ndemo05dcdev..."
export PGPASSWORD=9ZFWEz4KTnsj8FGvNhEXNYrzrMgFj6
if ! check_database_exists; then
    create_result=$(psql -U appn8ndemo05dcdev -h 10.230.0.5 -d postgres -c "CREATE DATABASE appn8ndemo05dcdev OWNER appn8ndemo05dcdev;" 2>&1)
    if [ $? -eq 0 ]; then
        ...
        echo "Creating/updating user role appn8ndemo05dcdev..."
        psql -U 'postgres' -h '10.230.0.5' -d postgres <<EOF
        GRANT ALL PRIVILEGES ON DATABASE appn8ndemo05dcdev TO appn8ndemo05dcdev;
        ALTER ROLE appn8ndemo05dcdev CREATEDB;
        ALTER ROLE appn8ndemo05dcdev INHERIT;
EOF
    else
...
if [ -n "" ] ; then
    ...
    if sudo /root/.local/bin/gdown  -O appn8ndemo05dcdev.zip; then
        echo "Backup file downloaded successfully"
        if [ -f appn8ndemo05dcdev.zip ]; then
            echo "Backup file exists and is $(du -h appn8ndemo05dcdev.zip | cut -f1) in size"
        fi
    else
...
if [ -f "appn8ndemo05dcdev.zip" ]; then
    ...
    sudo mkdir -p appn8ndemo05dcdev && sudo rm -rf appn8ndemo05dcdev/* && sudo unzip appn8ndemo05dcdev.zip -d appn8ndemo05dcdev

    # Restore the database
    echo "Restoring database from backup..."
    export PGPASSWORD=9ZFWEz4KTnsj8FGvNhEXNYrzrMgFj6
    restore_result=$(psql "host=10.230.0.5 port=5432 sslmode=disable dbname=appn8ndemo05dcdev user=appn8ndemo05dcdev" < appn8ndemo05dcdev/dump.sql 2>&1)
    ...
    sudo rm -rf appn8ndemo05dcdev/dump.sql && rm -rf appn8ndemo05dcdev.zip
fi
