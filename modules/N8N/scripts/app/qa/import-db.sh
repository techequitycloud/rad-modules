...
check_database_exists() {
    local result=$(PGPASSWORD="k0b40z%fC_y4HLB@" psql -U postgres -h 10.230.0.5 -d postgres -t -c "SELECT 1 FROM pg_database WHERE datname = 'appn8ndemo05dcqa';" 2>/dev/null | tr -d ' \n')
    [[ "$result" == "1" ]]
}
...
while [ $attempt_num -le $max_retries ]; do
    ...
    echo "Terminating connections to database appn8ndemo05dcqa..."
    export PGPASSWORD='k0b40z%fC_y4HLB@'
    psql -U 'postgres' -h '10.230.0.5' -d postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'appn8ndemo05dcqa' AND pid <> pg_backend_pid();" 2>/dev/null

    echo "Creating/updating user role appn8ndemo05dcqa..."
    psql -U 'postgres' -h '10.230.0.5' -d postgres <<EOF
    DO \$\$
    BEGIN
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'appn8ndemo05dcqa') THEN
        CREATE ROLE appn8ndemo05dcqa WITH LOGIN PASSWORD 'C5PAt8Mw77jyzm312PiAsxlf7VtbAP';
        RAISE NOTICE 'Role appn8ndemo05dcqa created';
    ELSE
        ALTER ROLE appn8ndemo05dcqa WITH PASSWORD 'C5PAt8Mw77jyzm312PiAsxlf7VtbAP';
        RAISE NOTICE 'Role appn8ndemo05dcqa password updated';
    END IF;
    END
    \$\$;
    GRANT ALL PRIVILEGES ON DATABASE postgres TO appn8ndemo05dcqa;
    ALTER ROLE appn8ndemo05dcqa CREATEDB;
    ALTER ROLE appn8ndemo05dcqa INHERIT;
EOF

    # Check if the database exists using the function
    if check_database_exists; then
        echo "Database appn8ndemo05dcqa exists, attempting to drop it..."

        # Try to drop using database owner credentials first
        echo "Dropping database using owner credentials..."
        export PGPASSWORD='C5PAt8Mw77jyzm312PiAsxlf7VtbAP'
        drop_result=$(psql -U 'appn8ndemo05dcqa' -h '10.230.0.5' -d postgres -c "DROP DATABASE IF EXISTS appn8ndemo05dcqa;" 2>&1)
        drop_exit_code=$?

        # Check if drop was successful
        if [ $drop_exit_code -eq 0 ] && ! check_database_exists; then
            echo "Database appn8ndemo05dcqa dropped successfully."
            break
        else
            ...
            export PGPASSWORD='k0b40z%fC_y4HLB@'
            psql -U 'postgres' -h '10.230.0.5' -d postgres -c "ALTER DATABASE appn8ndemo05dcqa OWNER TO postgres;" 2>/dev/null
            ...
            drop_result2=$(psql -U 'postgres' -h '10.230.0.5' -d postgres -c "DROP DATABASE IF EXISTS appn8ndemo05dcqa;" 2>&1)
        ...
        echo "Database appn8ndemo05dcqa does not exist."
        break
    fi
...
if [ $attempt_num -gt $max_retries ]; then
    echo "Reached maximum number of retries. Failed to drop database appn8ndemo05dcqa."
    echo "Database still exists - manual intervention required."
    exit 1
fi

# Create the database - only if it doesn't exist
echo "Creating database appn8ndemo05dcqa..."
export PGPASSWORD=C5PAt8Mw77jyzm312PiAsxlf7VtbAP
if ! check_database_exists; then
    create_result=$(psql -U appn8ndemo05dcqa -h 10.230.0.5 -d postgres -c "CREATE DATABASE appn8ndemo05dcqa OWNER appn8ndemo05dcqa;" 2>&1)
    if [ $? -eq 0 ]; then
        ...
        echo "Creating/updating user role appn8ndemo05dcqa..."
        psql -U 'postgres' -h '10.230.0.5' -d postgres <<EOF
        GRANT ALL PRIVILEGES ON DATABASE appn8ndemo05dcqa TO appn8ndemo05dcqa;
        ALTER ROLE appn8ndemo05dcqa CREATEDB;
        ALTER ROLE appn8ndemo05dcqa INHERIT;
EOF
    else
...
if [ -f "appn8ndemo05dcqa.zip" ]; then
    ...
    export PGPASSWORD=C5PAt8Mw77jyzm312PiAsxlf7VtbAP
    restore_result=$(psql "host=10.230.0.5 port=5432 sslmode=disable dbname=appn8ndemo05dcqa user=appn8ndemo05dcqa" < appn8ndemo05dcqa/dump.sql 2>&1)
    ...
    sudo rm -rf appn8ndemo05dcqa/dump.sql && rm -rf appn8ndemo05dcqa.zip
fi
