...
resource "google_cloud_run_v2_service" "app_service" {
  ...
  template {
    ...
    containers {
      ...
      startup_probe {
        ...
        initial_delay_seconds = each.key == "prod" ? 15 : 10
        timeout_seconds       = each.key == "prod" ? 5 : 3
        period_seconds        = 10
        failure_threshold     = each.key == "prod" ? 5 : 3
      }
      ...
      liveness_probe {
        ...
        initial_delay_seconds = each.key == "prod" ? 60 : 30
        timeout_seconds       = each.key == "prod" ? 10 : 5
        period_seconds        = each.key == "prod" ? 60 : 30
        failure_threshold     = 3
      }
      ...
      dynamic "env" {
        for_each = each.key == "qa" || each.key == "prod" ? [1] : []
        content {
      ...
      env {
        ...
        value_source {
          secret_key_ref {
            secret  = google_secret_manager_secret.encryption_key[each.key].secret_id
            version = "latest"
          }
        }
      }
  ...
  depends_on = [
    ...
    # google_project_iam_member.storage_admin # Removed as per analysis
  ]
}

resource "google_cloud_run_service_iam_binding" "invoker" {
  for_each = { for k, v in local.environments : k => v if local.sql_server_exists && var.public_access }
