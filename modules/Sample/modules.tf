...
locals {
  #########################################################################
  # Preset Selection Logic (Hardcoded for Sample)
  #########################################################################

  # Always use the sample module preset
  using_module    = true
  selected_module = local.application_modules["sample"]
  module_exists   = true
  ...
  module_application_name       = try(local.selected_module.app_name, "sample")
  ...
  module_application_display_name = try(local.selected_module.display_name, "Sample Application")
  ...
  module_application_database_name = try(local.selected_module.db_name, "sample")
  module_application_database_user = try(local.selected_module.db_user, "sample")
