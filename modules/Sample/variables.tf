...
variable "module_description" {
  ...
  type        = string
  default     = "Sample: Reference implementation and copy-pasteable template for deploying custom web applications on GCP. Demonstrates production-ready wrapper module architecture inheriting platform best practices. Full-stack demo with Python 3.11 Flask app on Cloud Run v2, PostgreSQL via Cloud SQL (Unix socket/TCP), automated database initialization via Cloud Run Jobs (db-init), Secret Manager for secure password management, Serverless VPC Access for private networking, and GCS storage. Features wrapper pattern with CloudRunApp foundation inheritance, automated Day 0 operations (DB/user creation), no hardcoded passwords, custom Dockerfile builds, scale-to-zero (0-1 instances), startup/liveness probes, CPU boost, CI/CD hooks, and strict IAM with service accounts and RBAC. Ideal for developers creating custom modules, learning platform best practices for secure networking and secrets management, and deploying production-ready serverless applications with complete compute, database, and storage stack."
}

variable "module_documentation" {
  description = "The URL to the module documentation. {{UIMeta group=0 order=102 }}"
  type        = string
  default     = "https://docs.techequity.cloud/docs/applications/sample"
}
...
variable "module_services" {
  ...
  type        = list(string)
  default     = ["Cloud Run v2", "Cloud Run Jobs", "Cloud Build", "Artifact Registry", "Cloud Storage", "Cloud SQL", "Cloud SQL Auth Proxy", "PostgreSQL 15", "VPC Network", "Serverless VPC Access", "VPC Access Connector", "Unix Socket", "Secret Manager", "Cloud IAM", "Service Accounts", "Cloud Logging", "Cloud Monitoring", "Health Checks", "CPU Boost"]
}
