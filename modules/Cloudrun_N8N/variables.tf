...
variable "module_description" {
  ...
  type        = string
  default     = "This module deploys n8n workflow automation on Google Cloud Run, providing a serverless, secure, and automated environment with PostgreSQL database for persistence."
}
...
variable "n8n_encryption_key" {
  description = "Enter the n8n encryption key used to encrypt credentials in the database. If left empty, a random key will be generated. {{UIMeta group=0 order=515 updatesafe }}"
  type        = string
  default     = ""
  sensitive   = true
}
