...
resource "google_security_scanner_scan_config" "dev_scanner_config" {
  count            = (var.configure_development_environment && var.configure_application_security) ? 1 : 0
  project          = local.project.project_id
...
resource "google_security_scanner_scan_config" "qa_scanner_config" {
  count            = (var.configure_nonproduction_environment && var.configure_application_security) ? 1 : 0
  project          = local.project.project_id
  provider         = google-beta
...
resource "google_security_scanner_scan_config" "prod_scanner_config" {
  count            = (var.configure_production_environment && var.configure_application_security) ? 1 : 0
  project          = local.project.project_id
  provider         = google-beta
