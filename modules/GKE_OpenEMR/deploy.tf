...
timeout 300 bash -c '
gcloud container clusters get-credentials ${local.gke_cluster_name} --region ${local.gke_cluster_region} --project ${local.project.project_id}
kubectl create secret generic app${var.application_name}${local.random_id}dev-root-password --namespace=${var.application_name}${var.tenant_deployment_id}dev --from-literal=password=${local.db_root_password} || true
' || true
EOF
  kubectl_destroy_command     = <<EOF
timeout 300 bash -c '
kubectl delete secret app${var.application_name}${local.random_id}dev-root-password --namespace=${var.application_name}${var.tenant_deployment_id}dev --force || true
' || true
EOF
...
timeout 300 bash -c '
gcloud container clusters get-credentials ${local.gke_cluster_name} --region ${local.gke_cluster_region} --project ${local.project.project_id}
kubectl create secret generic app${var.application_name}${local.random_id}qa-root-password --namespace=${var.application_name}${var.tenant_deployment_id}qa --from-literal=password=${local.db_root_password} || true
' || true
EOF
  kubectl_destroy_command     = <<EOF
timeout 300 bash -c '
kubectl delete secret app${var.application_name}${local.random_id}qa-root-password --namespace=${var.application_name}${var.tenant_deployment_id}qa --force || true
' || true
EOF
...
timeout 300 bash -c '
gcloud container clusters get-credentials ${local.gke_cluster_name} --region ${local.gke_cluster_region} --project ${local.project.project_id}
kubectl create secret generic app${var.application_name}${local.random_id}prod-root-password --namespace=${var.application_name}${var.tenant_deployment_id}prod --from-literal=password=${local.db_root_password} || true
' || true
EOF
  kubectl_destroy_command     = <<EOF
timeout 300 bash -c '
kubectl delete secret app${var.application_name}${local.random_id}prod-root-password --namespace=${var.application_name}${var.tenant_deployment_id}prod --force || true
' || true
EOF
...
timeout 300 bash -c '
gcloud container clusters get-credentials ${local.gke_cluster_name} --region ${local.gke_cluster_region} --project ${local.project.project_id}
kubectl apply -f ${local_file.dev_storage_pv_yaml_output.filename} || true
' || true
EOF
  kubectl_destroy_command     = <<EOF
timeout 300 bash -c '
kubectl delete -f ${local_file.dev_storage_pv_yaml_output.filename} --force || true
' || true
EOF
...
timeout 300 bash -c '
gcloud container clusters get-credentials ${local.gke_cluster_name} --region ${local.gke_cluster_region} --project ${local.project.project_id}
kubectl apply -f ${local_file.dev_storage_pvc_yaml_output.filename} || true
' || true
EOF
  kubectl_destroy_command     = <<EOF
timeout 300 bash -c '
kubectl delete -f ${local_file.dev_storage_pvc_yaml_output.filename} --force || true
' || true
EOF
...
timeout 300 bash -c '
gcloud container clusters get-credentials ${local.gke_cluster_name} --region ${local.gke_cluster_region} --project ${local.project.project_id}
kubectl apply -f ${local_file.qa_storage_pv_yaml_output.filename} || true
' || true
EOF
  kubectl_destroy_command     = <<EOF
timeout 300 bash -c '
kubectl delete -f ${local_file.qa_storage_pv_yaml_output.filename} --force || true
' || true
EOF
...
timeout 300 bash -c '
gcloud container clusters get-credentials ${local.gke_cluster_name} --region ${local.gke_cluster_region} --project ${local.project.project_id}
kubectl apply -f ${local_file.qa_storage_pvc_yaml_output.filename} || true
' || true
EOF
  kubectl_destroy_command     = <<EOF
timeout 300 bash -c '
kubectl delete -f ${local_file.qa_storage_pvc_yaml_output.filename} --force || true
' || true
EOF
...
timeout 300 bash -c '
gcloud container clusters get-credentials ${local.gke_cluster_name} --region ${local.gke_cluster_region} --project ${local.project.project_id}
kubectl apply -f ${local_file.prod_storage_pv_yaml_output.filename} || true
' || true
EOF
  kubectl_destroy_command     = <<EOF
timeout 300 bash -c '
kubectl delete -f ${local_file.prod_storage_pv_yaml_output.filename} --force || true
' || true
EOF
...
module "deploy_prod_storage_pvc" {
  count                       = local.gke_cluster_exists ? 1 : 0
  source                      = "terraform-google-modules/gcloud/google//modules/kubectl-wrapper"
  project_id                  = local.project.project_id
  cluster_name                = local.gke_cluster_name
...
timeout 300 bash -c '
gcloud container clusters get-credentials ${local.gke_cluster_name} --region ${local.gke_cluster_region} --project ${local.project.project_id}
kubectl apply -f ${local_file.prod_storage_pvc_yaml_output.filename} || true
' || true
EOF
  kubectl_destroy_command     = <<EOF
timeout 300 bash -c '
kubectl delete -f ${local_file.prod_storage_pvc_yaml_output.filename} --force || true
' || true
EOF
