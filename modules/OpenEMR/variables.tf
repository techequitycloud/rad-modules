...
variable "module_description" {
  ...
  type        = string
  default     = "This module deploys OpenEMR, an open-source Electronic Medical Records (EMR) and practice management application, on Google Cloud Run, providing a serverless and secure environment with a database, file storage, networking, and features for patient demographics, scheduling, and billing."
}
...
variable "configure_environment" {
  description = "Select to configure environment. Code is committed to the main branch in the github repository. {{UIMeta group=0 order=703 }}"
  type        = bool
  default     = true
}
...
variable "application_backup_fileid" {
  description = "Enter application backup file ID. When enabled, terraform attempts to download the file from Google Drive, and if found, imports the backup files during deployment. {{UIMeta group=0 order=808}}"
  type        = string
  default     = ""
}
