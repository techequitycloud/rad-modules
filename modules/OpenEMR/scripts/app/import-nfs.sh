...
while [ $attempt -lt $max_attempts ]; do
  ...
  if gcloud run services describe "appopenemrdemofc08" --project="qwiklabs-gcp-00-9c58e150e7c1" --region="$APP_REGION_1" 2>/dev/null; then
    echo "Cloud Run service still exists in region $APP_REGION_1. Attempting to delete..."
    
    # Try to delete the service
    if gcloud run services delete "appopenemrdemofc08" --project="qwiklabs-gcp-00-9c58e150e7c1" --region="$APP_REGION_1" --quiet; then
  ...
  if gcloud run services describe "appopenemrdemofc08" --project="qwiklabs-gcp-00-9c58e150e7c1" --region="$APP_REGION_2" 2>/dev/null; then
    echo "Cloud Run service still exists in region $APP_REGION_2. Attempting to delete..."
    
    # Try to delete the service
    if gcloud run services delete "appopenemrdemofc08" --project="qwiklabs-gcp-00-9c58e150e7c1" --region="$APP_REGION_2" --quiet; then
...
sudo mkdir -p /share/appopenemrdemofc08 && sudo chown -R 1000:1000 /share/appopenemrdemofc08 && sudo chmod 775 /share/appopenemrdemofc08

# Check if the shared directory exists
if [ ! -d /share/appopenemrdemofc08 ]; then echo 'Error: /share/appopenemrdemofc08 does not exist.'; exit 1; fi
