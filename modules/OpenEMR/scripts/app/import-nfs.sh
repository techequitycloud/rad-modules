...
while [ $attempt -lt $max_attempts ]; do
  ...
  if gcloud run services describe "appopenemrdemo6209" --project="qwiklabs-gcp-03-7d6c3f1a2c26" --region="$APP_REGION_1" 2>/dev/null; then
    echo "Cloud Run service still exists in region $APP_REGION_1. Attempting to delete..."
    
    # Try to delete the service
    if gcloud run services delete "appopenemrdemo6209" --project="qwiklabs-gcp-03-7d6c3f1a2c26" --region="$APP_REGION_1" --quiet; then
  ...
  if gcloud run services describe "appopenemrdemo6209" --project="qwiklabs-gcp-03-7d6c3f1a2c26" --region="$APP_REGION_2" 2>/dev/null; then
    echo "Cloud Run service still exists in region $APP_REGION_2. Attempting to delete..."
    
    # Try to delete the service
    if gcloud run services delete "appopenemrdemo6209" --project="qwiklabs-gcp-03-7d6c3f1a2c26" --region="$APP_REGION_2" --quiet; then
...
sudo mkdir -p /share/appopenemrdemo6209 && sudo chown -R 1000:1000 /share/appopenemrdemo6209 && sudo chmod 775 /share/appopenemrdemo6209

# Check if the shared directory exists
if [ ! -d /share/appopenemrdemo6209 ]; then echo 'Error: /share/appopenemrdemo6209 does not exist.'; exit 1; fi
