...
check_database_exists() {
    local result=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "10.230.0.3" -e "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = 'appopenemrdemo4181qa';" 2>/dev/null | grep -c "appopenemrdemo4181qa")
    [[ "$result" == "1" ]]
}

# Create/update user with NO global privileges initially
echo "Creating/updating user appopenemrdemo4181qa..."
mysql --defaults-file=$HOME/.my.cnf -u root -h "10.230.0.3" <<EOF
CREATE USER IF NOT EXISTS 'appopenemrdemo4181qa'@'%' IDENTIFIED BY 'lqYYev1apU@eW%w5';
ALTER USER 'appopenemrdemo4181qa'@'%' IDENTIFIED BY 'lqYYev1apU@eW%w5';
# -- ✅ Remove any existing global privileges
# REVOKE ALL PRIVILEGES ON *.* FROM 'appopenemrdemo4181qa'@'%';
FLUSH PRIVILEGES;
EOF
...
while [ $attempt_num -le $max_retries ]; do
    ...
    echo "Terminating connections to database appopenemrdemo4181qa..."
    # Fixed connection termination logic
    kill_statements=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "10.230.0.3" -e "SELECT CONCAT('KILL ', id, ';') FROM INFORMATION_SCHEMA.PROCESSLIST WHERE db = 'appopenemrdemo4181qa' AND id != CONNECTION_ID();" -N -s 2>/dev/null)
    if [ -n "$kill_statements" ]; then
        echo "$kill_statements" | mysql --defaults-file=$HOME/.my.cnf -u root -h "10.230.0.3" 2>/dev/null
    fi

    if check_database_exists; then
        echo "Database appopenemrdemo4181qa exists, attempting to drop it..."
        
        # Try to drop as root user (DB_USER may not have drop privileges yet)
        echo "Dropping database as root user..."
        drop_result=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "10.230.0.3" -e "DROP DATABASE IF EXISTS \`appopenemrdemo4181qa\`;" 2>&1)
        
        if [ $? -eq 0 ] && ! check_database_exists; then
            echo "Database appopenemrdemo4181qa dropped successfully."
            break
        else
            echo "Failed to drop database. Error: $drop_result"
        fi
    else
        echo "Database appopenemrdemo4181qa does not exist."
        break
    fi
...
if [ $attempt_num -gt $max_retries ]; then
    echo "Reached maximum number of retries. Failed to drop database appopenemrdemo4181qa."
    echo "Database still exists - manual intervention required."
    exit 1
fi

# Create the database as root
echo "Creating database appopenemrdemo4181qa..."
if ! check_database_exists; then
    create_result=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "10.230.0.3" -e "CREATE DATABASE \`appopenemrdemo4181qa\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" 2>&1)
    if [ $? -eq 0 ]; then
...
echo "Granting privileges to appopenemrdemo4181qa ONLY on database appopenemrdemo4181qa..."
mysql --defaults-file=$HOME/.my.cnf -u root -h "10.230.0.3" <<EOF
-- Grant all privileges on the specific database only
GRANT ALL PRIVILEGES ON \`appopenemrdemo4181qa\`.* TO 'appopenemrdemo4181qa'@'%';
-- Allow user to grant privileges on this database to others (if needed)
GRANT GRANT OPTION ON \`appopenemrdemo4181qa\`.* TO 'appopenemrdemo4181qa'@'%';
FLUSH PRIVILEGES;
EOF

if [ $? -eq 0 ]; then
    echo "Privileges granted successfully on database appopenemrdemo4181qa only."
else
    echo "Failed to grant privileges on database appopenemrdemo4181qa."
    exit 1
fi

# Verify user has access only to the intended database
echo "Verifying user privileges..."
mysql --defaults-file=$HOME/.my.cnf -u root -h "10.230.0.3" -e "SHOW GRANTS FOR 'appopenemrdemo4181qa'@'%';"
...
if [ -f "appopenemrdemo4181qa.zip" ]; then
    ...
    cat > $HOME/.my.cnf << 'EOF'
    [client]
    user=appopenemrdemo4181qa
    password=lqYYev1apU@eW%w5
    host=10.230.0.3
EOF
    chmod 600 $HOME/.my.cnf

    restore_result=$(mysql --defaults-file=$HOME/.my.cnf -u "appopenemrdemo4181qa" -h "10.230.0.3" "appopenemrdemo4181qa" < "appopenemrdemo4181qa/dump.sql" 2>&1)
    ...
    sudo rm -rf "appopenemrdemo4181qa" && rm -rf "appopenemrdemo4181qa.zip"
fi
