...
check_database_exists() {
    local result=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "172.21.0.3" -e "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = 'appopenermdemof6f5qa';" 2>/dev/null | grep -c "appopenermdemof6f5qa")
    [[ "$result" == "1" ]]
}

# Create/update user with NO global privileges initially
echo "Creating/updating user appopenermdemof6f5qa..."
mysql --defaults-file=$HOME/.my.cnf -u root -h "172.21.0.3" <<EOF
CREATE USER IF NOT EXISTS 'appopenermdemof6f5qa'@'%' IDENTIFIED BY 'GCRApj6u7p6agSYd';
ALTER USER 'appopenermdemof6f5qa'@'%' IDENTIFIED BY 'GCRApj6u7p6agSYd';
# -- ✅ Remove any existing global privileges
# REVOKE ALL PRIVILEGES ON *.* FROM 'appopenermdemof6f5qa'@'%';
FLUSH PRIVILEGES;
EOF
...
while [ $attempt_num -le $max_retries ]; do
    ...
    echo "Terminating connections to database appopenermdemof6f5qa..."
    # Fixed connection termination logic
    kill_statements=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "172.21.0.3" -e "SELECT CONCAT('KILL ', id, ';') FROM INFORMATION_SCHEMA.PROCESSLIST WHERE db = 'appopenermdemof6f5qa' AND id != CONNECTION_ID();" -N -s 2>/dev/null)
    if [ -n "$kill_statements" ]; then
        echo "$kill_statements" | mysql --defaults-file=$HOME/.my.cnf -u root -h "172.21.0.3" 2>/dev/null
    fi

    if check_database_exists; then
        echo "Database appopenermdemof6f5qa exists, attempting to drop it..."
        
        # Try to drop as root user (DB_USER may not have drop privileges yet)
        echo "Dropping database as root user..."
        drop_result=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "172.21.0.3" -e "DROP DATABASE IF EXISTS \`appopenermdemof6f5qa\`;" 2>&1)
        
        if [ $? -eq 0 ] && ! check_database_exists; then
            echo "Database appopenermdemof6f5qa dropped successfully."
            break
        else
            echo "Failed to drop database. Error: $drop_result"
        fi
    else
        echo "Database appopenermdemof6f5qa does not exist."
        break
    fi
...
if [ $attempt_num -gt $max_retries ]; then
    echo "Reached maximum number of retries. Failed to drop database appopenermdemof6f5qa."
    echo "Database still exists - manual intervention required."
    exit 1
fi

# Create the database as root
echo "Creating database appopenermdemof6f5qa..."
if ! check_database_exists; then
    create_result=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "172.21.0.3" -e "CREATE DATABASE \`appopenermdemof6f5qa\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" 2>&1)
    if [ $? -eq 0 ]; then
...
echo "Granting privileges to appopenermdemof6f5qa ONLY on database appopenermdemof6f5qa..."
mysql --defaults-file=$HOME/.my.cnf -u root -h "172.21.0.3" <<EOF
-- Grant all privileges on the specific database only
GRANT ALL PRIVILEGES ON \`appopenermdemof6f5qa\`.* TO 'appopenermdemof6f5qa'@'%';
-- Allow user to grant privileges on this database to others (if needed)
GRANT GRANT OPTION ON \`appopenermdemof6f5qa\`.* TO 'appopenermdemof6f5qa'@'%';
FLUSH PRIVILEGES;
EOF

if [ $? -eq 0 ]; then
    echo "Privileges granted successfully on database appopenermdemof6f5qa only."
else
    echo "Failed to grant privileges on database appopenermdemof6f5qa."
    exit 1
fi

# Verify user has access only to the intended database
echo "Verifying user privileges..."
mysql --defaults-file=$HOME/.my.cnf -u root -h "172.21.0.3" -e "SHOW GRANTS FOR 'appopenermdemof6f5qa'@'%';"
...
if [ -f "appopenermdemof6f5qa.zip" ]; then
    ...
    cat > $HOME/.my.cnf << 'EOF'
    [client]
    user=appopenermdemof6f5qa
    password=GCRApj6u7p6agSYd
    host=172.21.0.3
EOF
    chmod 600 $HOME/.my.cnf

    restore_result=$(mysql --defaults-file=$HOME/.my.cnf -u "appopenermdemof6f5qa" -h "172.21.0.3" "appopenermdemof6f5qa" < "appopenermdemof6f5qa/dump.sql" 2>&1)
    ...
    sudo rm -rf "appopenermdemof6f5qa" && rm -rf "appopenermdemof6f5qa.zip"
fi
