...
check_database_exists() {
    local result=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "172.21.0.3" -e "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = 'appopenermdemof6f5dev';" 2>/dev/null | grep -c "appopenermdemof6f5dev")
    [[ "$result" == "1" ]]
}

# Create/update user with NO global privileges initially
echo "Creating/updating user appopenermdemof6f5dev..."
mysql --defaults-file=$HOME/.my.cnf -u root -h "172.21.0.3" <<EOF
CREATE USER IF NOT EXISTS 'appopenermdemof6f5dev'@'%' IDENTIFIED BY 'GCRApj6u7p6agSYd';
ALTER USER 'appopenermdemof6f5dev'@'%' IDENTIFIED BY 'GCRApj6u7p6agSYd';
# -- ✅ Remove any existing global privileges
# REVOKE ALL PRIVILEGES ON *.* FROM 'appopenermdemof6f5dev'@'%';
FLUSH PRIVILEGES;
EOF
...
while [ $attempt_num -le $max_retries ]; do
    ...
    echo "Terminating connections to database appopenermdemof6f5dev..."
    # Fixed connection termination logic
    kill_statements=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "172.21.0.3" -e "SELECT CONCAT('KILL ', id, ';') FROM INFORMATION_SCHEMA.PROCESSLIST WHERE db = 'appopenermdemof6f5dev' AND id != CONNECTION_ID();" -N -s 2>/dev/null)
    if [ -n "$kill_statements" ]; then
        echo "$kill_statements" | mysql --defaults-file=$HOME/.my.cnf -u root -h "172.21.0.3" 2>/dev/null
    fi

    if check_database_exists; then
        echo "Database appopenermdemof6f5dev exists, attempting to drop it..."
        
        # Try to drop as root user (DB_USER may not have drop privileges yet)
        echo "Dropping database as root user..."
        drop_result=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "172.21.0.3" -e "DROP DATABASE IF EXISTS \`appopenermdemof6f5dev\`;" 2>&1)
        
        if [ $? -eq 0 ] && ! check_database_exists; then
            echo "Database appopenermdemof6f5dev dropped successfully."
            break
        else
            echo "Failed to drop database. Error: $drop_result"
        fi
    else
        echo "Database appopenermdemof6f5dev does not exist."
        break
    fi
...
if [ $attempt_num -gt $max_retries ]; then
    echo "Reached maximum number of retries. Failed to drop database appopenermdemof6f5dev."
    echo "Database still exists - manual intervention required."
    exit 1
fi

# Create the database as root
echo "Creating database appopenermdemof6f5dev..."
if ! check_database_exists; then
    create_result=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "172.21.0.3" -e "CREATE DATABASE \`appopenermdemof6f5dev\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" 2>&1)
    if [ $? -eq 0 ]; then
...
echo "Granting privileges to appopenermdemof6f5dev ONLY on database appopenermdemof6f5dev..."
mysql --defaults-file=$HOME/.my.cnf -u root -h "172.21.0.3" <<EOF
-- Grant all privileges on the specific database only
GRANT ALL PRIVILEGES ON \`appopenermdemof6f5dev\`.* TO 'appopenermdemof6f5dev'@'%';
-- Allow user to grant privileges on this database to others (if needed)
GRANT GRANT OPTION ON \`appopenermdemof6f5dev\`.* TO 'appopenermdemof6f5dev'@'%';
FLUSH PRIVILEGES;
EOF

if [ $? -eq 0 ]; then
    echo "Privileges granted successfully on database appopenermdemof6f5dev only."
else
    echo "Failed to grant privileges on database appopenermdemof6f5dev."
    exit 1
fi

# Verify user has access only to the intended database
echo "Verifying user privileges..."
mysql --defaults-file=$HOME/.my.cnf -u root -h "172.21.0.3" -e "SHOW GRANTS FOR 'appopenermdemof6f5dev'@'%';"
...
if [ -f "appopenermdemof6f5dev.zip" ]; then
    ...
    cat > $HOME/.my.cnf << 'EOF'
    [client]
    user=appopenermdemof6f5dev
    password=GCRApj6u7p6agSYd
    host=172.21.0.3
EOF
    chmod 600 $HOME/.my.cnf

    restore_result=$(mysql --defaults-file=$HOME/.my.cnf -u "appopenermdemof6f5dev" -h "172.21.0.3" "appopenermdemof6f5dev" < "appopenermdemof6f5dev/dump.sql" 2>&1)
    ...
    sudo rm -rf "appopenermdemof6f5dev" && rm -rf "appopenermdemof6f5dev.zip"
fi
