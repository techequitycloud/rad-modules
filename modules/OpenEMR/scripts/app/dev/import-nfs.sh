...
sudo mkdir -p /share/appopenemrdemobd61dev && sudo rm -rf /share/appopenemrdemobd61dev/* && sudo chown -R 1000:1000 /share/appopenemrdemobd61dev && sudo chmod 775 /share/appopenemrdemobd61dev
...
if [ -f "appopenemrdemobd61dev.zip" ]; then
    ...
    # Move directory
    sudo rm -rf /share/appopenemrdemobd61dev/* && sudo mv appopenemrdemobd61dev/* /share/appopenemrdemobd61dev/

    # Change ownership
    sudo chmod -R 0777 /share/appopenemrdemobd61dev && sudo chown -R 1000:1000 /share/appopenemrdemobd61dev

    # Set proper ownership
    sudo chown -R 1000:1000 /share/appopenemrdemobd61dev

    # 2. Secure base permissions
    sudo find /share/appopenemrdemobd61dev -type d -exec chmod 755 {} \;  # Directories
    sudo find /share/appopenemrdemobd61dev -type f -exec chmod 644 {} \;  # Files

    # Make specific directories writable by web server only
    sudo chmod -R 755 /share/appopenemrdemobd61dev/default/documents

    # Secure sensitive files
    sudo chmod 600 /share/appopenemrdemobd61dev/default/sqlconf.php  # DB config

    # Define the path to the sqlconf.php file
    SQLCONF_FILE="/share/appopenemrdemobd61dev/default/sqlconf.php"

    # Replace hardcoded values with environment variables
    sudo sed -i "s/\$host\s*=\s*'[^']*'/\$host = '10.1.0.3'/" "$SQLCONF_FILE"
    sudo sed -i "s/\$port\s*=\s*'[^']*'/\$port = '3306'/" "$SQLCONF_FILE"
    sudo sed -i "s/\$login\s*=\s*'[^']*'/\$login = 'appopenemrdemobd61dev'/" "$SQLCONF_FILE"
    sudo sed -i "s/\$pass\s*=\s*'[^']*'/\$pass = 'JOw4_45TgAwdtZ3V'/" "$SQLCONF_FILE"
    sudo sed -i "s/\$dbase\s*=\s*'[^']*'/\$dbase = 'appopenemrdemobd61dev'/" "$SQLCONF_FILE"
    ...
    sudo rm -rf appopenemrdemobd61dev.zip && sudo rm -rf appopenemrdemobd61dev
fi

# Check if the shared directory exists
if [ ! -d /share/appopenemrdemobd61dev ]; then echo 'Error: /share/appopenemrdemobd61dev does not exist.'; exit 1; fi
