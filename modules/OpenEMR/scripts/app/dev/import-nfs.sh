...
sudo mkdir -p /share/appopenemrdemo4181dev && sudo rm -rf /share/appopenemrdemo4181dev/* && sudo chown -R 1000:1000 /share/appopenemrdemo4181dev && sudo chmod 775 /share/appopenemrdemo4181dev
...
if [ -f "appopenemrdemo4181dev.zip" ]; then
    ...
    # Move directory
    sudo rm -rf /share/appopenemrdemo4181dev/* && sudo mv appopenemrdemo4181dev/* /share/appopenemrdemo4181dev/

    # Change ownership
    sudo chmod -R 0777 /share/appopenemrdemo4181dev && sudo chown -R 1000:1000 /share/appopenemrdemo4181dev

    # Set proper ownership
    sudo chown -R 1000:1000 /share/appopenemrdemo4181dev

    # 2. Secure base permissions
    sudo find /share/appopenemrdemo4181dev -type d -exec chmod 755 {} \;  # Directories
    sudo find /share/appopenemrdemo4181dev -type f -exec chmod 644 {} \;  # Files

    # Make specific directories writable by web server only
    sudo chmod -R 755 /share/appopenemrdemo4181dev/default/documents

    # Secure sensitive files
    sudo chmod 600 /share/appopenemrdemo4181dev/default/sqlconf.php  # DB config

    # Define the path to the sqlconf.php file
    SQLCONF_FILE="/share/appopenemrdemo4181dev/default/sqlconf.php"

    # Replace hardcoded values with environment variables
    sudo sed -i "s/\$host\s*=\s*'[^']*'/\$host = '10.230.0.3'/" "$SQLCONF_FILE"
    sudo sed -i "s/\$port\s*=\s*'[^']*'/\$port = '3306'/" "$SQLCONF_FILE"
    sudo sed -i "s/\$login\s*=\s*'[^']*'/\$login = 'appopenemrdemo4181dev'/" "$SQLCONF_FILE"
    sudo sed -i "s/\$pass\s*=\s*'[^']*'/\$pass = 'lqYYev1apU@eW%w5'/" "$SQLCONF_FILE"
    sudo sed -i "s/\$dbase\s*=\s*'[^']*'/\$dbase = 'appopenemrdemo4181dev'/" "$SQLCONF_FILE"
    ...
    sudo rm -rf appopenemrdemo4181dev.zip && sudo rm -rf appopenemrdemo4181dev
fi

# Check if the shared directory exists
if [ ! -d /share/appopenemrdemo4181dev ]; then echo 'Error: /share/appopenemrdemo4181dev does not exist.'; exit 1; fi
