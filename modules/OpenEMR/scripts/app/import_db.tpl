...
mysql --defaults-file=$HOME/.my.cnf -u root -h "${DB_IP}" -e 'SHOW DATABASES;'
...
check_database_exists() {
    local result=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "${DB_IP}" -e "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = '${DB_NAME}';" 2>/dev/null | grep -c "${DB_NAME}")
    [[ "$result" == "1" ]]
}

# Create/update user with NO global privileges initially
echo "Creating/updating user ${DB_USER}..."
mysql --defaults-file=$HOME/.my.cnf -u root -h "${DB_IP}" <<EOF
...
while [ $attempt_num -le $max_retries ]; do
    ...
    kill_statements=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "${DB_IP}" -e "SELECT CONCAT('KILL ', id, ';') FROM INFORMATION_SCHEMA.PROCESSLIST WHERE db = '${DB_NAME}' AND id != CONNECTION_ID();" -N -s 2>/dev/null)
    if [ -n "$kill_statements" ]; then
        echo "$kill_statements" | mysql --defaults-file=$HOME/.my.cnf -u root -h "${DB_IP}" 2>/dev/null
    fi
    ...
    if check_database_exists; then
        ...
        drop_result=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "${DB_IP}" -e "DROP DATABASE IF EXISTS \`${DB_NAME}\`;" 2>&1)
...
if ! check_database_exists; then
    create_result=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "${DB_IP}" -e "CREATE DATABASE \`${DB_NAME}\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" 2>&1)
    if [ $? -eq 0 ]; then
