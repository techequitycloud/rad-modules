...
check_database_exists() {
    local result=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "10.1.0.3" -e "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = 'appopenemrdemobd61prod';" 2>/dev/null | grep -c "appopenemrdemobd61prod")
    [[ "$result" == "1" ]]
}

# Create/update user with NO global privileges initially
echo "Creating/updating user appopenemrdemobd61prod..."
mysql --defaults-file=$HOME/.my.cnf -u root -h "10.1.0.3" <<EOF
CREATE USER IF NOT EXISTS 'appopenemrdemobd61prod'@'%' IDENTIFIED BY 'JOw4_45TgAwdtZ3V';
ALTER USER 'appopenemrdemobd61prod'@'%' IDENTIFIED BY 'JOw4_45TgAwdtZ3V';
# -- ✅ Remove any existing global privileges
# REVOKE ALL PRIVILEGES ON *.* FROM 'appopenemrdemobd61prod'@'%';
FLUSH PRIVILEGES;
EOF
...
while [ $attempt_num -le $max_retries ]; do
    ...
    echo "Terminating connections to database appopenemrdemobd61prod..."
    # Fixed connection termination logic
    kill_statements=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "10.1.0.3" -e "SELECT CONCAT('KILL ', id, ';') FROM INFORMATION_SCHEMA.PROCESSLIST WHERE db = 'appopenemrdemobd61prod' AND id != CONNECTION_ID();" -N -s 2>/dev/null)
    if [ -n "$kill_statements" ]; then
        echo "$kill_statements" | mysql --defaults-file=$HOME/.my.cnf -u root -h "10.1.0.3" 2>/dev/null
    fi

    if check_database_exists; then
        echo "Database appopenemrdemobd61prod exists, attempting to drop it..."
        
        # Try to drop as root user (DB_USER may not have drop privileges yet)
        echo "Dropping database as root user..."
        drop_result=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "10.1.0.3" -e "DROP DATABASE IF EXISTS \`appopenemrdemobd61prod\`;" 2>&1)
        
        if [ $? -eq 0 ] && ! check_database_exists; then
            echo "Database appopenemrdemobd61prod dropped successfully."
            break
        else
            echo "Failed to drop database. Error: $drop_result"
        fi
    else
        echo "Database appopenemrdemobd61prod does not exist."
        break
    fi
...
if [ $attempt_num -gt $max_retries ]; then
    echo "Reached maximum number of retries. Failed to drop database appopenemrdemobd61prod."
    echo "Database still exists - manual intervention required."
    exit 1
fi

# Create the database as root
echo "Creating database appopenemrdemobd61prod..."
if ! check_database_exists; then
    create_result=$(mysql --defaults-file=$HOME/.my.cnf -u root -h "10.1.0.3" -e "CREATE DATABASE \`appopenemrdemobd61prod\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" 2>&1)
    if [ $? -eq 0 ]; then
...
echo "Granting privileges to appopenemrdemobd61prod ONLY on database appopenemrdemobd61prod..."
mysql --defaults-file=$HOME/.my.cnf -u root -h "10.1.0.3" <<EOF
-- Grant all privileges on the specific database only
GRANT ALL PRIVILEGES ON \`appopenemrdemobd61prod\`.* TO 'appopenemrdemobd61prod'@'%';
-- Allow user to grant privileges on this database to others (if needed)
GRANT GRANT OPTION ON \`appopenemrdemobd61prod\`.* TO 'appopenemrdemobd61prod'@'%';
FLUSH PRIVILEGES;
EOF

if [ $? -eq 0 ]; then
    echo "Privileges granted successfully on database appopenemrdemobd61prod only."
else
    echo "Failed to grant privileges on database appopenemrdemobd61prod."
    exit 1
fi

# Verify user has access only to the intended database
echo "Verifying user privileges..."
mysql --defaults-file=$HOME/.my.cnf -u root -h "10.1.0.3" -e "SHOW GRANTS FOR 'appopenemrdemobd61prod'@'%';"
...
if [ -f "appopenemrdemobd61prod.zip" ]; then
    ...
    cat > $HOME/.my.cnf << 'EOF'
    [client]
    user=appopenemrdemobd61prod
    password=JOw4_45TgAwdtZ3V
    host=10.1.0.3
EOF
    chmod 600 $HOME/.my.cnf

    restore_result=$(mysql --defaults-file=$HOME/.my.cnf -u "appopenemrdemobd61prod" -h "10.1.0.3" "appopenemrdemobd61prod" < "appopenemrdemobd61prod/dump.sql" 2>&1)
    ...
    sudo rm -rf "appopenemrdemobd61prod" && rm -rf "appopenemrdemobd61prod.zip"
fi
