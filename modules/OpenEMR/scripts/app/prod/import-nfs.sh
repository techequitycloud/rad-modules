...
sudo mkdir -p /share/appopenermdemof6f5prod && sudo rm -rf /share/appopenermdemof6f5prod/* && sudo chown -R 1000:1000 /share/appopenermdemof6f5prod && sudo chmod 775 /share/appopenermdemof6f5prod
...
if [ -f "appopenermdemof6f5prod.zip" ]; then
    ...
    # Move directory
    sudo rm -rf /share/appopenermdemof6f5prod/* && sudo mv appopenermdemof6f5prod/* /share/appopenermdemof6f5prod/

    # Change ownership
    sudo chmod -R 0777 /share/appopenermdemof6f5prod && sudo chown -R 1000:1000 /share/appopenermdemof6f5prod

    # Set proper ownership
    sudo chown -R 1000:1000 /share/appopenermdemof6f5prod

    # 2. Secure base permissions
    sudo find /share/appopenermdemof6f5prod -type d -exec chmod 755 {} \;  # Directories
    sudo find /share/appopenermdemof6f5prod -type f -exec chmod 644 {} \;  # Files

    # Make specific directories writable by web server only
    sudo chmod -R 755 /share/appopenermdemof6f5prod/default/documents

    # Secure sensitive files
    sudo chmod 600 /share/appopenermdemof6f5prod/default/sqlconf.php  # DB config

    # Define the path to the sqlconf.php file
    SQLCONF_FILE="/share/appopenermdemof6f5prod/default/sqlconf.php"

    # Replace hardcoded values with environment variables
    sudo sed -i "s/\$host\s*=\s*'[^']*'/\$host = '172.21.0.3'/" "$SQLCONF_FILE"
    sudo sed -i "s/\$port\s*=\s*'[^']*'/\$port = '3306'/" "$SQLCONF_FILE"
    sudo sed -i "s/\$login\s*=\s*'[^']*'/\$login = 'appopenermdemof6f5prod'/" "$SQLCONF_FILE"
    sudo sed -i "s/\$pass\s*=\s*'[^']*'/\$pass = 'GCRApj6u7p6agSYd'/" "$SQLCONF_FILE"
    sudo sed -i "s/\$dbase\s*=\s*'[^']*'/\$dbase = 'appopenermdemof6f5prod'/" "$SQLCONF_FILE"
    ...
    sudo rm -rf appopenermdemof6f5prod.zip && sudo rm -rf appopenermdemof6f5prod
fi

# Check if the shared directory exists
if [ ! -d /share/appopenermdemof6f5prod ]; then echo 'Error: /share/appopenermdemof6f5prod does not exist.'; exit 1; fi
