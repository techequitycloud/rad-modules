...
sudo mkdir -p /share/appopenemrdemo4181prod && sudo rm -rf /share/appopenemrdemo4181prod/* && sudo chown -R 1000:1000 /share/appopenemrdemo4181prod && sudo chmod 775 /share/appopenemrdemo4181prod
...
if [ -f "appopenemrdemo4181prod.zip" ]; then
    ...
    # Move directory
    sudo rm -rf /share/appopenemrdemo4181prod/* && sudo mv appopenemrdemo4181prod/* /share/appopenemrdemo4181prod/

    # Change ownership
    sudo chmod -R 0777 /share/appopenemrdemo4181prod && sudo chown -R 1000:1000 /share/appopenemrdemo4181prod

    # Set proper ownership
    sudo chown -R 1000:1000 /share/appopenemrdemo4181prod

    # 2. Secure base permissions
    sudo find /share/appopenemrdemo4181prod -type d -exec chmod 755 {} \;  # Directories
    sudo find /share/appopenemrdemo4181prod -type f -exec chmod 644 {} \;  # Files

    # Make specific directories writable by web server only
    sudo chmod -R 755 /share/appopenemrdemo4181prod/default/documents

    # Secure sensitive files
    sudo chmod 600 /share/appopenemrdemo4181prod/default/sqlconf.php  # DB config

    # Define the path to the sqlconf.php file
    SQLCONF_FILE="/share/appopenemrdemo4181prod/default/sqlconf.php"

    # Replace hardcoded values with environment variables
    sudo sed -i "s/\$host\s*=\s*'[^']*'/\$host = '10.230.0.3'/" "$SQLCONF_FILE"
    sudo sed -i "s/\$port\s*=\s*'[^']*'/\$port = '3306'/" "$SQLCONF_FILE"
    sudo sed -i "s/\$login\s*=\s*'[^']*'/\$login = 'appopenemrdemo4181prod'/" "$SQLCONF_FILE"
    sudo sed -i "s/\$pass\s*=\s*'[^']*'/\$pass = 'lqYYev1apU@eW%w5'/" "$SQLCONF_FILE"
    sudo sed -i "s/\$dbase\s*=\s*'[^']*'/\$dbase = 'appopenemrdemo4181prod'/" "$SQLCONF_FILE"
    ...
    sudo rm -rf appopenemrdemo4181prod.zip && sudo rm -rf appopenemrdemo4181prod
fi

# Check if the shared directory exists
if [ ! -d /share/appopenemrdemo4181prod ]; then echo 'Error: /share/appopenemrdemo4181prod does not exist.'; exit 1; fi
