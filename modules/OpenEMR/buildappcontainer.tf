...
resource "null_resource" "build_and_push_application_image" {
  count    = var.configure_environment ? 1 : 0
  # Trigger based on the hash of the build-container.sh script and the Dockerfile
  triggers = {
    script_hash = filesha256("${path.module}/scripts/app/build-container.sh")
    dockerfile_hash = length(local_file.app_dockerfile) > 0 ? sha256(local_file.app_dockerfile[0].content) : ""
