...
locals {
  #########################################################################
  # Preset Selection Logic (Hardcoded for Ghost)
  #########################################################################

  # Always use the ghost module preset
  using_module    = true
  selected_module = local.application_modules["ghost"]
  module_exists   = true
  ...
  module_application_name       = try(local.selected_module.app_name, "ghost")
  ...
  module_application_display_name = try(local.selected_module.display_name, "Ghost Blog")
  ...
  module_application_database_name = try(local.selected_module.db_name, "ghost")
  module_application_database_user = try(local.selected_module.db_user, "ghost")
  ...
  # Logic to append version only if no tag exists in the image name
