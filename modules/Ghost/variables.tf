...
variable "module_description" {
  ...
  type        = string
  default     = "This module can be used to deploy Ghost"
}
...
variable "application_module" {
  description = "Select a pre-configured application module for automatic configuration. Defaults to 'ghost' for this module. {{UIMeta group=3 order=299 OPTIONS=ghost updatesafe }}"
  type        = string
  default     = "ghost"
  ...
  validation {
    ...
    error_message = "Application module must be one of: ghost, or leave empty for manual configuration."
  }
}
