...
variable "module_description" {
  ...
  type        = string
  default     = "Ghost: Deploy Ghost publishing platform on Google Cloud Run with managed MySQL 8.0, GCS Fuse content storage, and optimized caching. Built for professional publishers, journalists, and newsletters with native membership features and clean writing experience. Features automated database setup, dynamic URL discovery, privacy-hardened configuration, serverless scaling for traffic spikes, and maintenance-free deployment. Ideal for content creators and membership-based businesses."
}

variable "module_documentation" {
  description = "The URL to the module documentation. {{UIMeta group=0 order=102 }}"
  type        = string
  default     = "https://docs.techequity.cloud/docs/applications/ghost"
}
...
variable "module_services" {
  ...
  type        = list(string)
  default     = ["Cloud Run", "Cloud Run Jobs", "Cloud Build", "Artifact Registry", "Cloud Storage", "GCS Fuse", "Cloud SQL (MySQL 8.0)", "VPC Network", "Serverless VPC Access", "Secret Manager", "Cloud IAM", "Cloud Logging", "Cloud Monitoring", "Health Checks", "Metadata Server", "Cloud CDN", "SMTP Integration"]
}
