...
variable "module_dependency" {
  description = "Ordered list of module names that must be fully deployed before this module can be deployed. The platform enforces this sequence. Defaults to ['AWS Account', 'GCP Project']. {{UIMeta group=0 order=102 }}"
  type        = list(string)
...
variable "cluster_name_prefix" {
  description = "Prefix prepended to all generated cluster and resource names (e.g. 'aws-eks-cluster' produces names like 'aws-eks-cluster-<deployment_id>'). Use lowercase letters, digits, and hyphens only. Defaults to 'aws-eks-cluster'. {{UIMeta group=0 order=301 updatesafe }}"
  type        = string
  default     = "aws-eks-cluster"
}

variable "gcp_location" {
  description = "GCP region where the EKS cluster will be registered in GKE Hub and appear in the Google Cloud console (e.g. 'us-central1', 'europe-west1'). Defaults to 'us-central1'. Must be a region that supports GKE Hub Attached Clusters. {{UIMeta group=2 order=302 updatesafe }}"
  type        = string
  default     = "us-central1"
}

variable "aws_region" {
  description = "AWS region where the EKS cluster, VPC, and supporting resources will be created (e.g. 'us-west-2', 'us-east-1', 'eu-west-1'). Defaults to 'us-west-2'. The subnet_availability_zones must be valid AZs within this region. {{UIMeta group=2 order=303 updatesafe }}"
  type        = string
  default     = "us-west-2"
}

variable "vpc_cidr_block" {
  description = "IPv4 CIDR block for the AWS VPC created for the EKS cluster (e.g. '10.0.0.0/16'). Must not overlap with other VPCs in the same AWS account if VPC peering is planned. Defaults to '10.0.0.0/16'. Only used when deploying the cluster. {{UIMeta group=0 order=305 updatesafe }}"
  type        = string
  default     = "10.0.0.0/16"
}

variable "public_subnet_cidr_blocks" {
  description = "List of IPv4 CIDR blocks for the public subnets, one per availability zone in subnet_availability_zones (e.g. ['10.0.101.0/24', '10.0.102.0/24', '10.0.103.0/24']). Must be subsets of vpc_cidr_block. Only used when enable_public_subnets is true. Defaults to three /24 subnets. {{UIMeta group=0 order=306 updatesafe }}"
  type        = list(string)
  default = [
    "10.0.101.0/24",
    "10.0.102.0/24",
...
variable "private_subnet_cidr_blocks" {
  description = "List of IPv4 CIDR blocks for the private subnets, one per availability zone in subnet_availability_zones (e.g. ['10.0.1.0/24', '10.0.2.0/24', '10.0.3.0/24']). Must be subsets of vpc_cidr_block. Used when enable_public_subnets is false. Defaults to three /24 subnets. {{UIMeta group=0 order=306 updatesafe }}"
  type        = list(string)
  default = [
    "10.0.1.0/24",
    "10.0.2.0/24",
...
variable "subnet_availability_zones" {
  description = "List of AWS availability zones in which to create subnets (e.g. ['us-west-2a', 'us-west-2b', 'us-west-2c']). Must be valid AZs within the selected aws_region. The number of entries must match the number of entries in public_subnet_cidr_blocks and private_subnet_cidr_blocks. Defaults to three AZs in us-west-2. {{UIMeta group=2 order=307 updatesafe }}"
  type        = list(string)
  default     = ["us-west-2a", "us-west-2b", "us-west-2c"]
}

variable "enable_public_subnets" {
  description = "Set to true (default) to place EKS worker nodes in public subnets with direct internet access. Set to false to place nodes in private subnets with internet access via NAT Gateway (recommended for production workloads for improved security). {{UIMeta group=0 order=401 updatesafe }}"
  type        = bool
  default     = true
}

variable "platform_version" {
  description = "GKE Hub Attached Clusters platform version for the managed components installed onto the EKS cluster (format: major.minor.patch-gke.N, e.g. '1.34.0-gke.1'). Must be compatible with the selected k8s_version. Defaults to '1.34.0-gke.1'. {{UIMeta group=0 order=308 updatesafe }}"
  type        = string
  default     = "1.34.0-gke.1"
}

variable "k8s_version" {
  description = "Kubernetes version to deploy on the EKS cluster, specified as major.minor (e.g. '1.34'). Must be a version currently supported by EKS in the selected aws_region. The patch version is managed automatically by EKS. Defaults to '1.34'. {{UIMeta group=0 order=309 updatesafe }}"
  type        = string
  default     = "1.34"
}

variable "node_group_desired_size" {
  description = "Desired number of worker nodes in the EKS managed node group at deployment time. Must be between node_group_min_size and node_group_max_size. Defaults to 2. The cluster autoscaler may adjust this value over time. {{UIMeta group=0 order=402 updatesafe }}"
  type        = number
  default     = 2
}

variable "node_group_max_size" {
  description = "Maximum number of worker nodes the EKS managed node group can scale up to. Must be >= node_group_desired_size. Defaults to 5. Higher values allow greater burst capacity but increase potential AWS compute costs. {{UIMeta group=0 order=403 updatesafe }}"
  type        = number
  default     = 5
}

variable "node_group_min_size" {
  description = "Minimum number of worker nodes the EKS managed node group will maintain. Must be <= node_group_desired_size. Defaults to 2. A minimum of 2 is recommended for high availability. {{UIMeta group=0 order=404 updatesafe }}"
  type        = number
  default     = 2
}
...
variable "trusted_users" {
  description = "List of Google account email addresses granted cluster-admin privileges on the EKS cluster (e.g. ['user@example.com']). Defaults to an empty list (no additional admin users). Entries must be valid, non-blank email addresses with no duplicates. {{UIMeta group=1 order=504 updatesafe }}"
  type        = list(string)
  default     = []
