...
| <a name="input_cluster_name_prefix"></a> [cluster\_name\_prefix](#input\_cluster\_name\_prefix) | Prefix used verbatim for the cluster and all supporting resource names (e.g. 'aws-eks-cluster' produces the EKS cluster 'aws-eks-cluster' and the VPC 'aws-eks-cluster-vpc'). No random suffix is appended, so the value must be unique per GCP project and AWS region. Use lowercase letters, digits, and hyphens only. Defaults to 'aws-eks-cluster'. | `string` | `"aws-eks-cluster"` | no |
...
| <a name="input_platform_version"></a> [platform\_version](#input\_platform\_version) | GKE Hub Attached Clusters platform version for the managed components installed onto the EKS cluster (format: major.minor.patch-gke.N, e.g. '1.35.0-gke.1'). Its minor version must equal k8s\_version or be exactly one below it. Defaults to '1.35.0-gke.1'. Run 'gcloud container attached get-server-config --location=<gcp\_location>' for the versions currently offered. | `string` | `"1.35.0-gke.1"` | no |
...
| <a name="input_trusted_users"></a> [trusted\_users](#input\_trusted\_users) | List of Google account email addresses granted cluster-admin privileges on the EKS cluster (e.g. ['user@example.com']). Required; no default — pass an empty list to grant cluster-admin to the deploying identity only. Entries must be valid, non-blank email addresses with no duplicates. | `list(string)` | n/a | yes |
