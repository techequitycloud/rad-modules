...
resource "google_secret_manager_secret_version" "superuser_password" {
  ...
  secret_data = try(length(var.django_superuser_password) > 0 ? var.django_superuser_password : random_password.superuser_password.result, random_password.superuser_password.result)
