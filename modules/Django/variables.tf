...
variable "module_description" {
  ...
  type        = string
  default     = "Django: Deploy production-ready Python web applications on Google Cloud Run with managed PostgreSQL, GCS Fuse media storage, and automated migrations. Features automated superuser creation, database initialization, secure secret management, non-root execution, health monitoring, and zero-downtime deployments. Ideal for CMS platforms, business applications, and API backends with cost-effective serverless scaling."
}

variable "module_documentation" {
  description = "The URL to the module documentation. {{UIMeta group=0 order=102 }}"
  type        = string
  default     = "https://docs.techequity.cloud/docs/applications/django"
}
...
variable "module_services" {
  ...
  type        = list(string)
  default     = ["Cloud Run", "Cloud Run Jobs", "Cloud Build", "Artifact Registry", "Cloud Storage", "GCS Fuse", "Cloud SQL (PostgreSQL 15)", "PostgreSQL Extensions", "VPC Network", "Serverless VPC Access", "Secret Manager", "Cloud IAM", "Cloud Logging", "Cloud Monitoring", "Health Checks", "Cloud Scheduler"]
}
...
variable "application_version" {
  description = "Application version tag (e.g., 3.11-slim, latest). {{UIMeta group=0 order=4002 updatesafe }}"
  type        = string
  default     = "3.11-slim"
}
...
variable "database_password_length" {
  description = "Length of auto-generated database password (8-64 characters). {{UIMeta group=0 order=811 updatesafe }}"
  type        = number
  default     = 16
...
variable "gcs_volumes" {
  description = "GCS FUSE volume mounts. Specify volume name, bucket name (null for auto-created), mount path, readonly mode, and mount options. {{UIMeta group=0 order=812 updatesafe }}"
  type = list(object({
    name        = string
...
variable "initialization_jobs" {
  description = "Cloud Run jobs for initialization tasks (e.g., database migrations, data seeding). Jobs can mount NFS/GCS volumes and access secrets. {{UIMeta group=0 order=813 updatesafe }}"
  type = list(object({
    name              = string
