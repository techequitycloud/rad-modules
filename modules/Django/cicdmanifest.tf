...
resource "local_file" "cicd_app_cloudbuild" {
...
resource "local_file" "cicd_app_dockerfile" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_environment ? 1 : 0
...
resource "github_repository_file" "app_cloudbuild" {
  count = var.configure_continuous_integration && var.application_git_token != null && var.application_git_token != "" && var.configure_environment ? 1 : 0
  repository          = "${local.project.project_id}-${var.application_name}-${var.tenant_deployment_id}-${local.random_id}"
  branch              = "main"
  commit_message      = "Add file to repo"
  ...
  content             = local_file.cicd_app_cloudbuild[count.index].content

  depends_on      = [
    local_file.cicd_app_cloudbuild,
    null_resource.init_git_repo
  ]
}
...
resource "github_repository_file" "app_dockerfile" {
  ...
  overwrite_on_create = true
  file                = "Dockerfile"
  content             = local_file.cicd_app_dockerfile[count.index].content

  depends_on      = [
    local_file.cicd_app_dockerfile,
    null_resource.init_git_repo
  ]
}
