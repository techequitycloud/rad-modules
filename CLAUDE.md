...
- **Provider version constraints must have an upper bound.** An unbounded `">=X.Y.Z"` lets a fresh `tofu init` silently resolve a new major version and break the module before it ever reaches the cloud provider. Confirmed live: `AKS_GKE`'s `azurerm = ">=3.17.0"` resolved 5.0.1, which made `node_provisioning_profile` a required block on `azurerm_kubernetes_cluster` and failed `tofu validate`/`plan` outright; pinning `"~> 4.0"` (resolving 4.81.0) fixed it with no code change. The same unbounded shape is still present in `EKS_GKE`'s `aws = ">=4.5.0"` and in the `google` constraint of five modules — `Istio_GKE`, `Bank_GKE`, `MC_Bank_GKE` (`>= 5.0`, plus `google-beta` on the first two) and `AKS_GKE`, `EKS_GKE` (`>=5.0.0`) — **now resolving 8.3.0, THREE majors past when most of these modules were written** (it was 7.43.0 when this was written on 2026-08-21; the five crossed a major boundary unattended in the weeks since, which is the whole point). `Container_Migration`, `Migration_Center` and `VMware_Engine` are bounded, at `>= 5.0, < 8.3` — **not the `< 6.0` they carried until 2026-09; dependabot widened all three** (#1014/#1015/#1018), and `Migration_Center`'s `aws` likewise to `>= 5.0, < 6.65` (#1017). `AKS_GKE`'s `azurerm = "~> 4.0"` is bounded too — the pessimistic operator caps it below 5.0 without naming an upper bound, so grepping for `<` will miss it.

**A dependabot-maintained upper bound is a different thing from a tested one, and reads identically.** These bounds now track *latest published minus epsilon* rather than *latest verified*: `< 8.3` was chosen because 8.3.0 is what the registry currently offers, not because 8.2.x was exercised against these modules. That is still far better than unbounded — a bot-raised ceiling moves on a PR someone can see and revert, where an unbounded constraint moves silently on the next `init`. But do not read a narrow-looking upper bound as evidence that version was tried. Treat the unbounded five as latent, not merely theoretical. Lock files (`.terraform.lock.hcl`) are gitignored repo-wide, so the `~>` constraint in `versions.tf`/`provider.tf` is the *only* thing preventing this in CI and on the Cloud Build pipelines, which both run a fresh `init`.
...
**Four other workflows run here, and `terraform-ci.yml` is not the only gate.** `cloudbuild-lint.yml` (see the step-arg limit below), `convention-check.yml`, and the two Claude semantic reviewers:

- **`claude-pr-review.yml`** reviews each PR. **`claude-weekly-audit.yml`** runs a weekly audit and opens (or comments on) a `doc-audit` issue — **gated on TWO independent signals**, not just the model: the Claude narrative being anything other than exactly `CLEAN`, *or* `scripts/check_conventions.py` emitting a `FAIL`. Nothing is reported only when both are clean. That second signal is deterministic and deliberately survives the model being unavailable — a `FAIL` opens an issue on its own even with `ANTHROPIC_API_KEY` unset, and adds the `convention-drift` label. `WARN` findings are legitimate per-module variation and do not trigger it.
- Both **resolve the Anthropic key from Secret Manager first** (`anthropic-api-key` in `GCP_PROJECT_ID`, via the same WIF the other jobs use), falling back to the per-repo GitHub secret, and they **print which source they used** — four hand-copied per-repo duplicates of one credential is why all four RAD repos broke on the same day. Rotate in Secret Manager; the repo secret is a backstop.
- Both are pinned to **Haiku** and print the turn's cost, so a review that silently becomes expensive is visible.
- **They fail LOUDLY** (#995, #1013). The original swallowed a non-zero exit with `|| echo "…error…" > review.txt`, so the step exited 0 while reviewing nothing — for two months, reporting SUCCESS the whole time. `claude` writes auth failures to **stdout**, not stderr, so both streams are dumped on failure; an empty review also fails.
- ⚠️ A red **Security Review** here is therefore ambiguous by design: read the log before assuming it found something. An exhausted Anthropic balance surfaces as `api_error_status 400 / "Credit balance is too low"` with zero tokens used — the reviewer never ran, rather than objecting.
